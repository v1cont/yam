/*
 * Attachment Tool Plug-in -- an attachment processing plug-in for YAM
...
static YamPluginInfo info = {
  ...
  "0.1.0",
  "Hiroyuki Yamamoto",
  "Attachment processing plug-in for YAM"
};
...
plugin_load (void)
{
  ...
  yam_plugin_add_menuitem ("/Tools", _("Remove attachments"), remove_attachment_cb, NULL);
  ...
  while (partinfo)
    {
      ...
      if (partinfo->sub)
      ...
      else if (partinfo->children)
        {
          ...
          err = !remove_attachment_rec (partinfo->children, fp, outfp);
...
static gboolean
has_attach_part (MimeInfo * mimeinfo)
{
...
static gboolean
remove_attachment (MsgInfo * msginfo)
{
  MimeInfo *mimeinfo;
  FILE *fp, *outfp;
...
remove_attachment_cb (void)
{
  ...
  if (yam_plugin_summary_is_locked ())
    return;

  item = yam_plugin_summary_get_current_folder ();
  if (!item || !item->folder || FOLDER_TYPE (item->folder) != F_MH || item->stype == F_VIRTUAL)
    {
      yam_plugin_alertpanel_message (_("Error"), _("This tool is available on local folders only."), 3);
      return;
    }
  ...
  val = yam_plugin_alertpanel (_("Remove attachments"),
  ...
  for (cur = mlist; cur != NULL; cur = cur->next)
    {
      ...
      debug_print ("Removing attachments of message: %u: %s\n", msginfo->msgnum,
                   msginfo->subject ? msginfo->subject : "(No Subject)");
      ...
      yam_plugin_summary_update_by_msgnum (msginfo->msgnum);
    }

  g_slist_free (mlist);

  yam_plugin_summary_redisplay_msg ();
}
