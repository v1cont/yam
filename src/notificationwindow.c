...
static gboolean nwin_button_pressed (GtkWidget * widget, GdkEventButton * event, gpointer data);
static gboolean nwin_entered (GtkWidget * widget, GdkEventCrossing * event, gpointer data);
static gboolean nwin_motion_notify (GtkWidget * widget, GdkEventMotion * event, gpointer data);
...
gint
notification_window_open (const gchar * message, const gchar * submessage, guint timeout)
{
  GtkWidget *window;
  GtkWidget *vbox;
  GtkWidget *msglabel;
  ...
  window_width = NOTIFICATIONWINDOW_WIDTH;
  window_height = NOTIFICATIONWINDOW_HEIGHT;
  ...
  gtk_widget_set_events (window,
                         GDK_EXPOSURE_MASK | GDK_BUTTON_MOTION_MASK | GDK_POINTER_MOTION_MASK |
  ...
  g_signal_connect (G_OBJECT (window), "destroy", G_CALLBACK (nwin_destroy_cb), NULL);
  g_signal_connect (G_OBJECT (window), "button_press_event", G_CALLBACK (nwin_button_pressed), NULL);
  g_signal_connect (G_OBJECT (window), "enter_notify_event", G_CALLBACK (nwin_entered), NULL);
  g_signal_connect (G_OBJECT (window), "motion_notify_event", G_CALLBACK (nwin_motion_notify), NULL);
  ...
  notify_window.notify_tag = g_timeout_add (FADE_REFRESH_RATE, notify_fadein_timeout_cb, NULL);
...
void
notification_window_set_message (const gchar * message, const gchar * submessage)
{
  if (notify_window.window)
    {
...
void
notification_window_close (void)
{
  notification_window_destroy ();
}

static void
notification_window_destroy (void)
{
  if (notify_window.window)
    {
...
static gboolean
notify_fadein_timeout_cb (gpointer data)
{
  gdk_threads_enter ();
  notify_window.fade_length--;
  notify_window.fade_count++;

  gtk_widget_set_opacity (notify_window.window, (gdouble) notify_window.fade_count / FADE_IN_LENGTH);
  ...
  if (notify_window.fade_length <= 0)
    {
      ...
      if (notify_window.timeout > 0)
        notify_window.notify_tag = g_timeout_add (notify_window.timeout * 1000, notify_timeout_cb, NULL);
      gdk_threads_leave ();
      return FALSE;
    }
  gdk_threads_leave ();
...
static gboolean
notify_fadeout_timeout_cb (gpointer data)
{
  gdk_threads_enter ();
  notify_window.fade_length -= FADE_SPEED;
  notify_window.fade_count++;

  gtk_window_move (GTK_WINDOW (notify_window.window), notify_window.x, notify_window.y + notify_window.fade_count * FADE_SPEED);
  gtk_widget_set_opacity (notify_window.window, (gdouble) notify_window.fade_length / (gdk_screen_height () - notify_window.y));
...
notify_timeout_cb (gpointer data)
{
  ...
  notify_window.notify_tag = g_timeout_add (FADE_REFRESH_RATE, notify_fadeout_timeout_cb, NULL);
  gdk_threads_leave ();
...
static gboolean
nwin_button_pressed (GtkWidget * widget, GdkEventButton * event, gpointer data)
{
  if (!event)
    return FALSE;
...
static gboolean
nwin_entered (GtkWidget * widget, GdkEventCrossing * event, gpointer data)
{
  return FALSE;
}

static gboolean
nwin_motion_notify (GtkWidget * widget, GdkEventMotion * event, gpointer data)
{
  if (notify_window.fade_count > 0 && notify_window.notify_event_count == 0)
    {
  ...
  notify_window.fade_count = 0;
  notify_window.notify_event_count = 0;
  gtk_widget_set_opacity (notify_window.window, 1.0);
  gtk_window_move (GTK_WINDOW (notify_window.window), notify_window.x, notify_window.y);
  if (notify_window.timeout > 0)
    notify_window.notify_tag = g_timeout_add (notify_window.timeout * 1000, notify_timeout_cb, NULL);
...
static void
nwin_destroy_cb (GtkWidget * widget, gpointer data)
{
}
