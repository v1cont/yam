...
gint
send_message (const gchar * file, PrefsAccount * ac_prefs, GSList * to_list)
{
  FILE *fp;
  gint val;
...
QueueInfo *
send_get_queue_info (const gchar * file)
{
  ...
  while ((hnum = procheader_get_one_field (buf, sizeof (buf), fp, qentry)) != -1)
    {
      gchar *p;
      ...
      switch (hnum)
        ...
        case Q_SMTPSERVER:
          ...
          break;
        case Q_RECIPIENTS:
          qinfo->to_list = address_list_append (qinfo->to_list, p);
          break;
        case Q_ACCOUNT_ID:
  ...
  if (((!qinfo->ac || (qinfo->ac && qinfo->ac->protocol != A_NNTP)) && !qinfo->to_list) || !qinfo->from)
    {
  ...
  if (!qinfo->ac)
    {
      qinfo->ac = account_find_from_smtp_server (qinfo->from, qinfo->server);
      if (!qinfo->ac)
        {
...
static gint
send_get_queue_contents (QueueInfo * qinfo, const gchar * dest)
{
  FILE *fp;
  glong pos;
  gchar buf[BUFFSIZE];
...
static gint
send_save_queued_message (QueueInfo * qinfo, gboolean filter_msgs)
{
  FolderItem *outbox;
  gboolean drop_done = FALSE;
  gchar tmp[MAXPATHLEN + 1];

  g_snprintf (tmp, sizeof (tmp), "%s%ctmpmsg.out.%08x", get_rc_dir (), G_DIR_SEPARATOR, g_random_int ());
...
void
send_queue_info_free (QueueInfo * qinfo)
{
  if (qinfo == NULL)
    return;
...
gint
send_message_queue (QueueInfo * qinfo)
{
  gint val = 0;
  glong fpos;
  ...
  if (prefs_common.use_extsend && prefs_common.extsend_cmd)
  ...
  else
    {
      ...
      if (qinfo->to_list)
        {
          if (mailac)
            val = send_message_smtp (mailac, qinfo->to_list, qinfo->fp);
          else
            {
              PrefsAccount tmp_ac;
              ...
              val = send_message_smtp (&tmp_ac, qinfo->to_list, qinfo->fp);
            }
        }
      ...
      if (val == 0 && newsac)
        {
          ...
          val = news_post_stream (FOLDER (newsac->folder), qinfo->fp);
          if (val < 0)
            alertpanel_error (_("Error occurred while posting the message to %s ."), newsac->nntp_server);
        }
    }
...
gint
send_message_queue_all (FolderItem * queue, gboolean save_msgs, gboolean filter_msgs)
{
  gint ret = 0;
  GSList *mlist = NULL;
  GSList *cur;
  ...
  for (cur = mlist; cur != NULL; cur = cur->next)
    {
      ...
      if (qinfo->reply_target)
        send_message_set_reply_flag (qinfo->reply_target, msginfo->inreplyto);
...
gint
send_message_set_reply_flag (const gchar * target, const gchar * msgid)
{
  FolderItem *item;
  gint num;
  MsgInfo *msginfo;
  ...
  debug_print ("send_message_set_reply_flag(): " "setting reply flag to: %s\n", target);
      ...
      if (msginfo)
        {
          if (msgid && strcmp2 (msgid, msginfo->msgid) != 0)
            {
              debug_print ("send_message_set_reply_flag(): " "message-id mismatch\n");
              return -1;
            }
      ...
      if (msginfo)
        {
          ...
          if (msginfo->folder)
            msginfo->folder->mark_dirty = TRUE;
          procmsg_add_flags (msginfo->folder, msginfo->msgnum, msginfo->flags);
          procmsg_msginfo_free (msginfo);
        }
    }
...
gint
send_message_set_forward_flags (const gchar * targets)
{
  FolderItem *item;
  gint num;
  MsgInfo *msginfo;
  ...
  debug_print ("send_message_set_forward_flags(): " "setting forward flags to: %s\n", targets);
...
static gint
send_message_local (const gchar * command, FILE * fp)
{
  gchar **argv;
  GPid pid;
  gint child_stdin;
  ...
  if (g_spawn_async_with_pipes (NULL, argv, NULL,
                                G_SPAWN_DO_NOT_REAP_CHILD, NULL, NULL, &pid, &child_stdin, NULL, NULL, NULL) == FALSE)
    {
      g_snprintf (buf, sizeof (buf), _("Can't execute command: %s"), command);
      log_warning ("%s\n", buf);
...
static gint
send_message_smtp (PrefsAccount * ac_prefs, GSList * to_list, FILE * fp)
{
  Session *session;
  ...
  smtp_session->hostname = ac_prefs->set_domain ? g_strdup (ac_prefs->domain) : NULL;
      ...
      if (ac_prefs->smtp_userid)
        {
          ...
          if (ac_prefs->smtp_passwd)
            smtp_session->pass = g_strdup (ac_prefs->smtp_passwd);
          else if (ac_prefs->tmp_smtp_pass)
            smtp_session->pass = g_strdup (ac_prefs->tmp_smtp_pass);
          else
            {
              smtp_session->pass = input_query_password (ac_prefs->smtp_server, smtp_session->user);
              if (!smtp_session->pass)
                smtp_session->pass = g_strdup ("");
              ac_prefs->tmp_smtp_pass = g_strdup (smtp_session->pass);
            }
        }
      else
        {
          ...
          if (ac_prefs->passwd)
            ...
            smtp_session->pass = g_strdup (ac_prefs->tmp_pass);
          else
            {
              smtp_session->pass = input_query_password (ac_prefs->smtp_server, smtp_session->user);
              if (!smtp_session->pass)
                smtp_session->pass = g_strdup ("");
              ac_prefs->tmp_pass = g_strdup (smtp_session->pass);
            }
        }
  ...
  progress_dialog_append (dialog->dialog, NULL, ac_prefs->smtp_server, _("Connecting"), "", NULL);

  g_snprintf (buf, sizeof (buf), _("Connecting to SMTP server: %s ..."), ac_prefs->smtp_server);
  progress_dialog_set_label (dialog->dialog, buf);
  log_message ("%s\n", buf);

  session_set_recv_message_notify (session, send_recv_message, dialog);
  session_set_send_data_progressive_notify (session, send_send_data_progressive, dialog);
  ...
  if (ac_prefs->use_socks && ac_prefs->use_socks_for_send)
    {
      socks_info = socks_info_new (ac_prefs->socks_type,
                                   ...
                                   ac_prefs->use_proxy_auth ? ac_prefs->proxy_pass : NULL);
    }

  inc_lock ();

  if (session_connect_full (session, ac_prefs->smtp_server, port, socks_info) < 0)
    {
  ...
  else if (session->state == SESSION_ERROR && SMTP_SESSION (session)->state == SMTP_QUIT)
    {
      ...
      ret = 0;
           ...
           SMTP_SESSION (session)->state == SMTP_ERROR || SMTP_SESSION (session)->error_val != SM_OK)
    ret = -1;
...
static gint
send_recv_message (Session * session, const gchar * msg, gpointer data)
{
  gchar buf[BUFFSIZE];
  ...
  switch (smtp_session->state)
    ...
    case SMTP_HELO:
      ...
      statusbar_print_all (_("Sending message via %s:%d..."), session->server, session->port);
      break;
    case SMTP_EHLO:
...
static gint
send_send_data_progressive (Session * session, guint cur_len, guint total_len, gpointer data)
{
  gchar buf[BUFFSIZE];
  SendProgressDialog *dialog = (SendProgressDialog *) data;
...
static gint
send_send_data_finished (Session * session, guint len, gpointer data)
{
  SendProgressDialog *dialog = (SendProgressDialog *) data;
...
static void
send_progress_dialog_destroy (SendProgressDialog * dialog)
{
...
static void
send_cancel_button_cb (GtkWidget * widget, gpointer data)
{
  SendProgressDialog *dialog = (SendProgressDialog *) data;
...
static void
send_put_error (Session * session)
{
  gchar *msg;
  gchar *log_msg = NULL;
  gchar *err_msg = NULL;
          ...
          if (session_get_error (session) == SESSION_ERROR_LOOKUP)
            {
              ...
              err_msg = g_strdup_printf (_("SMTP server not found: %s:%d"), session->server, session->port);
            }
          else if (SMTP_SESSION (session)->state == SMTP_READY)
            {
              log_msg = _("Can't connect to SMTP server.");
              err_msg = g_strdup_printf (_("Can't connect to SMTP server: %s:%d"), session->server, session->port);
            }
          else
            {
              log_msg = _("Error occurred while sending the message.");
