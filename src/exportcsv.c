...
static gint
exp_csv_write_email_record (ItemPerson * person, ItemEMail * email, FILE * fp)
{
...
static gint
exp_csv_export_folder_recursive (ItemFolder * itemFolder, FILE * fp)
{
  GList *items;
  GList *cur;
  gint count = 0;
...
static gint
exp_csv_export_data (gchar * csvFile)
{
  FILE *fp;
  ...
  csv = strconcat_csv (expcsv_dlg.delimiter, _("First Name"), _("Last Name"), _("Display Name"), _("Nick Name"),
                       _("E-Mail Address"), _("Remarks"), _("Alias"), NULL);
  debug_print ("%s\n", csv);
  fputs (csv, fp);
  fputc ('\n', fp);
  g_free (csv);
...
exp_csv_file_move ()
{
  ...
  if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (expcsv_dlg.comma_radiobtn)))
...
exp_csv_finish_show ()
{
  ...
  exp_csv_status_show (sMsg);
  gtk_button_set_label (GTK_BUTTON (expcsv_dlg.btnCancel), _("_Close"));
  gtk_widget_grab_focus (expcsv_dlg.btnCancel);
}

static void
exp_csv_next (GtkWidget * widget)
{
  gint pageNum;
  ...
  if (pageNum == PAGE_FILE_INFO)
    {
      if (exp_csv_file_move ())
        {
          gtk_notebook_set_current_page (GTK_NOTEBOOK (expcsv_dlg.notebook), PAGE_FINISH);
          exp_csv_finish_show ();
        }
    }
}
...
static void
exp_csv_cancel (GtkWidget * widget, gpointer data)
{
  gint pageNum;
...
static void
exp_csv_file_select (void)
{
  gchar *sSelFile;

  sSelFile = filesel_select_file (_("Enter CSV File Name"), NULL, GTK_FILE_CHOOSER_ACTION_SAVE);
  if (sSelFile)
    {
      gchar *sUTF8File;
...
static gint
exp_csv_delete_event (GtkWidget * widget, GdkEventAny * event, gpointer data)
{
...
static gboolean
exp_csv_key_pressed (GtkWidget * widget, GdkEventKey * event, gpointer data)
{
...
static void
exp_csv_page_file (gint pageNum, gchar * pageLbl)
{
  GtkWidget *vbox;
  GtkWidget *table;
  GtkWidget *label;
  ...
  gtk_notebook_set_tab_label (GTK_NOTEBOOK (expcsv_dlg.notebook),
  ...
  gtk_grid_attach (GTK_GRID (table), file_btn, 2, 0, 1, 1);
  ...
  /* Button handler */
  g_signal_connect (G_OBJECT (file_btn), "clicked", G_CALLBACK (exp_csv_file_select), NULL);
...
static void
exp_csv_page_finish (gint pageNum, gchar * pageLbl)
{
  GtkWidget *vbox;
  GtkWidget *table;
  GtkWidget *label;
  ...
  gtk_grid_attach (GTK_GRID (table), labelFile, 1, 0, 1, 1);
  ...
  gtk_grid_attach (GTK_GRID (table), labelRecs, 1, 1, 1, 1);
...
exp_csv_dialog_create ()
{
  ...
  g_signal_connect (G_OBJECT (window), "delete_event", G_CALLBACK (exp_csv_delete_event), NULL);
  g_signal_connect (G_OBJECT (window), "key_press_event", G_CALLBACK (exp_csv_key_pressed), NULL);
  ...
  /* Button panel */
  yam_stock_button_set_create (&hbbox, &btnNext, _("Next"), &btnCancel, "yam-cancel", NULL, NULL);
  gtk_box_pack_end (GTK_BOX (vnbox), hbbox, FALSE, FALSE, 0);
  gtk_widget_grab_default (btnNext);

  /* Button handlers */
  g_signal_connect (G_OBJECT (btnNext), "clicked", G_CALLBACK (exp_csv_next), NULL);
  g_signal_connect (G_OBJECT (btnCancel), "clicked", G_CALLBACK (exp_csv_cancel), NULL);
  ...
  expcsv_dlg.status_cid = gtk_statusbar_get_context_id (GTK_STATUSBAR (statusbar), "Export CSV Dialog");
...
gint
addressbook_exp_csv (AddressObject * obj)
{
  ...
  gtk_button_set_label (GTK_BUTTON (expcsv_dlg.btnCancel), _("_Cancel"));
  exp_csv_message ();
  gtk_widget_grab_focus (expcsv_dlg.file_entry);
