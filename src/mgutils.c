...
*/
void
mgu_print_list (GSList * list, FILE * stream)
{
  GSList *node = list;
  while (node)
    {
...
*/
void
mgu_print_dlist (GList * list, FILE * stream)
{
  GList *node = list;
  while (node)
    {
...
static const struct mgu_error_entry *
mgu_error_find (gint err)
{
  gint i;
...
*/
gchar *
mgu_replace_string (gchar * str, const gchar * value)
{
  if (str)
    g_free (str);
  if (value)
...
*/
gchar *
mgu_email_check_empty (gchar * address)
{
  gchar *retVal = NULL;
  if (address)
    {
