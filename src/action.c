...
static void action_update_menu (GtkItemFactory * ifactory, gchar * branch_path, gpointer callback, gpointer data);
static void compose_actions_execute_cb (Compose * compose, guint action_nb, GtkWidget * widget);
static void mainwin_actions_execute_cb (MainWindow * mainwin, guint action_nb, GtkWidget * widget);
static void msgview_actions_execute_cb (MessageView * msgview, guint action_nb, GtkWidget * widget);
static void message_actions_execute (MessageView * msgview, guint action_nb, GSList * msg_list);

static gboolean execute_actions (gchar * action, GSList * msg_list, GtkWidget * text, gint body_pos, MimeInfo * partinfo);

static gchar *parse_action_cmd (gchar * action, MsgInfo * msginfo, GSList * msg_list, MimeInfo * partinfo,
                                const gchar * user_str, const gchar * user_hidden_str, const gchar * sel_str);
...
static void catch_output (GIOChannel *src, GIOCondition cond, gpointer data);
static void catch_input (GIOChannel *src, GIOCondition cond, gpointer data);
static void catch_status (GIOChannel *src, GIOCondition cond, gpointer data);

static gchar *get_user_string (const gchar * action, ActionType type);


ActionType
action_get_type (const gchar * action_str)
{
  const gchar *p;
...
static gchar *
parse_action_cmd (gchar * action, MsgInfo * msginfo, GSList * msg_list, MimeInfo * partinfo,
                  const gchar * user_str, const gchar * user_hidden_str, const gchar * sel_str)
{
  GString *cmd;
  gchar *p;
...
static gboolean
parse_append_filename (GString * cmd, MsgInfo * msginfo)
{
  gchar *filename;
  gchar *p, *q;
  gchar escape_ch[] = "\\ ";
...
static gboolean
parse_append_msgpart (GString * cmd, MsgInfo * msginfo, MimeInfo * partinfo)
{
...
void
action_update_mainwin_menu (GtkItemFactory * ifactory, MainWindow * mainwin)
{
  action_update_menu (ifactory, "/Tools/Actions", mainwin_actions_execute_cb, mainwin);
}

void
action_update_msgview_menu (GtkItemFactory * ifactory, MessageView * msgview)
{
  action_update_menu (ifactory, "/Tools/Actions", msgview_actions_execute_cb, msgview);
}

void
action_update_compose_menu (GtkItemFactory * ifactory, Compose * compose)
{
  action_update_menu (ifactory, "/Tools/Actions", compose_actions_execute_cb, compose);
}

static void
action_update_menu (GtkItemFactory * ifactory, gchar * branch_path, gpointer callback, gpointer data)
{
  GtkWidget *menuitem;
  gchar *menu_path;
  GSList *cur;
  ...
  for (cur = prefs_common.actions_list; cur; cur = cur->next)
    {
      ...
      if (action_p && action_p[2] && action_get_type (&action_p[2]) != ACTION_ERROR)
        {
          action_p[0] = '\0';
          menu_path = g_strdup_printf ("%s/%s", branch_path, action);
          ifentry.path = menu_path;
          gtk_item_factory_create_item (ifactory, &ifentry, data, 1);
          g_free (menu_path);
        }
      g_free (action);
...
static void
compose_actions_execute_cb (Compose * compose, guint action_nb, GtkWidget * widget)
{
  gchar *buf, *action;
  ActionType action_type;
...
static void
mainwin_actions_execute_cb (MainWindow * mainwin, guint action_nb, GtkWidget * widget)
{
  GSList *msg_list;
...
static void
msgview_actions_execute_cb (MessageView * msgview, guint action_nb, GtkWidget * widget)
{
...
static void
message_actions_execute (MessageView * msgview, guint action_nb, GSList * msg_list)
{
  TextView *textview;
  MimeInfo *partinfo;
  gchar *buf;
...
static gboolean
execute_actions (gchar * action, GSList * msg_list, GtkWidget * text, gint body_pos, MimeInfo * partinfo)
{
      ...
      for (cur = children_list; cur; cur = cur->next)
        {
          ...
          ch = g_io_channel_unix_new (child_info->chld_status);
          child_info->tag_status = g_io_add_watch (ch, G_IO_IN | G_IO_PRI, (GIOFunc) catch_status, child_info);
          g_object_unref (ch);
        }
    }
...
static ChildInfo *
fork_child (gchar * cmd, const gchar * msg_str, Children * children)
{
  ...
  if (sync)
    {
      if (pipe (chld_status) || pipe (chld_in) || pipe (chld_out) || pipe (chld_err))
        {
          alertpanel_error (_("Command could not be started. " "Pipe creation failed.\n%s"), g_strerror (errno));
  ...
  child_info->chld_in = (children->action_type & (ACTION_PIPE_IN | ACTION_USER_IN | ACTION_USER_HIDDEN_IN)) ? chld_in[1] : -1;
  child_info->chld_out = chld_out[0];
  child_info->chld_err = chld_err[0];
  child_info->chld_status = chld_status[0];
  child_info->tag_in = -1;
  ch = g_io_channel_unix_new (chld_out[0]);
  child_info->tag_out = g_io_add_watch (ch, G_IO_IN | G_IO_PRI, (GIOFunc) catch_output, child_info);
  g_io_channel_unref (ch);
  ch = g_io_channel_unix_new (chld_out[0]);
  child_info->tag_err = g_io_add_watch (ch, G_IO_IN | G_IO_PRI, (GIOFunc) catch_output, child_info);
  g_io_channel_unref (ch);
...
static void
kill_children_cb (GtkWidget * widget, gpointer data)
{
  GSList *cur;
...
static gint
wait_for_children (Children * children)
{
  gboolean new_output;
  ChildInfo *child_info;
  GSList *cur;
...
static void
send_input (GtkWidget * w, gpointer data)
{
  GIOChannel *ch;
  Children *children = (Children *) data;
  ChildInfo *child_info = (ChildInfo *) children->list->data;

  ch = g_io_channel_unix_new (child_info->chld_in);
  child_info->tag_in = g_io_add_watch (ch, G_IO_IN | G_IO_PRI, (GIOFunc) catch_input, children);
  g_io_channel_unref (ch);
  gtk_widget_set_sensitive (children->input_hbox, FALSE);
}

static gint
delete_io_dialog_cb (GtkWidget * w, GdkEvent * e, gpointer data)
{
...
static void
hide_io_dialog_cb (GtkWidget * w, gpointer data)
{
  ...
  if (!children->nb)
    {
      g_signal_handlers_disconnect_matched (G_OBJECT (children->dialog), (GSignalMatchType) G_SIGNAL_MATCH_DATA,
...
static gint
io_dialog_key_pressed_cb (GtkWidget * widget, GdkEventKey * event, gpointer data)
{
...
static void
childinfo_close_pipes (ChildInfo * child_info)
{
...
static void
free_children (Children * children)
{
  ChildInfo *child_info;
...
static void
update_io_dialog (Children * children)
{
  GSList *cur;
  ...
  if (!children->nb)
    {
      ...
      g_signal_connect (G_OBJECT (children->dialog),
                        "key_press_event", G_CALLBACK (io_dialog_key_pressed_cb), children);
    }
...
static void
create_io_dialog (Children * children)
{
  GtkWidget *dialog;
  GtkWidget *vbox;
  ...
  yam_stock_button_set_create (&hbox, &abort_button, _("Abort"), &close_button, "yam-close", NULL, NULL);
...
static void
catch_status (GIOChannel *src, GIOCondition cond, gpointer data)
{
  ...
  c = read (g_io_channel_unix_get_fd (src), &buf, 1);
  if (c == 1)
...
static void
catch_input (GIOChannel *src, GIOCondition cond, gpointer data)
{
  Children *children = (Children *) data;
  ChildInfo *child_info = (ChildInfo *) children->list->data;
  gchar *input;
...
static void
catch_output (GIOChannel *src, GIOCondition cond, gpointer data)
{
  ChildInfo *child_info = (ChildInfo *) data;
  gint c;
  gchar buf[BUFFSIZE];
  ...
  sfd = g_io_channel_unix_get_fd (src);

  if (child_info->children->action_type & (ACTION_PIPE_OUT | ACTION_INSERT) && sfd == child_info->chld_out)
    {
      ...
      while (TRUE)
        {
          ...
          if (ret_str && bytes_written > 0)
            {
              gtk_text_buffer_insert (textbuf, &iter, ret_str, bytes_written);
              g_free (ret_str);
            }
          else
...
static gchar *
get_user_string (const gchar * action, ActionType type)
{
  gchar *message;
  gchar *user_str = NULL;
