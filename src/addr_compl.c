...
static void address_completion_entry_changed (GtkEditable * editable, gpointer data);
...
static gint
address_entry_find_func (gconstpointer a, gconstpointer b)
{
...
static void
add_completion_entry (const gchar *str, address_entry *ae)
{
  completion_entry *ce;
 ...
 */
static gint
add_address (const gchar * name, const gchar * firstname, const gchar * lastname, const gchar * nickname,
             const gchar * address)
{
  address_entry *ae;
  GList *found;
  ...
  if ((found = g_list_find_custom (address_list, ae, address_entry_find_func)))
    {
      g_free (ae->name);
      g_free (ae->address);
      g_free (ae);
 ...
 */
gchar *
get_address_from_edit (GtkEntry * entry, gint * start_pos)
{
  ...
  for (; *p && (*p == ',' || g_ascii_isspace (*p)); p = g_utf8_next_char (p))
    ;
 ...
 */
void
replace_address_in_edit (GtkEntry * entry, const gchar * newtext, gint start_pos)
{
  gchar *origtext;
  ...
  g_signal_handlers_block_by_func (entry, address_completion_entry_changed, NULL);
  ...
  g_signal_handlers_unblock_by_func (entry, address_completion_entry_changed, NULL);
}

#if 0
static gint
insert_address_func (gconstpointer a, gconstpointer b)
{
  const address_entry *ae1 = a;
  const address_entry *ae2 = b;
  gchar *s1, *s2;
 ...
 */
guint
complete_address (const gchar * str)
{
  GList *result;
  gchar *d;
  guint count, cpl;
      ...
      for (cpl = 0, result = g_list_first (result); result != NULL; result = g_list_next (result))
        {
          ...
          if (NULL == g_slist_find (completion_addresses, ce->ref))
            {
              cpl++;
              completion_addresses = g_slist_append (completion_addresses, ce->ref);
            }
        }
  ...
  if (index < completion_count)
    {
      if (index == 0)
      ...
      else
        {
          ...
          p = (address_entry *) g_slist_nth_data (completion_addresses, index - 1);
          if (p != NULL)
            {
              if (!p->name || p->name[0] == '\0')
                address = g_strdup (p->address);
              else if (p->name[0] != '"' && strpbrk (p->name, "(),.:;<>@[]") != NULL)
                address = g_strdup_printf ("\"%s\" <%s>", p->name, p->address);
              else
                address = g_strdup_printf ("%s <%s>", p->name, p->address);
            }
        }
...
#define WINDOW_DATA_COMPL_LIST "compl_list"   /* used to store list for compl. window */
...
static void address_completion_create_completion_window (GtkEntry * entry, gboolean select_next);

static void completion_window_select_row (GtkTreeView *list, GtkTreePath *path, GtkTreeViewColumn *col, GtkWidget ** window);
static gboolean completion_window_button_press (GtkWidget * widget, GdkEventButton * event, GtkWidget ** window);
static gboolean completion_window_key_press (GtkWidget * widget, GdkEventKey * event, GtkWidget ** window);

static void
completion_window_advance_selection (GtkTreeView *list, gboolean forward)
{
  ...
  gtk_widget_freeze_child_notify (GTK_WIDGET (list));
  gtk_tree_selection_select_iter (sel, &iter);
  gtk_widget_thaw_child_notify (GTK_WIDGET (list));
}

/* completion_window_apply_selection() - apply the current selection in the list */
static void
completion_window_apply_selection (GtkTreeView *list, GtkEntry *entry)
{
  GtkTreeSelection *sel;
  GtkTreeModel *model;
  GtkTreeIter iter;
...
static void
completion_window_apply_selection_address_only (GtkTreeView *list, GtkEntry *entry)
{
  GtkTreeSelection *sel;
  GtkTreeModel *model;
  GtkTreeIter iter;
 ...
 * completion entries */
void
address_completion_start (GtkWidget * mainwindow)
{
  start_address_completion ();

  /* register focus change hook */
  g_signal_connect (G_OBJECT (mainwindow), "set_focus",
                    G_CALLBACK (address_completion_mainwindow_set_focus), mainwindow);
}
...
void
address_completion_register_entry (GtkEntry * entry)
{
  g_return_if_fail (entry != NULL);
  ...
  /* add keypress event */
  g_signal_connect_closure (G_OBJECT (entry), "key_press_event",
                            g_cclosure_new (G_CALLBACK (address_completion_entry_key_pressed), COMPLETION_UNIQUE_DATA, NULL), FALSE);
  g_signal_connect (G_OBJECT (entry), "changed", G_CALLBACK (address_completion_entry_changed), NULL);
}

void
address_completion_unregister_entry (GtkEntry * entry)
{
  GObject *entry_obj;
  ...
  g_signal_handlers_disconnect_by_func
    (G_OBJECT (entry), G_CALLBACK (address_completion_entry_key_pressed), COMPLETION_UNIQUE_DATA);
}
 ...
 * the window */
void
address_completion_end (GtkWidget * mainwindow)
{
...
static void
address_completion_mainwindow_set_focus (GtkWindow * window, GtkWidget * widget, gpointer data)
{
 ...
 * completion cache */
static gboolean
address_completion_entry_key_pressed (GtkEntry * entry, GdkEventKey * ev, gpointer data)
{
  if (!prefs_common.fullauto_completion_mode && ev->keyval == GDK_KEY_Tab && !completion_window)
    {
  ...
  if (ev->keyval == GDK_KEY_Up || ev->keyval == GDK_KEY_Down || ev->keyval == GDK_KEY_Page_Up || ev->keyval == GDK_KEY_Page_Down
      || ev->keyval == GDK_KEY_Return || ev->keyval == GDK_KEY_Escape || ev->keyval == GDK_KEY_Tab || ev->keyval == GDK_KEY_ISO_Left_Tab)
    {
      completion_window_key_press (completion_window, ev, &completion_window);
      return TRUE;
    }
  else if (ev->keyval == GDK_KEY_Shift_L || ev->keyval == GDK_KEY_Shift_R || ev->keyval == GDK_KEY_Control_L || ev->keyval == GDK_KEY_Control_R
           || ev->keyval == GDK_KEY_Caps_Lock || ev->keyval == GDK_KEY_Shift_Lock || ev->keyval == GDK_KEY_Meta_L
           || ev->keyval == GDK_KEY_Meta_R || ev->keyval == GDK_KEY_Alt_L || ev->keyval == GDK_KEY_Alt_R)
    {
...
static void
address_completion_entry_changed (GtkEditable * editable, gpointer data)
{
  ...
  g_signal_handlers_block_by_func (editable, address_completion_entry_changed, data);
  if (address_completion_complete_address_in_entry (entry, TRUE))
  ...
  g_signal_handlers_unblock_by_func (editable, address_completion_entry_changed, data);
}
 ...
 * his / her mind again. :) */
static gboolean
address_completion_complete_address_in_entry (GtkEntry * entry, gboolean next)
{
  ...
  if (is_completion_pending ())
    new = next ? get_next_complete_address () : get_prev_complete_address ();
  else
    {
...
static void
address_completion_create_completion_window (GtkEntry * entry_, gboolean select_next)
{
  gint x, y, width;
   ...
   * list window to which this completion window has been attached */
  g_object_set_data (G_OBJECT (completion_window), WINDOW_DATA_COMPL_ENTRY, entry_);
  g_object_set_data (G_OBJECT (completion_window), WINDOW_DATA_COMPL_LIST, list);

  g_signal_connect (G_OBJECT (list), "row-activated", G_CALLBACK (completion_window_select_row), &completion_window);

  for (count = 0; count < get_completion_count (); count++)
    {
      gchar *txt;

      txt = get_complete_address (count);
      gtk_list_store_append (store, &iter);
      gtk_list_store_set (store, &iter, 0, txt, 1, count, -1);
      g_free (txt);
    }

  gdk_window_get_origin (gtk_widget_get_window (entry), &x, &y);
  gtk_widget_size_request (entry, &r);
  width = gtk_widget_get_allocated_width (entry);
  y += r.height;
  gtk_window_move (GTK_WINDOW (completion_window), x, y);

  gtk_widget_get_preferred_size (list, NULL, &r);
  ...
  if ((y + r.height) > gdk_screen_height ())
    gtk_widget_set_size_request (completion_window, width, gdk_screen_height () - y);

  g_signal_connect (G_OBJECT (completion_window), "button-press-event", G_CALLBACK (completion_window_button_press), &completion_window);
  g_signal_connect (G_OBJECT (completion_window), "key-press-event", G_CALLBACK (completion_window_key_press), &completion_window);
  gdk_pointer_grab (gtk_widget_get_window (completion_window), TRUE, GDK_POINTER_MOTION_MASK | GDK_BUTTON_PRESS_MASK |
  ...
  gtk_widget_set_can_focus (list, FALSE);
...
static void
completion_window_select_row (GtkTreeView *list, GtkTreePath *path, GtkTreeViewColumn *col, GtkWidget ** window)
{
  GtkEntry *entry;
  ...
  entry = GTK_ENTRY (g_object_get_data (G_OBJECT (*window), WINDOW_DATA_COMPL_ENTRY));
  g_return_if_fail (entry != NULL);

  completion_window_apply_selection (list, entry);
...
static gboolean
completion_window_button_press (GtkWidget * widget, GdkEventButton * event, GtkWidget ** window)
{
  ...
  entry = GTK_WIDGET (g_object_get_data (G_OBJECT (*window), WINDOW_DATA_COMPL_ENTRY));
  g_return_val_if_fail (entry != NULL, FALSE);
...
static gboolean
completion_window_key_press (GtkWidget * widget, GdkEventKey * event, GtkWidget ** window)
{
  GtkWidget *entry;
  gchar *prefix;
  gint cursor_pos;
  ...
  entry = GTK_WIDGET (g_object_get_data (G_OBJECT (*window), WINDOW_DATA_COMPL_ENTRY));
  list = GTK_WIDGET (g_object_get_data (G_OBJECT (*window), WINDOW_DATA_COMPL_LIST));
  g_return_val_if_fail (entry != NULL, FALSE);

  /* allow keyboard navigation in the alternatives list */
  if (event->keyval == GDK_KEY_Up || event->keyval == GDK_KEY_Down ||
      event->keyval == GDK_KEY_Page_Up || event->keyval == GDK_KEY_Page_Down)
    {
      completion_window_advance_selection
        (GTK_TREE_VIEW (list), event->keyval == GDK_KEY_Down || event->keyval == GDK_KEY_Page_Down ? TRUE : FALSE);
      return FALSE;
    }
  ...
  if (event->keyval == GDK_KEY_Tab || event->keyval == GDK_KEY_ISO_Left_Tab)
    {
      event->keyval = (event->state & GDK_SHIFT_MASK) ? GDK_KEY_Up : GDK_KEY_Down;
      /* need to reset shift state if going up */
      if (event->state & GDK_SHIFT_MASK)
        event->state &= ~GDK_SHIFT_MASK;
      completion_window_advance_selection (GTK_TREE_VIEW (list), event->keyval == GDK_KEY_Down ? TRUE : FALSE);
      return FALSE;
    }

  /* look for presses that accept the selection */
  if (event->keyval == GDK_KEY_Return || (!prefs_common.fullauto_completion_mode && event->keyval == GDK_KEY_space))
    {
      /* insert address only if shift or control is pressed */
      if (event->state & (GDK_SHIFT_MASK | GDK_CONTROL_MASK) || prefs_common.always_add_address_only)
        completion_window_apply_selection_address_only (GTK_TREE_VIEW (list), GTK_ENTRY (entry));
      clear_completion_cache ();
      gtk_widget_destroy (*window);
      *window = NULL;
      return FALSE;
  ...
  if (event->keyval == GDK_KEY_Shift_L || event->keyval == GDK_KEY_Shift_R || event->keyval == GDK_KEY_Control_L
      || event->keyval == GDK_KEY_Control_R || event->keyval == GDK_KEY_Caps_Lock || event->keyval == GDK_KEY_Shift_Lock
      || event->keyval == GDK_KEY_Meta_L || event->keyval == GDK_KEY_Meta_R || event->keyval == GDK_KEY_Alt_L || event->keyval == GDK_KEY_Alt_R)
    return FALSE;

  /* other key, let's restore the prefix (orignal text) */
  if (!prefs_common.fullauto_completion_mode || event->keyval == GDK_KEY_Escape)
    {
      prefix = get_complete_address (0);
      g_free (get_address_from_edit (GTK_ENTRY (entry), &cursor_pos));
      replace_address_in_edit (GTK_ENTRY (entry), prefix, cursor_pos);
  ...
  if ((!prefs_common.fullauto_completion_mode && event->length > 0 && event->keyval != GDK_KEY_Escape)
      || (prefs_common.fullauto_completion_mode && event->keyval != GDK_KEY_Escape))
    {
      ...
      while ((pwin = gtk_widget_get_parent (pwin)) != NULL)
        {
          if (gtk_widget_is_toplevel (pwin))
            {
              gtk_window_propagate_key_event (GTK_WINDOW (pwin), event);
              if (prefs_common.fullauto_completion_mode)
                return TRUE;
            }
        }
