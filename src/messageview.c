...
static void messageview_change_view_type (MessageView * messageview, MessageType type);
static void messageview_set_tool_menu (MessageView * messageview, MimeInfo * mimeinfo);
...
static void messageview_toggle_attach_cb (GtkToggleButton * toggle, MessageView * messageview);
static void messageview_switch_page_cb (GtkNotebook * notebook, GtkWidget * page, guint page_num, MessageView * messageview);
static gint messageview_menu_tool_btn_pressed (GtkWidget * widget, GdkEventButton * event, MessageView * messageview);
...
static GtkItemFactoryEntry msgview_entries[] = {
  ...
  {N_("/_Edit/_Find in current message..."), NULL, search_cb, 0, NULL},
...
#undef ENC_ACTION
  ...
  {N_("/_View/All _headers"), NULL, show_all_header_cb, 0, "<ToggleItem>"},

  {N_("/_Message"), NULL, NULL, 0, "<Branch>"},
  {N_("/_Message/Compose _new message"), NULL, compose_cb, 0, NULL},
  {N_("/_Message/---"), NULL, NULL, 0, "<Separator>"},
  {N_("/_Message/_Reply"), NULL, reply_cb, COMPOSE_REPLY, NULL},
  {N_("/_Message/Repl_y to/_all"), NULL, reply_cb, COMPOSE_REPLY_TO_ALL, NULL},
  {N_("/_Message/Repl_y to/_sender"), NULL, reply_cb, COMPOSE_REPLY_TO_SENDER, NULL},
  {N_("/_Message/Repl_y to/mailing _list"), NULL, reply_cb, COMPOSE_REPLY_TO_LIST, NULL},
  {N_("/_Message/---"), NULL, NULL, 0, "<Separator>"},
  {N_("/_Message/_Forward"), NULL, reply_cb, COMPOSE_FORWARD, NULL},
  {N_("/_Message/For_ward as attachment"), NULL, reply_cb, COMPOSE_FORWARD_AS_ATTACH, NULL},
...
messageview_create (void)
{
  ...
  gtk_notebook_set_tab_label_text (GTK_NOTEBOOK (notebook), GTK_WIDGET_PTR (textview), _("Text"));

  gtk_container_add (GTK_CONTAINER (notebook), GTK_WIDGET_PTR (mimeview));
  gtk_notebook_set_tab_label_text (GTK_NOTEBOOK (notebook), GTK_WIDGET_PTR (mimeview), _("Attachments"));

  gtk_notebook_set_current_page (GTK_NOTEBOOK (notebook), 0);
  gtk_widget_show_all (notebook);

  /* Attachment toolbar */
  toolbar_vbox = gtk_box_new (GTK_ORIENTATION_VERTICAL, 0);
  ...
  gtk_box_pack_start (GTK_BOX (toolbar_hbox), mime_toggle_btn, FALSE, FALSE, 0);
  g_signal_connect (G_OBJECT (mime_toggle_btn), "toggled", G_CALLBACK (messageview_toggle_attach_cb), messageview);

  image = stock_pixbuf_widget_scale (STOCK_PIXMAP_MAIL_ATTACH, 18, 18);
  gtk_widget_show (image);
  gtk_container_add (GTK_CONTAINER (mime_toggle_btn), image);

  gtk_widget_set_tooltip_text (mime_toggle_btn, _("Switch to attachment list view"));
  ...
  gtk_widget_set_can_focus (menu_tool_btn, FALSE);
  gtk_widget_show (menu_tool_btn);
  gtk_box_pack_start (GTK_BOX (toolbar_hbox), menu_tool_btn, FALSE, FALSE, 0);
  g_signal_connect (G_OBJECT (menu_tool_btn), "button_press_event", G_CALLBACK (messageview_menu_tool_btn_pressed), messageview);
  ...
  g_signal_connect (G_OBJECT (menuitem), "activate", G_CALLBACK (messageview_save_all_cb), messageview);
  ...
  g_object_ref (G_OBJECT (GTK_WIDGET_PTR (mimeview->textview)));
  g_object_ref (G_OBJECT (GTK_WIDGET_PTR (mimeview->imageview)));

  g_signal_connect (G_OBJECT (notebook), "switch_page", G_CALLBACK (messageview_switch_page_cb), messageview);
...
MessageView *
messageview_create_with_new_window (void)
{
  MessageView *msgview;
  GtkWidget *window;
  ...
  gtk_window_set_title (GTK_WINDOW (window), _("Message View - YAM"));
  gtk_window_set_default_size (GTK_WINDOW (window), prefs_common.msgwin_width, prefs_common.msgwin_height);
  ...
  menubar = menubar_create (window, msgview_entries, n_menu_entries, "<MessageView>", msgview);
  gtk_box_pack_start (GTK_BOX (window_vbox), menubar, FALSE, TRUE, 0);

  vspacer = gtk_box_new (GTK_ORIENTATION_VERTICAL, 0);
  gtk_box_pack_start (GTK_BOX (window_vbox), vspacer, FALSE, TRUE, BORDER_WIDTH);

  body_vbox = gtk_box_new (GTK_ORIENTATION_VERTICAL, BORDER_WIDTH);
  gtk_box_pack_start (GTK_BOX (window_vbox), body_vbox, TRUE, TRUE, 0);

  gtk_box_pack_start (GTK_BOX (body_vbox), GTK_WIDGET_PTR (msgview), TRUE, TRUE, 0);
...
void
messageview_init (MessageView * messageview)
{
...
void
messageview_reflect_prefs (MessageView * messageview)
{
...
GList *
messageview_get_window_list (void)
{
  return messageview_list;
}

gint
messageview_show (MessageView * messageview, MsgInfo * msginfo, gboolean all_headers)
{
  gchar *file;
  MimeInfo *mimeinfo;
  ...
  if (messageview->window && msginfo->subject)
    {
      ...
      title = g_strconcat (msginfo->subject, " - YAM", NULL);
  ...
  if (mimeinfo->mime_type != MIME_TEXT && (prefs_common.html_only_as_attach || mimeinfo->mime_type != MIME_TEXT_HTML))
    {
  ...
  yam_plugin_signal_emit ("messageview-show", messageview, msginfo, all_headers);

  return 0;
}

static void
messageview_set_mime_view_layout (MessageView * messageview)
{
  ...
  gtk_notebook_set_show_tabs (GTK_NOTEBOOK (messageview->notebook), prefs_common.show_attach_tab);
}

static void
messageview_change_view_type (MessageView * messageview, MessageType type)
{
  GtkWidget *notebook = messageview->notebook;

  if (messageview->type == type)
    return;

  if (type == MVIEW_MIME)
    {
      messageview_set_mime_view_layout (messageview);
      gtk_notebook_set_current_page (GTK_NOTEBOOK (notebook), messageview->current_page);
...
static void
messageview_set_tool_menu (MessageView * messageview, MimeInfo * mimeinfo)
{
  MimeInfo *partinfo;
  const gchar *name;
  GtkWidget *menuitem;
  ...
  for (partinfo = mimeinfo; partinfo != NULL; partinfo = procmime_mimeinfo_next (partinfo))
    {
      if (partinfo->filename || partinfo->name)
        {
          name = basename (partinfo->filename ? partinfo->filename : partinfo->name);
          if (*name == '\0')
            name = "mimetmp";
          ...
          g_signal_connect (G_OBJECT (menuitem), "activate", G_CALLBACK (messageview_open_file_cb), messageview);
        }
    }
...
static void
messageview_set_menu_state (MessageView * messageview)
{
  GtkItemFactory *ifactory;
  GtkWidget *menuitem;
  ...
  gtk_check_menu_item_set_active (GTK_CHECK_MENU_ITEM (menuitem), messageview->textview->show_all_headers);

  if (messageview->msginfo && FOLDER_ITEM_IS_SENT_FOLDER (messageview->msginfo->folder))
...
static void
messageview_set_encoding_menu (MessageView * messageview)
{
  ...
  for (entry = msgview_entries; entry->callback != view_source_cb; entry++)
    {
      if (entry->callback == set_charset_cb && (CharSet) entry->callback_action == encoding)
        {
...
void
messageview_clear (MessageView * messageview)
{
...
void
messageview_destroy (MessageView * messageview)
{
  ...
  g_object_unref (imageview);
}

void
messageview_quote_color_set (void)
{
}

void
messageview_set_font (MessageView * messageview)
{
  textview_set_font (messageview->textview, NULL);
}

TextView *
messageview_get_current_textview (MessageView * messageview)
{
  TextView *text = NULL;
  ...
  if (messageview->type == MVIEW_TEXT)
    ...
    {
      if (gtk_notebook_get_current_page (GTK_NOTEBOOK (messageview->notebook)) == 0)
        text = messageview->textview;
...
MimeInfo *
messageview_get_selected_mime_part (MessageView * messageview)
{
  if (messageview->type == MVIEW_MIME)
...
void
messageview_copy_clipboard (MessageView * messageview)
{
  TextView *text;
...
void
messageview_select_all (MessageView * messageview)
{
  TextView *text;
...
void
messageview_set_position (MessageView * messageview, gint pos)
{
  textview_set_position (messageview->textview, pos);
}

gboolean
messageview_search_string (MessageView * messageview, const gchar * str, gboolean case_sens)
{
...
gboolean
messageview_search_string_backward (MessageView * messageview, const gchar * str, gboolean case_sens)
{
  return textview_search_string_backward (messageview->textview, str, case_sens);
  return FALSE;
}

gboolean
messageview_is_visible (MessageView * messageview)
{
  return messageview->visible;
}

void
messageview_save_as (MessageView * messageview)
{
  gchar *filename = NULL;
  MsgInfo *msginfo;
  gchar *src, *dest;
...
static gint
messageview_delete_cb (GtkWidget * widget, GdkEventAny * event, MessageView * messageview)
{
...
static void
messageview_size_allocate_cb (GtkWidget * widget, GtkAllocation * allocation)
{
...
static void
messageview_toggle_attach_cb (GtkToggleButton * toggle, MessageView * messageview)
{
...
static void
messageview_switch_page_cb (GtkNotebook * notebook, GtkWidget * page, guint page_num, MessageView * messageview)
{
...
static gint
messageview_menu_tool_btn_pressed (GtkWidget * widget, GdkEventButton * event, MessageView * messageview)
{
  if (!event)
    return FALSE;

  gtk_menu_popup_at_widget (GTK_MENU (messageview->tool_menu), widget, GDK_GRAVITY_SOUTH_WEST,
...
static void
messageview_open_file_cb (GtkWidget * widget, MessageView * messageview)
{
  MimeInfo *partinfo;
...
static void
messageview_save_all_cb (GtkWidget * widget, MessageView * messageview)
{
  mimeview_save_all (messageview->mimeview);
}

static gboolean
key_pressed (GtkWidget * widget, GdkEventKey * event, MessageView * messageview)
{
...
static void
save_as_cb (gpointer data, guint action, GtkWidget * widget)
{
...
static void
page_setup_cb (gpointer data, guint action, GtkWidget * widget)
{
  printing_page_setup_gtk ();
}

static void
print_cb (gpointer data, guint action, GtkWidget * widget)
{
  MessageView *messageview = (MessageView *) data;
  ...
  printing_print_message (messageview->msginfo, messageview->textview->show_all_headers);
}
...
static void
copy_cb (gpointer data, guint action, GtkWidget * widget)
{
  MessageView *messageview = (MessageView *) data;
...
static void
allsel_cb (gpointer data, guint action, GtkWidget * widget)
{
  MessageView *messageview = (MessageView *) data;
...
static void
search_cb (gpointer data, guint action, GtkWidget * widget)
{
  MessageView *messageview = (MessageView *) data;
...
static void
set_charset_cb (gpointer data, guint action, GtkWidget * widget)
{
  MessageView *messageview = (MessageView *) data;
  const gchar *charset;
...
static void
view_source_cb (gpointer data, guint action, GtkWidget * widget)
{
  MessageView *messageview = (MessageView *) data;
  SourceWindow *srcwin;
...
static void
show_all_header_cb (gpointer data, guint action, GtkWidget * widget)
{
  MessageView *messageview = (MessageView *) data;
  ...
  if (!msginfo)
    return;
  if (messageview->menu_locked)
    return;

  messageview->msginfo = NULL;
  messageview_show (messageview, msginfo, gtk_check_menu_item_get_active (GTK_CHECK_MENU_ITEM (widget)));
  procmsg_msginfo_free (msginfo);
}

static void
compose_cb (gpointer data, guint action, GtkWidget * widget)
{
  MessageView *messageview = (MessageView *) data;
...
static void
reply_cb (gpointer data, guint action, GtkWidget * widget)
{
  MessageView *messageview = (MessageView *) data;
  GSList *mlist = NULL;
  MsgInfo *msginfo;
  ...
  text = yam_text_view_get_selection (GTK_TEXT_VIEW (messageview->textview->text));
  if (text && *text == '\0')
    {
      g_free (text);
      text = NULL;
  ...
  if (!COMPOSE_QUOTE_MODE (mode))
    mode |= prefs_common.reply_with_quote ? COMPOSE_WITH_QUOTE : COMPOSE_WITHOUT_QUOTE;
...
static void
reedit_cb (gpointer data, guint action, GtkWidget * widget)
{
  MessageView *messageview = (MessageView *) data;
  MsgInfo *msginfo;
  ...
  if (!msginfo->folder)
    return;
  if (msginfo->folder->stype != F_OUTBOX && msginfo->folder->stype != F_DRAFT && msginfo->folder->stype != F_QUEUE)
    return;
...
static void
addressbook_open_cb (gpointer data, guint action, GtkWidget * widget)
{
  addressbook_open (NULL);
}

static void
add_address_cb (gpointer data, guint action, GtkWidget * widget)
{
  MessageView *messageview = (MessageView *) data;
  MsgInfo *msginfo;
  gchar *from;
...
static void
create_filter_cb (gpointer data, guint action, GtkWidget * widget)
{
  MessageView *messageview = (MessageView *) data;
  gchar *header = NULL;
  gchar *key = NULL;
  ...
  filter_get_keyword_from_msg (messageview->msginfo, &header, &key, (FilterCreateType) action);
...
static void
about_cb (gpointer data, guint action, GtkWidget * widget)
{
  about_show ();
}
