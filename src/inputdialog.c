...
static gchar *input_dialog_open (const gchar * title, const gchar * message, const gchar * default_string);
...
gchar *
input_dialog (const gchar * title, const gchar * message, const gchar * default_string)
{
  if (dialog)
    return NULL;
...
gchar *
input_dialog_with_invisible (const gchar * title, const gchar * message, const gchar * default_string)
{
  if (dialog)
    return NULL;
...
gchar *
input_dialog_combo (const gchar * title, const gchar * message,
                    const gchar * default_string, GList * list, gboolean case_sensitive)
{
  ...
  if (!list)
  ...
  else
    {
      ...
      for (ptr = list; ptr; ptr = ptr->next)
        gtk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (combo), (char *) ptr->data);
    }
...
gchar *
input_dialog_query_password (const gchar * server, const gchar * user)
{
  gchar *message;
  gchar *pass;
...
gchar *
input_dialog_with_filesel (const gchar * title, const gchar * message,
                           const gchar * default_string, GtkFileChooserAction action)
{
  if (dialog)
    return NULL;
...
static void
input_dialog_create (InputDialogType dialog_type)
{
  GtkWidget *vbox;
  GtkWidget *hbox;
  GtkWidget *sel_btn;
  ...
  g_signal_connect (G_OBJECT (dialog), "focus_out_event", G_CALLBACK (focus_out), NULL);
  ...
  if (dialog_type == INPUT_DIALOG_COMBO)
    {
      ...
      g_signal_connect (G_OBJECT (gtk_bin_get_child (GTK_BIN (combo))), "activated", G_CALLBACK (combo_activated), NULL);
    }
  else
    {
      entry = gtk_entry_new ();
      ...
      if (dialog_type == INPUT_DIALOG_FILESEL)
        {
          ...
          g_signal_connect (G_OBJECT (sel_btn), "clicked", G_CALLBACK (sel_btn_clicked), NULL);
        }
...
static gchar *
input_dialog_open (const gchar * title, const gchar * message, const gchar * default_string)
{
  gchar *str;
...
static void
input_dialog_set (const gchar * title, const gchar * message, const gchar * default_string)
{
  GtkWidget *entry_;
...
static void
combo_activated (GtkEditable * editable)
{
  ack = TRUE;
  fin = TRUE;
}
...
static void
sel_btn_clicked (GtkButton * button, gpointer data)
{
  gchar *file;
  gchar *utf8_file;
  ...
  if (chooser_action == GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER)
    ...
    file = filesel_select_file (_("Select file"), NULL, chooser_action);
  if (file)
    {
