...
static gchar *passphrase_mbox (const gchar * uid_hint, const gchar * pass_hint, gint prev_bad);

static GtkWidget *create_description (const gchar * uid_hint, const gchar * pass_hint, gint prev_bad);
...
passphrase_mbox (const gchar * uid_hint, const gchar * pass_hint, gint prev_bad)
{
  ...
  yam_stock_button_set_create (&confirm_box, &ok_button, "yam-ok", &cancel_button, "yam-cancel", NULL, NULL);
  ...
  if (grab_all)
    {
      ...
      if (gdk_pointer_grab (gtk_widget_get_window (window), TRUE, 0, gtk_widget_get_window (window), NULL, GDK_CURRENT_TIME))
        {
...
static int
free_passphrase (gpointer _unused)
{
  if (last_pass != NULL)
    {
#if HAVE_MLOCK
...
gpgme_error_t
gpgmegtk_passphrase_cb (void *opaque, const char *uid_hint, const char *passphrase_hint, int prev_bad, int fd)
{
  const char *pass;
      ...
      if (prefs_common.store_passphrase)
        {
          ...
          if (prefs_common.store_passphrase_timeout > 0)
            {
              g_timeout_add_full (G_PRIORITY_LOW, prefs_common.store_passphrase_timeout * 60 * 1000, free_passphrase,
                                  NULL, NULL);
            }
        }
