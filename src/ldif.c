...
*/
void
ldif_set_file (LdifFile * ldifFile, const gchar * value)
{
...
void
ldif_set_accessed (LdifFile * ldifFile, const gboolean value)
{
  g_return_if_fail (ldifFile != NULL);
...
*/
void
ldif_set_callback (LdifFile * ldifFile, void *func)
{
...
*/
static Ldif_FieldRec *
ldif_create_fieldrec (gchar * field)
{
...
*/
static void
ldif_free_fieldrec (Ldif_FieldRec * rec)
{
  if (rec)
    {
      g_free (rec->tagName);
...
*/
static gint
ldif_hash_free_vis (gpointer key, gpointer value, gpointer data)
{
...
*/
void
ldif_free (LdifFile * ldifFile)
{
  g_return_if_fail (ldifFile != NULL);

  /* Close file */
  if (ldifFile->file)
    fclose (ldifFile->file);
...
*/
void
ldif_print_fieldrec (Ldif_FieldRec * rec, FILE * stream)
{
...
*/
static void
ldif_print_file_vis (gpointer key, gpointer value, gpointer data)
{
...
*/
void
ldif_print_file (LdifFile * ldifFile, FILE * stream)
{
  g_return_if_fail (ldifFile != NULL);
...
*/
static gint
ldif_open_file (LdifFile * ldifFile)
{
  /* printf( "Opening file\n" ); */
  if (ldifFile->path)
    {
...
*/
static void
ldif_close_file (LdifFile * ldifFile)
{
  g_return_if_fail (ldifFile != NULL);
  if (ldifFile->file)
    fclose (ldifFile->file);
...
*/
static gchar *
ldif_get_line (LdifFile * ldifFile)
{
...
*/
static gchar *
ldif_get_tagname (char *line, gboolean * flag64)
{
  gint len = 0;
  gchar *tag = NULL;
  gchar *lptr = line;
...
*/
static void
ldif_dump_lists (GSList * listName, GSList * listAddr, GSList * listRem, GSList * listID, FILE * stream)
{
...
*/
static void
ldif_build_items (LdifFile * ldifFile, Ldif_ParsedRec * rec, AddressCache * cache)
{
  GSList *nodeFirst;
  GSList *nodeAddress;
  GSList *nodeAttr;
...
*/
static void
ldif_add_user_attr (Ldif_ParsedRec * rec, gchar * tagName, gchar * tagValue, GHashTable * hashField)
{
...
*/
static void
ldif_add_value (Ldif_ParsedRec * rec, gchar * tagName, gchar * tagValue, GHashTable * hashField)
{
  gchar *nm, *val;

  nm = g_strdup (tagName);
  g_ascii_strdown (nm, -1);
  if (tagValue)
  ...
  if (g_ascii_strcasecmp (nm, LDIF_TAG_COMMONNAME) == 0)
    ...
    rec->listLName = g_slist_append (rec->listLName, val);
           ...
           g_ascii_strcasecmp (nm, LDIF_TAG_NICKNAME2) == 0 || g_ascii_strcasecmp (nm, LDIF_TAG_XNICKNAME) == 0)
...
*/
static void
ldif_clear_rec (Ldif_ParsedRec * rec)
{
  GSList *list;
...
*/
static void
ldif_print_record (Ldif_ParsedRec * rec, FILE * stream)
{
  GSList *list;
...
*/
static void
ldif_read_file (LdifFile * ldifFile, AddressCache * cache)
{
...
*/
static void
ldif_hash_add_list (GHashTable * table, GSList * list)
{
  GSList *node = list;
...
*/
static int
ldif_field_compare (gconstpointer ptr1, gconstpointer ptr2)
{
...
*/
static void
ldif_hash2list_vis (gpointer key, gpointer value, gpointer data)
{
  LdifFile *ldf = data;
...
*/
static void
ldif_read_tag_list (LdifFile * ldifFile)
{
  gchar *tagName = NULL;
...
*/
gint
ldif_import_data (LdifFile * ldifFile, AddressCache * cache)
{
...
*/
gint
ldif_read_tags (LdifFile * ldifFile)
{
  g_return_val_if_fail (ldifFile != NULL, MGU_BAD_ARGS);
  ldifFile->retVal = MGU_SUCCESS;
...
*/
GList *
ldif_get_fieldlist (LdifFile * ldifFile)
{
  GList *list = NULL;
