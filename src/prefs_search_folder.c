...
static gint prefs_search_folder_delete_cb (GtkWidget * widget, GdkEventAny * event, PrefsSearchFolderDialog * dialog);
static gint prefs_search_folder_key_press_cb (GtkWidget * widget, GdkEventKey * event, PrefsSearchFolderDialog * dialog);

static void prefs_search_folder_select_folder (GtkWidget * widget, PrefsSearchFolderDialog * dialog);

static void prefs_search_folder_ok_cb (GtkWidget * widget, PrefsSearchFolderDialog * dialog);
static void prefs_search_folder_apply_cb (GtkWidget * widget, PrefsSearchFolderDialog * dialog);
static void prefs_search_folder_cancel_cb (GtkWidget * widget, PrefsSearchFolderDialog * dialog);

gboolean
prefs_search_folder_open (FolderItem * item)
{
...
static PrefsSearchFolderDialog *
prefs_search_folder_create (FolderItem * item)
{
  ...
  g_signal_connect (G_OBJECT (dialog->window), "delete_event", G_CALLBACK (prefs_search_folder_delete_cb), new_dialog);
  g_signal_connect (G_OBJECT (dialog->window), "key_press_event", G_CALLBACK (prefs_search_folder_key_press_cb), new_dialog);
  ...
  name_hbox = gtk_box_new (GTK_ORIENTATION_HORIZONTAL, 8);
  ...
  g_signal_connect (G_OBJECT (folder_btn), "clicked", G_CALLBACK (prefs_search_folder_select_folder), new_dialog);
  g_signal_connect (G_OBJECT (dialog->ok_btn), "clicked", G_CALLBACK (prefs_search_folder_ok_cb), new_dialog);
  g_signal_connect (G_OBJECT (dialog->apply_btn), "clicked", G_CALLBACK (prefs_search_folder_apply_cb), new_dialog);
  g_signal_connect (G_OBJECT (dialog->cancel_btn), "clicked", G_CALLBACK (prefs_search_folder_cancel_cb), new_dialog);
...
static void
prefs_search_folder_set_dialog (PrefsSearchFolderDialog * dialog)
{
  GSList *flist;
  FilterRule *rule;
  GSList *cur;
  ...
  gtk_combo_box_set_active (GTK_COMBO_BOX (dialog->bool_optmenu), rule->bool_op);

  gtk_entry_set_text (GTK_ENTRY (dialog->folder_entry), rule->target_folder);
  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (dialog->subfolder_checkbtn), rule->recursive);
  ...
  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (dialog->case_checkbtn), case_sens);
...
static void
prefs_search_folder_destroy (PrefsSearchFolderDialog * dialog)
{
...
static gint
prefs_search_folder_delete_cb (GtkWidget * widget, GdkEventAny * event, PrefsSearchFolderDialog * dialog)
{
...
static gint
prefs_search_folder_key_press_cb (GtkWidget * widget, GdkEventKey * event, PrefsSearchFolderDialog * dialog)
{
...
static void
prefs_search_folder_select_folder (GtkWidget * widget, PrefsSearchFolderDialog * dialog)
{
  FolderItem *item;
  gchar *id;
...
static void
prefs_search_folder_ok_cb (GtkWidget * widget, PrefsSearchFolderDialog * dialog)
{
...
static void
prefs_search_folder_apply_cb (GtkWidget * widget, PrefsSearchFolderDialog * dialog)
{
  const gchar *id;
  FolderItem *item;
  ...
  bool_op = gtk_combo_box_get_active (GTK_COMBO_BOX (dialog->bool_optmenu));
  recursive = gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (dialog->subfolder_checkbtn));
  case_sens = gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (dialog->case_checkbtn));

  cond_list = prefs_filter_edit_cond_edit_to_list (dialog->cond_edit, case_sens);
  if (!cond_list)
    return;
...
static void
prefs_search_folder_cancel_cb (GtkWidget * widget, PrefsSearchFolderDialog * dialog)
{
  dialog->finished = TRUE;
}
