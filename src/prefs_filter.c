...
static void prefs_filter_set_list_row (GtkTreeIter * iter, FilterRule * rule, gboolean move_view);
...
static void prefs_filter_enable_toggled (GtkCellRenderer * cell, gchar * path, gpointer data);
...
static void prefs_filter_row_reordered (GtkTreeModel * model,
                                        GtkTreePath * path, GtkTreeIter * iter, gpointer data, gpointer user_data);
static void prefs_filter_row_inserted (GtkTreeModel * model,
                                       GtkTreePath * path, GtkTreeIter * iter, gpointer user_data);

static gint prefs_filter_deleted (GtkWidget * widget, GdkEventAny * event, gpointer data);
static gboolean prefs_filter_key_pressed (GtkWidget * widget, GdkEventKey * event, gpointer data);
static void prefs_filter_close (void);


void
prefs_filter_open (MsgInfo * msginfo, const gchar * header, const gchar * key)
{
  inc_lock ();
  ...
  yam_plugin_signal_emit ("prefs-filter-open", rule_list_window.window);
...
prefs_filter_create (void)
{
  ...
  yam_stock_button_set_create (&confirm_area, &close_btn, "yam-close", NULL, NULL, NULL, NULL);
  gtk_box_pack_end (GTK_BOX (vbox), confirm_area, FALSE, FALSE, 0);
  gtk_widget_grab_default (close_btn);

  gtk_window_set_title (GTK_WINDOW (window), _("Filter settings"));
  g_signal_connect (G_OBJECT (window), "delete_event", G_CALLBACK (prefs_filter_deleted), NULL);
  g_signal_connect (G_OBJECT (window), "key_press_event", G_CALLBACK (prefs_filter_key_pressed), NULL);
  MANAGE_WINDOW_SIGNALS_CONNECT (window);
  g_signal_connect (G_OBJECT (close_btn), "clicked", G_CALLBACK (prefs_filter_close), NULL);
  ...
  store = gtk_list_store_new (N_COLS, G_TYPE_BOOLEAN, G_TYPE_STRING, G_TYPE_POINTER);
  ...
  gtk_tree_selection_set_select_function (selection, prefs_filter_select, NULL, NULL);

  renderer = gtk_cell_renderer_toggle_new ();
  g_signal_connect (renderer, "toggled", G_CALLBACK (prefs_filter_enable_toggled), NULL);
  column = gtk_tree_view_column_new_with_attributes (_("Enabled"), renderer, "active", COL_ENABLED, NULL);
  ...
  g_signal_connect (G_OBJECT (treeview), "row-activated", G_CALLBACK (prefs_filter_row_activated), NULL);
  g_signal_connect_after (G_OBJECT (store), "rows-reordered", G_CALLBACK (prefs_filter_row_reordered), NULL);
  g_signal_connect_after (G_OBJECT (store), "row-inserted", G_CALLBACK (prefs_filter_row_inserted), NULL);
  ...
  top_btn = yam_button_new ("yam-go-top");
  gtk_box_pack_start (GTK_BOX (btn_vbox), top_btn, FALSE, FALSE, 0);
  g_signal_connect (G_OBJECT (top_btn), "clicked", G_CALLBACK (prefs_filter_top), NULL);

  PACK_VSPACER (btn_vbox, spc_vbox, VSPACING_NARROW_2);

  up_btn = yam_button_new ("yam-go-up");
  gtk_box_pack_start (GTK_BOX (btn_vbox), up_btn, FALSE, FALSE, 0);
  g_signal_connect (G_OBJECT (up_btn), "clicked", G_CALLBACK (prefs_filter_up), NULL);

  down_btn = yam_button_new ("yam-go-down");
  gtk_box_pack_start (GTK_BOX (btn_vbox), down_btn, FALSE, FALSE, 0);
  g_signal_connect (G_OBJECT (down_btn), "clicked", G_CALLBACK (prefs_filter_down), NULL);

  PACK_VSPACER (btn_vbox, spc_vbox, VSPACING_NARROW_2);

  bottom_btn = yam_button_new ("yam-go-bottom");
  gtk_box_pack_start (GTK_BOX (btn_vbox), bottom_btn, FALSE, FALSE, 0);
  g_signal_connect (G_OBJECT (bottom_btn), "clicked", G_CALLBACK (prefs_filter_bottom), NULL);
  ...
  add_btn = yam_button_new ("yam-add");
  gtk_box_pack_start (GTK_BOX (btn_hbox), add_btn, FALSE, TRUE, 0);
  g_signal_connect (G_OBJECT (add_btn), "clicked", G_CALLBACK (prefs_filter_add_cb), NULL);

  edit_btn = yam_button_new ("yam-edit");
  gtk_box_pack_start (GTK_BOX (btn_hbox), edit_btn, FALSE, TRUE, 0);
  g_signal_connect (G_OBJECT (edit_btn), "clicked", G_CALLBACK (prefs_filter_edit_cb), NULL);

  copy_btn = yam_button_new ("yam-copy");
  gtk_box_pack_start (GTK_BOX (btn_hbox), copy_btn, FALSE, TRUE, 0);
  g_signal_connect (G_OBJECT (copy_btn), "clicked", G_CALLBACK (prefs_filter_copy_cb), NULL);

  del_btn = yam_button_new ("yam-delete");
  gtk_box_pack_start (GTK_BOX (btn_hbox), del_btn, FALSE, TRUE, 0);
  g_signal_connect (G_OBJECT (del_btn), "clicked", G_CALLBACK (prefs_filter_delete_cb), NULL);
...
static void
prefs_filter_set_list_row (GtkTreeIter * iter, FilterRule * rule, gboolean move_view)
{
  ...
  if (!iter)
    {
      ...
      gtk_list_store_set (store, &iter_, COL_ENABLED, rule->enabled, COL_NAME, rule_name, COL_FILTER_RULE, rule, -1);
    }
  else
    {
      ...
      iter_ = *iter;
      gtk_tree_model_get (GTK_TREE_MODEL (store), &iter_, COL_FILTER_RULE, &prev_rule, -1);
      if (!prev_rule)
        {
  ...
  if (move_view)
    {
      ...
      gtk_tree_view_scroll_to_cell (GTK_TREE_VIEW (rule_list_window.treeview), path, NULL, TRUE, 0.5, 0.0);
...
GSList *
prefs_filter_get_header_list (void)
{
  GSList *list = NULL;
  GSList *cur;
  GHashTable *table;
  ...
  APPEND_HDR_LIST (rule_list_window.default_hdr_list) APPEND_HDR_LIST (rule_list_window.user_hdr_list);
...
GSList *
prefs_filter_get_user_header_list (void)
{
  return rule_list_window.user_hdr_list;
}

gchar *
prefs_filter_get_msg_header_field (const gchar * header_name)
{
  if (!rule_list_window.msg_hdr_table)
    return NULL;

  return (gchar *) g_hash_table_lookup (rule_list_window.msg_hdr_table, header_name);
}

void
prefs_filter_set_user_header_list (GSList * list)
{
...
void
prefs_filter_set_msg_header_list (MsgInfo * msginfo)
{
  gchar *file;
  GSList *cur;
  GSList *next;
  ...
  rule_list_window.msg_hdr_list = procheader_get_header_list_from_file (file);

  g_free (file);

  rule_list_window.msg_hdr_table = g_hash_table_new (str_case_hash, str_case_equal);

  for (cur = rule_list_window.msg_hdr_list; cur != NULL; cur = next)
    {
      next = cur->next;
      header = (Header *) cur->data;
      if (!g_ascii_strcasecmp (header->name, "Received") ||
          !g_ascii_strcasecmp (header->name, "Mime-Version") || !g_ascii_strcasecmp (header->name, "X-UIDL"))
        {
          procheader_header_free (header);
          rule_list_window.msg_hdr_list = g_slist_remove (rule_list_window.msg_hdr_list, header);
          continue;
        }
      if (!g_hash_table_lookup (rule_list_window.msg_hdr_table, header->name))
        {
          g_hash_table_insert (rule_list_window.msg_hdr_table, header->name, header->body);
        }
    }
}

void
prefs_filter_set_header_list (MsgInfo * msginfo)
{
  GSList *list = NULL;
  gchar *path;
  FILE *fp;
  ...
  list = NULL;
  path = g_strconcat (get_rc_dir (), G_DIR_SEPARATOR_S, FILTER_HEADER_RC, NULL);
...
void
prefs_filter_write_user_header_list (void)
{
  gchar *path;
  PrefFile *pfile;
  GSList *cur;
  ...
  for (cur = rule_list_window.user_hdr_list; cur != NULL; cur = cur->next)
    {
  ...
  do
    {
      ...
      if (rule)
        prefs_common.fltlist = g_slist_append (prefs_common.fltlist, rule);
    }
  while (gtk_tree_model_iter_next (model, &iter));
}

static void
prefs_filter_add_cb (void)
{
  FilterRule *rule;
...
static void
prefs_filter_edit_cb (void)
{
  GtkTreeIter iter;
  FilterRule *rule, *new_rule;

  if (!gtk_tree_selection_get_selected (rule_list_window.selection, NULL, &iter))
    return;

  gtk_tree_model_get (GTK_TREE_MODEL (rule_list_window.store), &iter, COL_FILTER_RULE, &rule, -1);
  g_return_if_fail (rule != NULL);
...
prefs_filter_delete_cb (void)
{
  ...
  g_snprintf (buf, sizeof (buf), _("Do you really want to delete the rule '%s'?"), rule->name ? rule->name : _("(Untitled)"));
  if (alertpanel (_("Delete rule"), buf, "yam-yes", "yam-no", NULL) != G_ALERTDEFAULT)
    return;

  valid = gtk_list_store_remove (rule_list_window.store, &iter);
  if (valid)
    gtk_tree_selection_select_iter (rule_list_window.selection, &iter);
...
prefs_filter_down (void)
{
  ...
  next = iter;
  if (gtk_tree_model_iter_next (GTK_TREE_MODEL (rule_list_window.store), &next))
    gtk_list_store_swap (rule_list_window.store, &iter, &next);
}

static void
prefs_filter_bottom (void)
{
  GtkTreeIter iter;
...
static gboolean
prefs_filter_select (GtkTreeSelection * selection,
                     GtkTreeModel * model, GtkTreePath * path, gboolean cur_selected, gpointer data)
{
  return TRUE;
}
...
static void
prefs_filter_enable_toggled (GtkCellRenderer * cell, gchar * path_str, gpointer data)
{
  FilterRule *rule;
  GtkTreeIter iter;
  GtkTreePath *path;
  ...
  gtk_tree_model_get_iter (GTK_TREE_MODEL (rule_list_window.store), &iter, path);
  gtk_tree_path_free (path);
  gtk_tree_model_get (GTK_TREE_MODEL (rule_list_window.store), &iter, COL_FILTER_RULE, &rule, -1);

  rule->enabled ^= TRUE;

  gtk_list_store_set (rule_list_window.store, &iter, COL_ENABLED, rule->enabled, -1);
  rule_list_window.modified = TRUE;
}

static void
prefs_filter_row_activated (GtkTreeView * treeview, GtkTreePath * path, GtkTreeViewColumn * column, gpointer data)
{
  gtk_button_clicked (GTK_BUTTON (rule_list_window.edit_btn));
}

static void
prefs_filter_row_reordered (GtkTreeModel * model,
                            GtkTreePath * path, GtkTreeIter * iter, gpointer data, gpointer user_data)
{
  GtkTreeIter iter_;
  GtkTreePath *path_;

  if (!gtk_tree_selection_get_selected (rule_list_window.selection, NULL, &iter_))
    return;
  path_ = gtk_tree_model_get_path (GTK_TREE_MODEL (rule_list_window.store), &iter_);
  gtk_tree_view_scroll_to_cell (GTK_TREE_VIEW (rule_list_window.treeview), path_, NULL, FALSE, 0.0, 0.0);
  gtk_tree_path_free (path_);
  rule_list_window.modified = TRUE;
}

static void
prefs_filter_row_inserted (GtkTreeModel * model, GtkTreePath * path, GtkTreeIter * iter, gpointer user_data)
{
  rule_list_window.modified = TRUE;
}

static gint
prefs_filter_deleted (GtkWidget * widget, GdkEventAny * event, gpointer data)
{
  prefs_filter_close ();
  return TRUE;
}
...
static gboolean
prefs_filter_key_pressed (GtkWidget * widget, GdkEventKey * event, gpointer data)
{
