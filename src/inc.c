...
static GSList *inc_add_message_count (GSList * list, PrefsAccount * account, gint new_messages);
...
static gint inc_account_mail_real (MainWindow * mainwin, PrefsAccount * account, IncResult * result);
...
static void inc_update_folderview (IncProgressDialog * inc_dialog, IncSession * inc_session);

static void inc_progress_dialog_update_periodic (IncProgressDialog * inc_dialog, IncSession * inc_session);
static void inc_update_folderview_periodic (IncProgressDialog * inc_dialog, IncSession * inc_session);
 ...
 **/
static void
inc_finished (MainWindow * mainwin, IncResult * result)
{
  FolderItem *item;
  gint new_messages = 0;
  gint other_new = 0;
  ...
  if (new_messages > 0 && !block_notify)
    {
      ...
      str = g_string_new ("");
      g_string_printf (str, _("YAM: %d new messages"), new_messages);
      if (result)
        {
          for (cur = result->count_list; cur != NULL; cur = cur->next)
            {
              ...
              if (count->new_messages > 0)
                {
                  ...
                  if (!count->account)
                    ...
                    g_string_append_printf (str, "%s: %d",
                                            count->account->account_name ? count->account->account_name : "[?]",
                                            count->new_messages);
                }
            }
        }
      ...
      if (prefs_common.enable_newmsg_notify_window)
        {
          ...
          g_snprintf (buf, sizeof (buf), _("YAM: %d new messages"), new_messages);
  ...
  yam_plugin_signal_emit ("inc-mail-finished", new_messages);

  inc_block_notify (FALSE);

  if (new_messages <= 0 && !prefs_common.scan_all_after_inc)
    return;

  if (prefs_common.open_inbox_on_inc)
    {
      item = cur_account && cur_account->inbox
        ? folder_find_item_from_identifier (cur_account->inbox) : folder_get_default_inbox ();
  ...
  /* Notification */
  if (new_messages > 0 && prefs_common.enable_newmsg_notify && prefs_common.newmsg_notify_cmd)
    {
      gchar buf[1024];

      if (str_find_format_times (prefs_common.newmsg_notify_cmd, 'd') == 1)
        g_snprintf (buf, sizeof (buf), prefs_common.newmsg_notify_cmd, new_messages);
      else
        strncpy2 (buf, prefs_common.newmsg_notify_cmd, sizeof (buf));
      execute_command_line (buf, TRUE);
    }
}

static GSList *
inc_add_message_count (GSList * list, PrefsAccount * account, gint new_messages)
{
  IncAccountNewMsgCount *count;
...
static void
inc_result_free (IncResult * result, gboolean free_self)
{
  GSList *cur;
...
inc_mail (MainWindow * mainwin)
{
  ...
  yam_plugin_signal_emit ("inc-mail-start", cur_account);
...
static gint
inc_remote_account_mail (MainWindow * mainwin, PrefsAccount * account)
{
  ...
  if (account->protocol == A_IMAP4 && account->imap_filter_inbox_on_recv)
    {
      ...
      for (cur = mlist; cur != NULL; cur = cur->next)
        {
          ...
          if (prefs_common.enable_junk &&
              prefs_common.filter_junk_on_recv && prefs_common.filter_junk_before && junk_rule)
            {
          ...
          if (fltinfo->actions[FLT_ACTION_MOVE] && fltinfo->move_dest)
            {
              folder_item_move_msg (fltinfo->move_dest, msginfo);
              if (account->imap_check_inbox_only || fltinfo->move_dest->folder != inbox->folder)
                {
                  if (!is_junk &&
                      fltinfo->move_dest->stype != F_TRASH &&
                      fltinfo->move_dest->stype != F_JUNK &&
                      (MSG_IS_NEW (fltinfo->flags) || MSG_IS_UNREAD (fltinfo->flags)))
                    ++new_msgs;
                }
            }
          else if (fltinfo->actions[FLT_ACTION_DELETE])
            folder_item_remove_msg (inbox, msginfo);
          else if (!is_junk && (MSG_IS_NEW (msginfo->flags) || MSG_IS_UNREAD (msginfo->flags)))
            ++new_msgs;
      ...
      if (!prefs_common.scan_all_after_inc && item != NULL && inbox == item)
        update_summary = TRUE;
    }
  ...
  if (account->protocol == A_IMAP4 && account->imap_check_inbox_only)
  ...
  else
    {
      ...
      if (!prefs_common.scan_all_after_inc && item != NULL && FOLDER (account->folder) == item->folder)
        update_summary = TRUE;
    }
...
static gint
inc_account_mail_real (MainWindow * mainwin, PrefsAccount * account, IncResult * result)
{
...
gint
inc_account_mail (MainWindow * mainwin, PrefsAccount * account)
{
  IncResult result = { NULL, NULL };
  gint new_msgs;
  ...
  yam_plugin_signal_emit ("inc-mail-start", account);
...
void
inc_all_account_mail (MainWindow * mainwin, gboolean autocheck)
{
  ...
  yam_plugin_signal_emit ("inc-mail-start", NULL);
  ...
  for (list = account_get_list (); list != NULL; list = list->next)
    {
      ...
      if ((account->protocol == A_IMAP4 || account->protocol == A_NNTP) && account->recv_at_getall)
        {
...
gint
inc_pop_before_smtp (PrefsAccount * account)
{
  MainWindow *mainwin;
  IncProgressDialog *inc_dialog;
  IncSession *session;
  ...
  inc_dialog = inc_progress_dialog_create (FALSE);
  gtk_window_set_title (GTK_WINDOW (inc_dialog->dialog->window), _("Authenticating with POP3"));
  inc_dialog->queue_list = g_list_append (inc_dialog->queue_list, session);
  inc_dialog->mainwin = mainwin;
...
inc_progress_dialog_create (gboolean autocheck)
{
  ...
  cancel_all_btn = gtk_dialog_add_button (GTK_DIALOG (progress->window), _("Cancel _all"), GTK_RESPONSE_NONE);
  g_signal_connect (G_OBJECT (progress->cancel_btn), "clicked", G_CALLBACK (inc_cancel_cb), dialog);
  g_signal_connect (G_OBJECT (cancel_all_btn), "clicked", G_CALLBACK (inc_cancel_all_cb), dialog);
  g_signal_connect (G_OBJECT (progress->window), "delete_event", G_CALLBACK (inc_dialog_delete_cb), dialog);
  /* manage_window_set_transient(GTK_WINDOW(progress->window)); */

  progress_dialog_set_value (progress, 0.0);

  stock_pixbuf_gdk (STOCK_PIXMAP_COMPLETE, &ok_pixbuf);
  stock_pixbuf_gdk (STOCK_PIXMAP_CONTINUE, &current_pixbuf);
  stock_pixbuf_gdk (STOCK_PIXMAP_ERROR, &error_pixbuf);

  if (prefs_common.recv_dialog_mode == RECV_DIALOG_ALWAYS ||
      (prefs_common.recv_dialog_mode == RECV_DIALOG_MANUAL && !autocheck))
    {
  ...
  dialog->progress_tv = dialog->folder_tv = g_get_monotonic_time ();
...
static void
inc_progress_dialog_set_list (IncProgressDialog * inc_dialog)
{
  GList *list;
  ...
  for (list = inc_dialog->queue_list; list != NULL; list = list->next)
    {
      ...
      progress_dialog_append (inc_dialog->dialog, NULL, pop3_session->ac_prefs->account_name, _("Standby"), "", NULL);
    }
}

static void
inc_progress_dialog_clear (IncProgressDialog * inc_dialog)
{
...
static void
inc_progress_dialog_destroy (IncProgressDialog * inc_dialog)
{
...
static IncSession *
inc_session_new (PrefsAccount * account)
{
  IncSession *session;
  FilterRule *rule;
...
static void
inc_session_destroy (IncSession * session)
{
...
static void
inc_update_folder_foreach (GHashTable * table)
{
...
static gint
inc_start (IncProgressDialog * inc_dialog)
{
  IncSession *session;
  GList *qlist;
      ...
      if (!pop3_session->pass)
        {
          ...
          pass = input_query_password (pop3_session->ac_prefs->recv_server, pop3_session->user);
          ...
          if (pass)
            {
              pop3_session->ac_prefs->tmp_pass = g_strdup (pass);
  ...
  for (; inc_dialog->queue_list != NULL; inc_dialog->cur_row++)
    {
      ...
      if (session->inc_state == INC_CANCEL || pop3_session->pass == NULL)
        {
      ...
      switch (inc_state)
          ...
          if (pop3_session->cur_total_num > 0)
            msg = g_strdup_printf
              (_("%d message(s) (%s) received"),
               pop3_session->cur_total_num, to_human_readable (pop3_session->cur_total_recv_bytes));
          else
        ...
        case INC_LOOKUP_ERROR:
          SET_PIXMAP_AND_TEXT (error_pixbuf, _("Server not found"), NULL);
          break;
        case INC_CONNECT_ERROR:
          SET_PIXMAP_AND_TEXT (error_pixbuf, _("Connection failed"), NULL);
          break;
        case INC_AUTH_FAILED:
          SET_PIXMAP_AND_TEXT (error_pixbuf, _("Auth failed"), NULL);
          break;
        case INC_LOCKED:
      ...
      if (inc_dialog->result)
        inc_dialog->result->count_list =
          inc_add_message_count (inc_dialog->result->count_list, pop3_session->ac_prefs, session->new_msgs);
      new_msgs += session->new_msgs;

      if (!prefs_common.scan_all_after_inc)
        inc_update_folder_foreach (session->folder_table);

      if (pop3_session->error_val == PS_AUTHFAIL && pop3_session->ac_prefs->tmp_pass)
        {
      ...
      if (inc_state != INC_SUCCESS && inc_state != INC_CANCEL)
        {
          ...
          if (inc_dialog->show_dialog)
            manage_window_focus_in (inc_dialog->dialog->window, NULL, NULL);
          inc_put_error (session, inc_state, pop3_session->error_msg);
          if (inc_dialog->show_dialog)
            manage_window_focus_out (inc_dialog->dialog->window, NULL, NULL);
...
#undef SET_PIXMAP_AND_TEXT
  ...
  if (new_msgs > 0)
    fin_msg = g_strdup_printf (_("Finished (%d new message(s))"), new_msgs);
  else
  ...
  if (prefs_common.close_recv_dialog || !inc_dialog->show_dialog)
  ...
  else
    {
      ...
      gtk_button_set_label (GTK_BUTTON (inc_dialog->dialog->cancel_btn), _("_Close"));
    }
...
static IncState
inc_pop3_session_do (IncSession * session)
{
  Pop3Session *pop3_session = POP3_SESSION (session->session);
  ...
  if (pop3_session->auth_only)
    buf = g_strdup_printf (_("%s: Authenticating with POP3"), ac->recv_server);
  else
  ...
  buf = g_strdup_printf (_("Connecting to POP3 server: %s..."), ac->recv_server);
  log_message ("%s\n", buf);
  ...
  if (ac->use_socks && ac->use_socks_for_recv)
    {
      socks_info = socks_info_new (ac->socks_type, ac->proxy_host, ac->proxy_port, ac->use_proxy_auth ? ac->proxy_name : NULL,
                                   ac->use_proxy_auth ? ac->proxy_pass : NULL);
    }

  GTK_EVENTS_FLUSH ();

  if (session_connect_full (SESSION (pop3_session), SESSION (pop3_session)->server, SESSION (pop3_session)->port, socks_info) < 0)
    {
      log_warning (_("Can't connect to POP3 server: %s:%d\n"), SESSION (pop3_session)->server, SESSION (pop3_session)->port);
  ...
  while (session_is_connected (SESSION (pop3_session)) && session->inc_state != INC_CANCEL)
    gtk_main_iteration ();
  log_window_flush ();
...
static void
inc_progress_dialog_update (IncProgressDialog * inc_dialog, IncSession * inc_session)
{
...
static void
inc_progress_dialog_set_label (IncProgressDialog * inc_dialog, IncSession * inc_session)
{
  ...
  switch (session->state)
    ...
    case POP3_GETAUTH_APOP:
      ...
      statusbar_print_all (_("Retrieving messages from %s..."), SESSION (session)->server);
      break;
...
static void
inc_progress_dialog_set_progress (IncProgressDialog * inc_dialog, IncSession * inc_session)
{
  gchar buf[BUFFSIZE];
  ...
  if ((pop3_session->state == POP3_RETR ||
       pop3_session->state == POP3_RETR_RECV || pop3_session->state == POP3_DELETE) && total_num_to_recv > 0)
    {
      gchar total_size_str[16];

      to_human_readable_buf (total_size_str, sizeof (total_size_str), total);
      g_snprintf (buf, sizeof (buf),
                  _("Retrieving message (%d / %d) (%s / %s)"),
                  cur_num, total_num_to_recv, to_human_readable (cur_total), total_size_str);
      progress_dialog_set_label (inc_dialog->dialog, buf);
    }
  ...
  if (pop3_session->cur_total_num > 0)
    {
      g_snprintf (buf, sizeof (buf), _("%d message(s) (%s) received"),
                  pop3_session->cur_total_num, to_human_readable (pop3_session->cur_total_recv_bytes));
      progress_dialog_set_row_progress (inc_dialog->dialog, inc_dialog->cur_row, buf);
    }
}

static gboolean
hash_remove_func (gpointer key, gpointer value, gpointer data)
{
  return TRUE;
}

static void
inc_update_folderview (IncProgressDialog * inc_dialog, IncSession * inc_session)
{
  MainWindow *mainwin;

  if (g_hash_table_size (inc_session->tmp_folder_table) > 0)
    {
      folderview_update_item_foreach (inc_session->tmp_folder_table, FALSE);
      g_hash_table_foreach_remove (inc_session->tmp_folder_table, hash_remove_func, NULL);
    }
...
static void
inc_progress_dialog_update_periodic (IncProgressDialog * inc_dialog, IncSession * inc_session)
{
  gint64 tv_cur;
  gint msec;
  ...
  msec = (tv_cur - inc_dialog->progress_tv) * 1000;
...
static void
inc_update_folderview_periodic (IncProgressDialog * inc_dialog, IncSession * inc_session)
{
  guint64 tv_cur;
  gint msec;
  ...
  msec = (tv_cur - inc_dialog->folder_tv) * 1000;
  if (msec > FOLDER_UPDATE_INTERVAL)
    {
      inc_update_folderview (inc_dialog, inc_session);
      inc_dialog->folder_tv = tv_cur;
    }
}

static gint
inc_recv_data_progressive (Session * session, guint cur_len, guint total_len, gpointer data)
{
  ...
  if (pop3_session->state != POP3_RETR && pop3_session->state != POP3_RETR_RECV &&
      pop3_session->state != POP3_DELETE && pop3_session->state != POP3_LOGOUT)
    return 0;
...
static gint
inc_recv_data_finished (Session * session, guint len, gpointer data)
{
  IncSession *inc_session = (IncSession *) data;
  IncProgressDialog *inc_dialog;
...
static gint
inc_recv_message (Session * session, const gchar * msg, gpointer data)
{
  IncSession *inc_session = (IncSession *) data;
  IncProgressDialog *inc_dialog;
  Pop3Session *pop3_session = POP3_SESSION (session);
      ...
      if (inc_session->retr_count == 0)
        {
          ...
          g_print ("total_bytes_to_recv = %lld total_num_to_recv = %d\n",
                   pop3_session->total_bytes - inc_session->start_recv_bytes,
                   pop3_session->count - inc_session->start_num + 1);
          g_print ("pop: total_bytes = %lld cur_total_bytes = %lld\n", pop3_session->total_bytes,
                   pop3_session->cur_total_bytes);
 ...
 **/
static gint
inc_drop_message (Pop3Session * session, const gchar * file)
{
  FolderItem *inbox;
  GSList *cur;
  MsgInfo *msginfo;
  ...
  if (prefs_common.enable_junk &&
      prefs_common.filter_junk_on_recv && prefs_common.filter_junk_before && inc_session->junk_fltlist)
    {
      filter_apply_msginfo (inc_session->junk_fltlist, msginfo, fltinfo);
      if (fltinfo->drop_done)
        is_junk = TRUE;
      else if (fltinfo->error == FLT_ERROR_EXEC_FAILED || fltinfo->last_exec_exit_status >= 3)
        {
          g_warning ("inc_drop_message: junk filter command returned %d", fltinfo->last_exec_exit_status);
          alertpanel_error
  ...
  for (cur = fltinfo->dest_list; cur != NULL; cur = cur->next)
    {
      ...
      val = GPOINTER_TO_INT (g_hash_table_lookup (inc_session->folder_table, drop_folder));
      if (val == 0)
        g_hash_table_insert (inc_session->folder_table, drop_folder, GINT_TO_POINTER (1));
      g_hash_table_insert (inc_session->tmp_folder_table, drop_folder, GINT_TO_POINTER (1));

      if (drop_folder->stype != F_TRASH && drop_folder->stype != F_JUNK)
        is_counted = TRUE;
    }
  ...
  if (fltinfo->actions[FLT_ACTION_NOT_RECEIVE] == TRUE)
    ...
    {
      val = DROP_OK;
      if (!is_junk && is_counted && fltinfo->actions[FLT_ACTION_MARK_READ] == FALSE)
        {
          inc_session->new_msgs++;

          if (inc_dialog->result &&
              msginfo->subject && msginfo->fromname && g_slist_length (inc_dialog->result->msg_summaries) < 5)
            {
...
static void
inc_put_error (IncSession * session, IncState istate, const gchar * pop3_msg)
{
  gchar *log_msg = NULL;
  gchar *err_msg = NULL;
  ...
  switch (istate)
    ...
    case INC_LOOKUP_ERROR:
      ...
      if (prefs_common.no_recv_err_panel)
        break;
      err_msg = g_strdup_printf (_("Server %s not found."), session->session->server);
      break;
    case INC_CONNECT_ERROR:
...
static void
inc_cancel (IncProgressDialog * dialog, gboolean cancel_all)
{
  IncSession *session;
  GList *list;
...
void
inc_block_notify (gboolean block)
{
  if (!block)
    block_notify = FALSE;
...
static void
inc_cancel_cb (GtkWidget * widget, gpointer data)
{
  inc_cancel ((IncProgressDialog *) data, FALSE);
}

static void
inc_cancel_all_cb (GtkWidget * widget, gpointer data)
{
  inc_cancel ((IncProgressDialog *) data, TRUE);
}

static gint
inc_dialog_delete_cb (GtkWidget * widget, GdkEventAny * event, gpointer data)
{
...
inc_spool (void)
{
  ...
  spool_path = prefs_common.spool_path ? prefs_common.spool_path : DEFAULT_SPOOL_PATH;
  if (is_file_exist (spool_path))
    mbox = g_strdup (spool_path);
  else if (is_dir_exist (spool_path))
    mbox = g_strconcat (spool_path, G_DIR_SEPARATOR_S, g_get_user_name (), NULL);
  else
    {
...
static gint
get_spool (FolderItem * dest, const gchar * mbox)
{
  gint msgs, size;
  gint lockfd;
  ...
  msgs = proc_mbox_full (dest, tmp_mbox, folder_table,
                         prefs_common.filter_on_inc, prefs_common.enable_junk && prefs_common.filter_junk_on_recv);
...
void
inc_autocheck_timer_init (MainWindow * mainwin)
{
...
static void
inc_autocheck_timer_set_interval (guint interval)
{
  inc_autocheck_timer_remove ();

  if (prefs_common.autochk_newmail && autocheck_data)
    {
      autocheck_timer = g_timeout_add_full (G_PRIORITY_LOW, interval, inc_autocheck_func, autocheck_data, NULL);
...
void
inc_autocheck_timer_remove (void)
{
  if (autocheck_timer)
    {
