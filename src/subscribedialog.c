...
static gboolean subscribe_recv_func (SockInfo * sock, gint count, gint read_bytes, gpointer data);
...
static void subscribe_toggled (GtkCellRenderer * cell, gchar * path, gpointer data);

static void entry_activated (GtkEditable * editable);
static void search_clicked (GtkWidget * widget, gpointer data);

GSList *
subscribe_dialog (Folder * folder)
{
  GNode *node;
  FolderItem *item;
  ...
  for (node = folder->node->children; node != NULL; node = node->next)
    {
      ...
      subscribe_list = g_slist_append (subscribe_list, g_strdup (item->path));
    }
      ...
      for (cur = group_list; cur != NULL; cur = cur->next)
        {
          ...
          if (ginfo->subscribed)
            subscribe_list = g_slist_append (subscribe_list, g_strdup (ginfo->name));
        }
    }
...
subscribe_dialog_create (void)
{
  ...
  gtk_widget_set_size_request (dialog, SUBSCRIBE_DIALOG_WIDTH, SUBSCRIBE_DIALOG_HEIGHT);
  ...
  tree_store = gtk_tree_store_new (N_SUBSCRIBE_COLUMNS, G_TYPE_BOOLEAN,
                                   G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_POINTER, G_TYPE_BOOLEAN);
  gtk_tree_sortable_set_sort_column_id (GTK_TREE_SORTABLE (tree_store), SUBSCRIBE_NAME, GTK_SORT_ASCENDING);

  treeview = gtk_tree_view_new_with_model (GTK_TREE_MODEL (tree_store));
  g_object_unref (G_OBJECT (tree_store));
  gtk_tree_view_set_search_column (GTK_TREE_VIEW (treeview), SUBSCRIBE_NAME);

  selection = gtk_tree_view_get_selection (GTK_TREE_VIEW (treeview));
  gtk_tree_selection_set_mode (selection, GTK_SELECTION_MULTIPLE);
  gtk_tree_selection_set_select_function (selection, subscribe_selected, NULL, NULL);

  gtk_container_add (GTK_CONTAINER (scrolledwin), treeview);

  renderer = gtk_cell_renderer_toggle_new ();
  column = gtk_tree_view_column_new_with_attributes (NULL, renderer, "active", SUBSCRIBE_TOGGLE,
                                                     "activatable", SUBSCRIBE_CAN_TOGGLE,
                                                     "visible", SUBSCRIBE_CAN_TOGGLE, NULL);
  gtk_tree_view_column_set_min_width (column, 20);
  gtk_tree_view_append_column (GTK_TREE_VIEW (treeview), column);
  g_signal_connect (renderer, "toggled", G_CALLBACK (subscribe_toggled), NULL);

  renderer = gtk_cell_renderer_text_new ();
  column = gtk_tree_view_column_new_with_attributes (_("Newsgroup name"), renderer, "text", SUBSCRIBE_NAME, NULL);
  gtk_tree_view_append_column (GTK_TREE_VIEW (treeview), column);
  gtk_tree_view_set_expander_column (GTK_TREE_VIEW (treeview), column);

  renderer = gtk_cell_renderer_text_new ();
  column = gtk_tree_view_column_new_with_attributes (_("Messages"), renderer, "text", SUBSCRIBE_NUM, NULL);
  gtk_tree_view_append_column (GTK_TREE_VIEW (treeview), column);

  renderer = gtk_cell_renderer_text_new ();
  column = gtk_tree_view_column_new_with_attributes (_("Type"), renderer, "text", SUBSCRIBE_TYPE, NULL);
  gtk_tree_view_append_column (GTK_TREE_VIEW (treeview), column);
  ...
  yam_stock_button_set_create (&confirm_area, &ok_button, "yam-ok", &cancel_button, "yam-cancel", &refresh_button, "yam-refresh");
...
static void
subscribe_hash_init (void)
{
  branch_node_table = g_hash_table_new_full (g_str_hash, g_str_equal, NULL, (GDestroyNotify) gtk_tree_iter_free);
}

static void
subscribe_hash_free (void)
{
...
static gboolean
subscribe_hash_get_branch_node (const gchar * name, GtkTreeIter * iter)
{
  GtkTreeIter *iter_;
...
static void
subscribe_hash_set_branch_node (const gchar * name, GtkTreeIter * iter)
{
  GtkTreeIter *iter_;
...
static gchar *
subscribe_get_parent_name (const gchar * name)
{
  gchar *p;
...
static gboolean
subscribe_create_parent (const gchar * name, const gchar * pattern, GtkTreeIter * parent)
{
  GtkTreeIter parent_;
  GtkTreeIter iter;
  gchar *parent_name;
  ...
  if (has_parent)
    {
      ...
      if (g_pattern_match_simple (pattern, parent_name) == FALSE)
        yam_tree_view_expand_parent_all (GTK_TREE_VIEW (treeview), &iter);
    }
  else
...
static gboolean
subscribe_create_branch (NewsGroupInfo * ginfo, const gchar * pattern, GtkTreeIter * iter)
{
  GtkTreeIter iter_;
  GtkTreeIter parent;
      ...
      if (has_parent)
        {
          ...
          if (g_pattern_match_simple (pattern, parent_name) == FALSE)
            yam_tree_view_expand_parent_all (GTK_TREE_VIEW (treeview), &iter_);
        }
      else
        gtk_tree_store_append (tree_store, &iter_, NULL);
    }

  gtk_tree_store_set (tree_store, &iter_, SUBSCRIBE_NAME, name, SUBSCRIBE_NUM, count_str,
                      SUBSCRIBE_TYPE, type_str, SUBSCRIBE_INFO, ginfo, SUBSCRIBE_CAN_TOGGLE, TRUE, -1);
...
static void
subscribe_dialog_set_list (const gchar * pattern, gboolean refresh)
{
  gchar *pattern_;
  GSList *cur;
  GPatternSpec *pspec;
  ...
  if (refresh)
    {
      ...
      gtk_label_set_text (GTK_LABEL (status_label), _("Getting newsgroup list..."));
      GTK_EVENTS_FLUSH ();
  ...
  for (cur = group_list; cur != NULL; cur = cur->next)
    {
      ...
      if (g_slist_find_custom (subscribe_list, ginfo->name, (GCompareFunc) g_ascii_strcasecmp) != NULL)
      ...
      if (g_pattern_match_string (pspec, ginfo->name))
        {
          ...
          if (ginfo->subscribed)
            gtk_tree_store_set (tree_store, &iter, SUBSCRIBE_TOGGLE, TRUE, -1);
        }
    }
...
static gboolean
subscribe_recv_func (SockInfo * sock, gint count, gint read_bytes, gpointer data)
{
  gchar buf[BUFFSIZE];

  g_snprintf (buf, sizeof (buf), _("%d newsgroups received (%s read)"), count, to_human_readable (read_bytes));
...
static gint
window_deleted (GtkWidget * widget, GdkEventAny * event, gpointer data)
{
  ack = FALSE;
  if (gtk_main_level () > 1)
    gtk_main_quit ();
...
static gboolean
subscribe_selected (GtkTreeSelection * selection,
                    GtkTreeModel * model, GtkTreePath * path, gboolean cur_selected, gpointer data)
{
  GtkTreeIter iter;
  NewsGroupInfo *ginfo;
  ...
  gtk_tree_model_get (GTK_TREE_MODEL (tree_store), &iter, SUBSCRIBE_INFO, &ginfo, -1);
  if (!ginfo)
    return FALSE;
...
static void
subscribe_toggled (GtkCellRenderer * cell, gchar * path_str, gpointer data)
{
  GtkTreePath *path;
  GtkTreeIter iter;
  gboolean enabled;
  ...
  gtk_tree_model_get (GTK_TREE_MODEL (tree_store), &iter,
                      SUBSCRIBE_TOGGLE, &enabled, SUBSCRIBE_INFO, &ginfo, SUBSCRIBE_CAN_TOGGLE, &can_toggle, -1);
  if (ginfo && can_toggle)
    {
      ginfo->subscribed = !enabled;
      gtk_tree_store_set (tree_store, &iter, SUBSCRIBE_TOGGLE, !enabled, -1);
    }
}

static void
entry_activated (GtkEditable * editable)
{
  subscribe_search ();
}

static void
search_clicked (GtkWidget * widget, gpointer data)
{
  subscribe_search ();
}
