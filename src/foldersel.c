...
static gboolean foldersel_selected (GtkTreeSelection * selection, GtkTreeModel * model,
...
static gboolean foldersel_tree_key_pressed (GtkWidget * widget, GdkEventKey * event, gpointer data);
...
FolderItem *
foldersel_folder_sel (Folder * cur_folder, FolderSelectionType type, const gchar * default_folder)
{
  return foldersel_folder_sel_full (cur_folder, type, default_folder, NULL);
}

FolderItem *
foldersel_folder_sel_full (Folder * cur_folder, FolderSelectionType type, const gchar * default_folder, const gchar * message)
{
  selected_item = NULL;
  sel_type = type;
      ...
      if (fis.path)
        {
          ...
          yam_tree_view_expand_parent_all (GTK_TREE_VIEW (treeview), &fis.iter);
  ...
  if (type == FOLDER_SEL_ALL ||
      (selected_item->stype != F_VIRTUAL &&
       (sel_type == FOLDER_SEL_MOVE_FOLDER || (selected_item->path && !selected_item->no_select))))
    {
...
foldersel_create (void)
{
  ...
  gtk_widget_set_size_request (vbox, -1, 460);
  ...
  gtk_widget_set_size_request (scrolledwin, 320, -1);
  ...
  tree_store = gtk_tree_store_new (N_FOLDERSEL_COLUMNS, G_TYPE_STRING,
                                   G_TYPE_POINTER, GDK_TYPE_PIXBUF, GDK_TYPE_PIXBUF, GDK_TYPE_RGBA, G_TYPE_INT);
  gtk_tree_sortable_set_sort_func (GTK_TREE_SORTABLE (tree_store),
                                   FOLDERSEL_FOLDERNAME, foldersel_folder_name_compare, NULL, NULL);
  ...
  g_signal_connect (G_OBJECT (treeview), "key_press_event", G_CALLBACK (foldersel_tree_key_pressed), NULL);
  ...
  gtk_tree_view_column_set_attributes (column, renderer, "pixbuf", FOLDERSEL_PIXBUF,
                                       "pixbuf-expander-open", FOLDERSEL_PIXBUF_OPEN,
                                       "pixbuf-expander-closed", FOLDERSEL_PIXBUF, NULL);

  /* create text renderer */
  renderer = gtk_cell_renderer_text_new ();
  g_object_set (renderer, "ypad", 0, NULL);
  gtk_tree_view_column_pack_start (column, renderer, TRUE);
  gtk_tree_view_column_set_attributes (column, renderer, "text", FOLDERSEL_FOLDERNAME, "foreground-rgba",
                                       FOLDERSEL_FOREGROUND, "weight", FOLDERSEL_BOLD, NULL);
  ...
  g_signal_connect (G_OBJECT (entry), "activate", G_CALLBACK (foldersel_entry_activated), NULL);
  ...
  new_button = yam_button_new ("yam-new");
  gtk_widget_show (new_button);
  gtk_box_pack_start (GTK_BOX (confirm_area), new_button, FALSE, FALSE, 0);
  gtk_button_box_set_child_secondary (GTK_BUTTON_BOX (confirm_area), new_button, TRUE);
...
static void
foldersel_append_item (GtkTreeStore * store, FolderItem * item, GtkTreeIter * iter, GtkTreeIter * parent)
{
  gchar *name;
  gchar *sub = "";
  ...
  static GdkRGBA color_noselect = { 0.5, 0.5, 0.5, 1.0 };
  static GdkRGBA color_new = { 0.9, 0.2, 0.2, 1.0 };
  ...
  no_select = item->no_select || (sel_type != FOLDER_SEL_ALL && item->stype == F_VIRTUAL);
  ...
  gtk_tree_store_set (store, iter, FOLDERSEL_FOLDERNAME, name, FOLDERSEL_FOLDERITEM, item, FOLDERSEL_PIXBUF, pixbuf,
                      FOLDERSEL_PIXBUF_OPEN, pixbuf_open, FOLDERSEL_FOREGROUND, foreground, FOLDERSEL_BOLD, weight, -1);
  g_free (name);
}

static void
foldersel_insert_gnode_in_store (GtkTreeStore * store, GNode * node, GtkTreeIter * parent)
{
  FolderItem *item;
  GtkTreeIter child;
  GNode *iter;
  ...
  if (parent && item->parent && node->parent->children == node && !item->parent->collapsed)
    {
      GtkTreePath *path;
...
static void
foldersel_set_tree (Folder * cur_folder)
{
  Folder *folder;
  GList *list;
...
static gboolean
foldersel_selected (GtkTreeSelection * selection,
                    GtkTreeModel * model, GtkTreePath * path, gboolean currently_selected, gpointer data)
{
  GtkTreeIter iter;
...
static void
foldersel_ok (GtkButton * button, gpointer data)
{
  finished = TRUE;
}

static void
foldersel_cancel (GtkButton * button, gpointer data)
{
  cancelled = TRUE;
  finished = TRUE;
}
...
static void
foldersel_new_folder (GtkButton * button, gpointer data)
{
  FolderItem *new_item;
  gchar *new_folder;
  gchar *disp_name;
  ...
  if ((p && FOLDER_TYPE (selected_item->folder) != F_IMAP) ||
      (p && FOLDER_TYPE (selected_item->folder) == F_IMAP && *(p + 1) != '\0'))
    {
  ...
  new_item = selected_item->folder->klass->create_folder (selected_item->folder, selected_item, new_folder);
  if (!new_item)
    {
...
static void
foldersel_entry_activated (GtkEntry * entry, gpointer data)
{
  gtk_button_clicked (GTK_BUTTON (ok_button));
}

static void
foldersel_tree_activated (GtkTreeView * treeview, GtkTreePath * path, GtkTreeViewColumn * column, gpointer data)
{
  gtk_button_clicked (GTK_BUTTON (ok_button));
}

static gboolean
foldersel_tree_key_pressed (GtkWidget * widget, GdkEventKey * event, gpointer data)
{
  GtkTreeSelection *selection;
  GtkTreeModel *model;
  GtkTreeIter iter;
  ...
  switch (event->keyval)
    ...
    case GDK_KEY_KP_Left:
      if ((event->state & (GDK_SHIFT_MASK | GDK_MOD1_MASK | GDK_CONTROL_MASK)) != 0)
        return FALSE;
      adj = gtk_scrolled_window_get_hadjustment (GTK_SCROLLED_WINDOW (scrolledwin));
...
static gint
foldersel_folder_name_compare (GtkTreeModel * model, GtkTreeIter * a, GtkTreeIter * b, gpointer context)
{
...
static gboolean
tree_view_folder_item_func (GtkTreeModel * model, GtkTreePath * path, GtkTreeIter * iter, FolderItemSearch * data)
{
  FolderItem *item = NULL;
