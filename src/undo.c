...
static gint undo_merge (GList * list, guint start_pos, guint end_pos, gint action, const gchar * text);
...
void
undo_destroy (UndoMain * undostruct)
{
...
static UndoInfo *
undo_object_new (gchar * text, gint start_pos, gint end_pos, UndoAction action, gfloat window_position)
{
  UndoInfo *undoinfo;
...
static void
undo_object_free (UndoInfo * undo)
{
  g_free (undo->text);
  g_free (undo);
}
 ...
 **/
static void
undo_free_list (GList ** list_pointer)
{
  UndoInfo *undo;
...
void
undo_set_change_state_func (UndoMain * undostruct, UndoChangeStateFunc func, gpointer data)
{
 ...
 **/
static void
undo_check_size (UndoMain * undostruct)
{
  UndoInfo *last_undo;
  guint length;
 ...
 **/
static gint
undo_merge (GList * list, guint start_pos, guint end_pos, gint action, const gchar * text)
{
  gchar *temp_string;
  UndoInfo *last_undo;
  ...
  if (action == UNDO_ACTION_DELETE)
    {
      ...
      if (last_undo->start_pos != end_pos && last_undo->start_pos != start_pos)
      ...
      else
        {
          ...
          if (text[0] != ' ' && text[0] != '\t' && (last_undo->text[0] == ' ' || last_undo->text[0] == '\t'))
            checkit = FALSE;
          else
            {
...
static void
undo_add (const gchar * text, gint start_pos, gint end_pos, UndoAction action, UndoMain * undostruct)
{
  UndoInfo *undoinfo;
  GtkAdjustment *vadj;
  ...
  vadj = gtk_scrollable_get_vadjustment (GTK_SCROLLABLE (undostruct->textview));
...
undo_undo (UndoMain * undostruct)
{
  ...
  gtk_adjustment_set_value (gtk_scrollable_get_vadjustment (GTK_SCROLLABLE (textview)), undoinfo->window_position);
  ...
  switch (undoinfo->action)
    ...
    case UNDO_ACTION_INSERT:
      ...
      undo_debug ("undo: UNDO_ACTION_INSERT: %d: delete %d chars\n",
                  undoinfo->start_pos, undoinfo->end_pos - undoinfo->start_pos);
      break;
...
undo_redo (UndoMain * undostruct)
{
  ...
  gtk_adjustment_set_value (gtk_scrollable_get_vadjustment (GTK_SCROLLABLE (textview)), redoinfo->window_position);
  ...
  switch (redoinfo->action)
    ...
    case UNDO_ACTION_DELETE:
      ...
      undo_debug ("redo: UNDO_ACTION_INSERT: %d: delete %d chars\n",
                  redoinfo->start_pos, redoinfo->end_pos - redoinfo->start_pos);
      break;
    case UNDO_ACTION_REPLACE_DELETE:
      gtk_text_buffer_get_iter_at_offset (buffer, &start_iter, redoinfo->start_pos);
      gtk_text_buffer_get_iter_at_offset (buffer, &end_iter, redoinfo->end_pos);
...
void
undo_block (UndoMain * undostruct)
{
  GtkTextBuffer *buffer;
...
void
undo_unblock (UndoMain * undostruct)
{
  GtkTextBuffer *buffer;
...
void
undo_insert_text_cb (GtkTextBuffer * textbuf, GtkTextIter * iter,
                     gchar * new_text, gint new_text_length, UndoMain * undostruct)
{
  ...
  undo_add (text_to_insert, pos, pos + g_utf8_strlen (text_to_insert, -1), UNDO_ACTION_INSERT, undostruct);
  g_free (text_to_insert);
}

void
undo_delete_text_cb (GtkTextBuffer * textbuf, GtkTextIter * start, GtkTextIter * end, UndoMain * undostruct)
{
...
void
undo_paste_clipboard_cb (GtkTextView * textview, UndoMain * undostruct)
{
 ...
 **/
static gint
undo_get_selection (GtkTextView * textview, guint * start, guint * end)
{
  GtkTextBuffer *buffer;
