...
static void
do_destroy (GpgmegtkSigStatus hd)
{
  if (!hd->running)
    {
...
static void
okay_cb (GtkWidget * widget, gpointer data)
{
...
GpgmegtkSigStatus
gpgmegtk_sig_status_create (void)
{
  GtkWidget *window;
  GtkWidget *vbox;
  GtkWidget *hbox;
  ...
  g_signal_connect (G_OBJECT (window), "delete_event", G_CALLBACK (delete_event), hd);
  g_signal_connect (G_OBJECT (window), "key_press_event", G_CALLBACK (key_pressed), hd);
  ...
  yam_stock_button_set_create (&okay_area, &okay_btn, "yam-close", NULL, NULL, NULL, NULL);
  gtk_box_pack_end (GTK_BOX (gtk_dialog_get_action_area (GTK_DIALOG (window))), okay_area, FALSE, FALSE, 0);
  gtk_container_set_border_width (GTK_CONTAINER (okay_area), 5);
  gtk_widget_grab_default (okay_btn);
  gtk_widget_grab_focus (okay_btn);
  g_signal_connect (G_OBJECT (okay_btn), "clicked", G_CALLBACK (okay_cb), hd);
...
void
gpgmegtk_sig_status_destroy (GpgmegtkSigStatus hd)
{
  if (hd)
    {
      ...
      if (hd->running && !hd->timeout_id_valid)
        {
          hd->timeout_id = g_timeout_add (MY_TIMEOUT, timeout_cb, hd);
...
void
gpgmegtk_sig_status_update (GpgmegtkSigStatus hd, gpgme_ctx_t ctx)
{
  ...
  while (sig)
    {
      ...
      tmp = g_strdup_printf (_("%s%s%s from \"%s\""), text ? text : "", text ? "\n" : "",
...
const gchar *
gpgmegtk_sig_status_to_string (gpgme_signature_t signature, gboolean use_name)
{
  const gchar *result = "?";
  ...
  switch (gpg_err_code (signature->status))
    ...
    case GPG_ERR_NO_ERROR:
      switch (signature->validity)
        ...
        case GPGME_VALIDITY_MARGINAL:
          result = use_name ? _("Good signature from \"%s\"") : _("Good signature");
          break;
        default:
          result = use_name ? _("Valid signature but the key for \"%s\" is not trusted") : _("Valid signature (untrusted key)");
          break;
        }
      break;
    case GPG_ERR_SIG_EXPIRED:
      result = use_name ? _("Signature valid but expired for \"%s\"") : _("Signature valid but expired");
      break;
    ...
    case GPG_ERR_BAD_SIGNATURE:
      result = use_name ? _("BAD signature from \"%s\"") : _("BAD signature");
      break;
