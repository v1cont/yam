...
#if 0
static void
dump_mimeinfo (const gchar * text, MimeInfo * x)
{
  ...
  debug_print ("      cont_disp=`%s' fname=`%s' fpos=%ld size=%u, lvl=%d\n",
               x->content_disposition, x->filename, x->fpos, x->size, x->level);
...
static void
dump_part (MimeInfo * mimeinfo, FILE * fp)
{
...
gboolean
rfc2015_is_available (void)
{
  return gpg_available;
}

void
rfc2015_secure_remove (const gchar * fname)
{
  if (!fname)
    return;
  ...
  if (err || key == NULL || key->uids->uid == NULL)
    {
      ...
      if ((sig->fpr != NULL) && (*(sig->fpr) != '\0'))
        g_string_append_printf (str, "Key fingerprint: %s\n", sig->fpr);
      g_string_append (str, _("Cannot find user ID for this key."));
      g_string_append (str, "\n");
      return;
    }
  user = key->uids;
  g_string_append_printf (str, gpgmegtk_sig_status_to_string (sig, TRUE), user->uid);
  g_string_append (str, "\n");

  user = user->next;
  while (user)
    {
      g_string_append_printf (str, _("		aka \"%s\"\n"), user->uid);
      user = user->next;
    }
}
      ...
      if (sig->timestamp != 0)
        {
          ...
          my_strftime (ctime_str, sizeof (ctime_str), "%c", ctime_val);
          ctime_str_utf8 = g_locale_to_utf8 (ctime_str, -1, NULL, NULL, NULL);
          if (!ctime_str_utf8)
            ctime_str_utf8 = g_strdup (ctime_str);
          g_string_append_printf (str, _("Signature made at %s\n"), ctime_str_utf8);
...
static void
check_signature (MimeInfo * mimeinfo, MimeInfo * partinfo, FILE * fp)
{
  ...
  if (mimeinfo->children->size > 0)
    {
      if (fseek (fp, mimeinfo->children->fpos + mimeinfo->children->size - 1, SEEK_SET) < 0)
        {
          perror ("fseek");
          goto leave;
        }
      if (fgetc (fp) == '\n')
        {
          ...
          if (mimeinfo->children->size > 1)
            {
              if (fseek (fp, mimeinfo->children->fpos + mimeinfo->children->size - 2, SEEK_SET) < 0)
                {
                  perror ("fseek");
                  goto leave;
                }
  ...
  if (copy_file_part (fp, mimeinfo->children->fpos, mimeinfo->children->size - n_exclude_chars, tmp_file) < 0)
    {
      g_free (tmp_file);
      goto leave;
    }
  ...
  if (!err)
    err = gpgme_data_new_from_filepart (&sig, NULL, fp, partinfo->fpos, partinfo->size);
  if (err)
    {
...
copy_gpgmedata_to_temp (GpgmeData data, guint * length)
{
  ...
  tmp = g_strdup_printf ("%s%cgpgtmp.%08x", get_mime_tmp_dir (), G_DIR_SEPARATOR, ++id);
...
static gpgme_data_t
pgp_decrypt (MsgInfo * msginfo, MimeInfo * partinfo, FILE * fp)
{
  gpgme_ctx_t ctx = NULL;
  gpgme_error_t err;
  ...
  err = gpgme_data_new_from_filepart (&cipher, NULL, fp, partinfo->fpos, partinfo->size);
  if (err)
    {
      debug_print ("gpgme_data_new_from_filepart failed: %s\n", gpgme_strerror (err));
      goto leave;
  ...
  if (verifyresult && verifyresult->signatures)
    {
      ...
      msginfo->encinfo->sigstatus_full = sig_status_full (ctx, verifyresult);
      debug_print ("verification status: %s\n", result);
...
MimeInfo **
rfc2015_find_signature (MimeInfo * mimeinfo)
{
  MimeInfo *partinfo;
  ...
  if (!g_ascii_strcasecmp (mimeinfo->content_type, "multipart/mixed"))
    {
      for (partinfo = mimeinfo->children; partinfo != NULL; partinfo = partinfo->next)
        {
...
gboolean
rfc2015_has_signature (MimeInfo * mimeinfo)
{
  return rfc2015_find_signature (mimeinfo) != NULL;
}

void
rfc2015_check_signature (MimeInfo * mimeinfo, FILE * fp)
{
  MimeInfo **signedinfo;
...
gboolean
rfc2015_is_pgp_signature_part (MimeInfo * mimeinfo)
{
  if (!mimeinfo || !mimeinfo->content_type)
    return FALSE;

  return !g_ascii_strcasecmp (mimeinfo->content_type, "application/pgp-signature");
}

gboolean
rfc2015_is_pkcs7_signature_part (MimeInfo * mimeinfo)
{
  const gchar *c_type;
...
gboolean
rfc2015_is_signature_part (MimeInfo * mimeinfo)
{
  return (rfc2015_is_pgp_signature_part (mimeinfo) || rfc2015_is_pkcs7_signature_part (mimeinfo));
}

gint
rfc2015_is_encrypted (MimeInfo * mimeinfo)
{
...
gboolean
rfc2015_msg_is_encrypted (const gchar * file)
{
  FILE *fp;
  MimeInfo *mimeinfo;
  gint ret;
...
static gint
name_cmp (const gchar * a, const gchar * b)
{
  for (; *a && *b; a++, b++)
    {
      if (*a != *b && g_ascii_toupper (*(guchar *) a) != g_ascii_toupper (*(guchar *) b))
        return 1;
    }
...
void
rfc2015_decrypt_message (MsgInfo * msginfo, MimeInfo * mimeinfo, FILE * fp)
{
  static gint id;
  ...
  if (!g_ascii_strcasecmp (partinfo->content_type, "application/pgp-encrypted"))
    {
      /* Fixme: check that the version is 1 */
      ver_ok = 1;
    }
  partinfo = partinfo->next;
  if (ver_ok && !g_ascii_strcasecmp (partinfo->content_type, "application/octet-stream"))
    {
      if (partinfo->next)
  ...
  err = (gpgme_data_seek (plain, 0, SEEK_SET) == -1) ? gpgme_error_from_errno (errno) : 0;
  if (err)
...
FILE *
rfc2015_open_message_decrypted (MsgInfo * msginfo, MimeInfo ** mimeinfo)
{
  FILE *fp;
  MimeInfo *mimeinfo_;
  glong fpos;
  ...
  if (!MSG_IS_ENCRYPTED (msginfo->flags) && rfc2015_is_encrypted (mimeinfo_))
    {
      MSG_SET_TMP_FLAGS (msginfo->flags, MSG_ENCRYPTED);
    }

  if (MSG_IS_ENCRYPTED (msginfo->flags) &&
      (!msginfo->encinfo || (!msginfo->encinfo->plaintext_file && !msginfo->encinfo->decryption_failed)))
    {
      fpos = ftell (fp);
      rfc2015_decrypt_message (msginfo, mimeinfo_, fp);
      if (msginfo->encinfo && msginfo->encinfo->plaintext_file && !msginfo->encinfo->decryption_failed)
        {
          fclose (fp);
      ...
      if (!err)
        {
           ...
           */
          err = gpgme_op_encrypt (ctx, kset, GPGME_ENCRYPT_ALWAYS_TRUST, plain, cipher);
        }
    }
...
GSList *
rfc2015_create_signers_list (const gchar * keyid)
{
...
gint
rfc2015_encrypt (const gchar * file, GSList * recp_list)
{
  FILE *fp = NULL;
  gchar buf[BUFFSIZE];
  ...
  err = (gpgme_data_seek (header, 0, SEEK_SET) == -1) ? gpgme_error_from_errno (errno) : 0;
  if (err)
    {
      debug_print ("gpgme_data_seek failed: %s\n", gpgme_strerror (err));
      goto failure;
  ...
  err = (gpgme_data_seek (cipher, 0, SEEK_SET) == -1) ? gpgme_error_from_errno (errno) : 0;
  if (err)
    {
...
gint
rfc2015_encrypt_armored (const gchar * file, GSList * recp_list)
{
  FILE *fp = NULL;
  gchar buf[BUFFSIZE];
  gint i;
 ...
 */
static gpgme_data_t
pgp_sign (gpgme_data_t plain, GSList * key_list, gboolean clearsign, gchar ** micalg)
{
  GSList *p;
  gpgme_ctx_t ctx = NULL;
  gpgme_error_t err;
  ...
  *micalg = NULL;
  ...
  if (!err)
    {
      err = gpgme_op_sign (ctx, plain, sig, clearsign ? GPGME_SIG_MODE_CLEAR : GPGME_SIG_MODE_DETACH);
    }
  if (!err)
    {
      ...
      if (result && result->signatures)
        {
          if (gpgme_get_protocol (ctx) == GPGME_PROTOCOL_OpenPGP)
            {
              *micalg = g_strdup_printf ("PGP-%s", gpgme_hash_algo_name (result->signatures->hash_algo));
            }
          else
            {
 ...
 */
static gpgme_data_t
pgp_encrypt_sign (gpgme_data_t plain, gpgme_key_t kset[], GSList * key_list, gchar ** micalg)
{
  GSList *p;
  gpgme_ctx_t ctx = NULL;
  gpgme_error_t err;
 ...
 */
gint
rfc2015_sign (const gchar * file, GSList * key_list)
{
  FILE *fp = NULL;
  gchar buf[BUFFSIZE];
  gint i, clineidx, saved_last;
  ...
  if (!mime_version_seen)
    fputs ("MIME-Version: 1.0\r\n", fp);
  fprintf (fp, "Content-Type: multipart/signed; " "protocol=\"application/pgp-signature\";\r\n");
  if (micalg)
    fprintf (fp, " micalg=\"%s\";\r\n", micalg);
  ...
  err = (gpgme_data_seek (sigdata, 0, SEEK_SET) == -1) ? gpgme_error_from_errno (errno) : 0;
  if (err)
    {
 ...
 */
gint
rfc2015_encrypt_sign (const gchar * file, GSList * recp_list, GSList * key_list)
{
  FILE *fp = NULL;
  gchar buf[BUFFSIZE];
  gint i, clineidx, saved_last;
...
gint
rfc2015_encrypt_sign_armored (const gchar * file, GSList * recp_list, GSList * key_list)
{
  FILE *fp = NULL;
  gchar buf[BUFFSIZE];
  gint i;
