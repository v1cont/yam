...
static GdkPixbuf *folderpix = NULL;
static GdkPixbuf *folderopenpix = NULL;
static GdkPixbuf *grouppix = NULL;
static GdkPixbuf *interfacepix = NULL;
static GdkPixbuf *bookpix = NULL;
static GdkPixbuf *personpix = NULL;
static GdkPixbuf *addresspix = NULL;
static GdkPixbuf *vcardpix = NULL;
static GdkPixbuf *categorypix = NULL;
static GdkPixbuf *ldappix = NULL;
...
static void addressbook_tree_selection_changed (GtkTreeSelection * selection, gpointer data);
static void addressbook_list_selection_changed (GtkTreeSelection * selection, gpointer data);
...
static void addressbook_new_folder_cb (gpointer data, guint action, GtkWidget * widget);
static void addressbook_new_group_cb (gpointer data, guint action, GtkWidget * widget);
static void addressbook_treenode_edit_cb (gpointer data, guint action, GtkWidget * widget);
static void addressbook_treenode_delete_cb (gpointer data, guint action, GtkWidget * widget);
...
static void addressbook_delete_address_cb (gpointer data, guint action, GtkWidget * widget);
static void addressbook_copy_address_cb (gpointer data, guint action, GtkWidget * widget);
static void addressbook_paste_address_cb (gpointer data, guint action, GtkWidget * widget);
...
static gboolean addressbook_add_object (GtkTreeIter * iter, GtkTreeIter * new_iter, AddressObject * obj);
static AddressDataSource *addressbook_find_datasource (GtkTreeIter * iter);

static AddressBookFile *addressbook_get_book_file ();

static gboolean addressbook_node_add_folder (GtkTreeIter * iter, AddressDataSource * ds,
                                             ItemFolder * itemFolder, AddressObjectType otype, GtkTreeIter * new_iter);
static gboolean addressbook_node_add_group (GtkTreeIter * iter,
                                            AddressDataSource * ds, ItemGroup * itemGroup, GtkTreeIter * new_iter);
static void addressbook_tree_remove_children (GtkTreeModel * model, GtkTreeIter * parent);
...
static void size_allocated (GtkWidget * widget, GtkAllocation * allocation, gpointer data);

static gint addressbook_tree_compare (GtkTreeModel * model, GtkTreeIter * a, GtkTreeIter * b, gpointer data);
static gint addressbook_list_name_compare (GtkTreeModel * model, GtkTreeIter * a, GtkTreeIter * b, gpointer data);
static gint addressbook_list_address_compare (GtkTreeModel * model, GtkTreeIter * a, GtkTreeIter * b, gpointer data);
static gint addressbook_list_nickname_compare (GtkTreeModel * model, GtkTreeIter * a, GtkTreeIter * b, gpointer data);
static gint addressbook_list_remarks_compare (GtkTreeModel * model, GtkTreeIter * a, GtkTreeIter * b, gpointer data);

static gboolean addressbook_list_select_func (GtkTreeSelection * selection, GtkTreeModel * model,
                                              GtkTreePath * path, gboolean cur_selected, gpointer data);
...
static GtkItemFactoryEntry addressbook_entries[] = {
  ...
  {N_("/_Address/Add _to recipient"), "<control>M", addressbook_compose_to_cb, COMPOSE_ENTRY_TO, NULL},
  {N_("/_Address/Add to _Cc"), NULL, addressbook_compose_to_cb, COMPOSE_ENTRY_CC, NULL},
  {N_("/_Address/Add to _Bcc"), NULL, addressbook_compose_to_cb, COMPOSE_ENTRY_BCC, NULL},
  {N_("/_Address/---"), NULL, NULL, 0, "<Separator>"},
...
static GtkItemFactoryEntry addressbook_list_popup_entries[] = {
  ...
  {N_("/New _Folder"), NULL, addressbook_new_folder_cb, 0, NULL},
  {"/---", NULL, NULL, 0, "<Separator>"},
  {N_("/Add _to recipient"), NULL, addressbook_compose_to_cb, COMPOSE_ENTRY_TO, NULL},
  {N_("/Add t_o Cc"), NULL, addressbook_compose_to_cb, COMPOSE_ENTRY_CC, NULL},
  {N_("/Add to _Bcc"), NULL, addressbook_compose_to_cb, COMPOSE_ENTRY_BCC, NULL},
  {"/---", NULL, NULL, 0, "<Separator>"},
...
void
addressbook_open (Compose * target)
{
  if (!addrbook.window)
    {
...
void
addressbook_set_target_compose (Compose * target)
{
...
addressbook_create (void)
{
  ...
  gtk_window_set_default_size (GTK_WINDOW (window), prefs_common.addressbook_width, prefs_common.addressbook_height);
  yam_window_move (GTK_WINDOW (window), prefs_common.addressbook_x, prefs_common.addressbook_y);
  ...
  g_signal_connect (G_OBJECT (window), "size_allocate", G_CALLBACK (size_allocated), NULL);
  MANAGE_WINDOW_SIGNALS_CONNECT (window);

  vbox = gtk_box_new (GTK_ORIENTATION_VERTICAL, 0);
  gtk_container_add (GTK_CONTAINER (window), vbox);

  n_entries = sizeof (addressbook_entries) / sizeof (addressbook_entries[0]);
  menubar = menubar_create (window, addressbook_entries, n_entries, "<AddressBook>", NULL);
  gtk_box_pack_start (GTK_BOX (vbox), menubar, FALSE, TRUE, 0);
  menu_factory = gtk_item_factory_from_widget (menubar);

  spc_hbox = gtk_box_new (GTK_ORIENTATION_HORIZONTAL, 0);
  ...
  gtk_scrolled_window_set_policy (GTK_SCROLLED_WINDOW (tree_swin), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
  ...
  /* Address index */
  tree_store = gtk_tree_store_new (N_TREE_COLS, G_TYPE_STRING, G_TYPE_POINTER, GDK_TYPE_PIXBUF, GDK_TYPE_PIXBUF);
  gtk_tree_sortable_set_sort_func (GTK_TREE_SORTABLE (tree_store), COL_FOLDER_NAME, addressbook_tree_compare, NULL, NULL);
  ...
  gtk_tree_view_set_search_column (GTK_TREE_VIEW (treeview), COL_FOLDER_NAME);
  ...
  gtk_tree_view_column_set_attributes
    (column, renderer, "pixbuf", COL_PIXBUF,
     "pixbuf-expander-open", COL_PIXBUF_OPEN, "pixbuf-expander-closed", COL_PIXBUF, NULL);

  renderer = gtk_cell_renderer_text_new ();
  g_object_set (renderer, "ellipsize", PANGO_ELLIPSIZE_END, "ypad", 0, NULL);
  gtk_tree_view_column_pack_start (column, renderer, TRUE);
  gtk_tree_view_column_set_attributes (column, renderer, "text", COL_FOLDER_NAME, NULL);

  gtk_tree_view_append_column (GTK_TREE_VIEW (treeview), column);
  gtk_tree_view_set_expander_column (GTK_TREE_VIEW (treeview), column);

  gtk_tree_sortable_set_sort_column_id (GTK_TREE_SORTABLE (tree_store), COL_FOLDER_NAME, GTK_SORT_ASCENDING);

  g_signal_connect (G_OBJECT (tree_swin), "size-allocate", G_CALLBACK (addressbook_folder_resized), NULL);

  g_signal_connect (G_OBJECT (selection), "changed", G_CALLBACK (addressbook_tree_selection_changed), NULL);
  g_signal_connect (G_OBJECT (treeview), "button_press_event", G_CALLBACK (addressbook_tree_button_pressed), NULL);
  g_signal_connect (G_OBJECT (treeview), "button_release_event", G_CALLBACK (addressbook_tree_button_released), NULL);

  gtk_drag_dest_set (treeview, GTK_DEST_DEFAULT_ALL, addressbook_drag_types, N_DRAG_TYPES, GDK_ACTION_MOVE | GDK_ACTION_COPY);
  g_signal_connect (G_OBJECT (treeview), "drag-motion", G_CALLBACK (addressbook_drag_motion), NULL);
  g_signal_connect (G_OBJECT (treeview), "drag-leave", G_CALLBACK (addressbook_drag_leave), NULL);
  g_signal_connect (G_OBJECT (treeview), "drag-data-received", G_CALLBACK (addressbook_drag_received), NULL);

  list_vbox = gtk_box_new (GTK_ORIENTATION_VERTICAL, 4);

  list_swin = gtk_scrolled_window_new (NULL, NULL);
  gtk_scrolled_window_set_policy (GTK_SCROLLED_WINDOW (list_swin), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
  gtk_scrolled_window_set_shadow_type (GTK_SCROLLED_WINDOW (list_swin), GTK_SHADOW_IN);
  gtk_box_pack_start (GTK_BOX (list_vbox), list_swin, TRUE, TRUE, 0);

  /* Address list */
  list_store = gtk_tree_store_new (N_LIST_COLS, G_TYPE_STRING,
                                   G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_POINTER, GDK_TYPE_PIXBUF);
  gtk_tree_sortable_set_sort_func (GTK_TREE_SORTABLE (list_store), COL_NAME, addressbook_list_name_compare, NULL, NULL);
  gtk_tree_sortable_set_sort_func (GTK_TREE_SORTABLE (list_store), COL_ADDRESS, addressbook_list_address_compare, NULL, NULL);
  gtk_tree_sortable_set_sort_func (GTK_TREE_SORTABLE (list_store), COL_NICKNAME, addressbook_list_nickname_compare, NULL, NULL);
  gtk_tree_sortable_set_sort_func (GTK_TREE_SORTABLE (list_store), COL_REMARKS, addressbook_list_remarks_compare, NULL, NULL);

  listview = gtk_tree_view_new_with_model (GTK_TREE_MODEL (list_store));
  g_object_unref (G_OBJECT (list_store));
  selection = gtk_tree_view_get_selection (GTK_TREE_VIEW (listview));
  gtk_tree_selection_set_mode (selection, GTK_SELECTION_MULTIPLE);
  gtk_tree_selection_set_select_function (selection, addressbook_list_select_func, NULL, NULL);
  ...
  gtk_tree_view_column_set_spacing (column, 1);
  gtk_tree_view_column_set_sizing (column, GTK_TREE_VIEW_COLUMN_FIXED);
  gtk_tree_view_column_set_fixed_width (column, prefs_common.addressbook_col_name);
  gtk_tree_view_column_set_resizable (column, TRUE);
  ...
  g_object_set (renderer, "ypad", 0, NULL);
  gtk_tree_view_column_pack_start (column, renderer, FALSE);
  gtk_tree_view_column_set_attributes (column, renderer, "pixbuf", COL_L_PIXBUF, NULL);
  ...
  g_signal_connect (G_OBJECT (column), "notify::width", G_CALLBACK (addressbook_col_resized), GINT_TO_POINTER (COL_NAME));

  renderer = gtk_cell_renderer_text_new ();
  g_object_set (renderer, "ellipsize", PANGO_ELLIPSIZE_END, "ypad", 0, NULL);
  column = gtk_tree_view_column_new_with_attributes (_("E-Mail address"), renderer, "text", COL_ADDRESS, NULL);
  gtk_tree_view_column_set_sizing (column, GTK_TREE_VIEW_COLUMN_FIXED);
  gtk_tree_view_column_set_fixed_width (column, prefs_common.addressbook_col_addr);
  gtk_tree_view_column_set_resizable (column, TRUE);
  gtk_tree_view_column_set_sort_column_id (column, COL_ADDRESS);
  gtk_tree_view_append_column (GTK_TREE_VIEW (listview), column);
  g_signal_connect (G_OBJECT (column), "notify::width", G_CALLBACK (addressbook_col_resized), GINT_TO_POINTER (COL_ADDRESS));

  renderer = gtk_cell_renderer_text_new ();
  g_object_set (renderer, "ellipsize", PANGO_ELLIPSIZE_END, "ypad", 0, NULL);
  column = gtk_tree_view_column_new_with_attributes (_("Nick Name"), renderer, "text", COL_NICKNAME, NULL);
  gtk_tree_view_column_set_sizing (column, GTK_TREE_VIEW_COLUMN_FIXED);
  gtk_tree_view_column_set_fixed_width (column, prefs_common.addressbook_col_nickname);
  gtk_tree_view_column_set_resizable (column, TRUE);
  gtk_tree_view_column_set_sort_column_id (column, COL_NICKNAME);
  gtk_tree_view_append_column (GTK_TREE_VIEW (listview), column);
  g_signal_connect (G_OBJECT (column), "notify::width", G_CALLBACK (addressbook_col_resized), GINT_TO_POINTER (COL_NICKNAME));

  renderer = gtk_cell_renderer_text_new ();
  g_object_set (renderer,  "ellipsize", PANGO_ELLIPSIZE_END, "ypad", 0, NULL);
  column = gtk_tree_view_column_new_with_attributes (_("Remarks"), renderer, "text", COL_REMARKS, NULL);
  gtk_tree_view_column_set_sizing (column, GTK_TREE_VIEW_COLUMN_FIXED);
  gtk_tree_view_column_set_resizable (column, TRUE);
  gtk_tree_view_column_set_sort_column_id (column, COL_REMARKS);
  gtk_tree_view_append_column (GTK_TREE_VIEW (listview), column);
  g_signal_connect (G_OBJECT (column), "notify::width", G_CALLBACK (addressbook_col_resized), GINT_TO_POINTER (COL_REMARKS));

  gtk_tree_sortable_set_sort_column_id (GTK_TREE_SORTABLE (list_store), COL_NAME, GTK_SORT_ASCENDING);

  g_signal_connect (G_OBJECT (selection), "changed", G_CALLBACK (addressbook_list_selection_changed), NULL);
  g_signal_connect (G_OBJECT (listview), "button_press_event", G_CALLBACK (addressbook_list_button_pressed), NULL);
  g_signal_connect (G_OBJECT (listview), "button_release_event", G_CALLBACK (addressbook_list_button_released), NULL);
  g_signal_connect (G_OBJECT (listview), "row_expanded", G_CALLBACK (addressbook_person_expand_node), NULL);
  g_signal_connect (G_OBJECT (listview), "row_collapsed", G_CALLBACK (addressbook_person_collapse_node), NULL);

  gtk_tree_view_enable_model_drag_source
    (GTK_TREE_VIEW (listview), GDK_BUTTON1_MASK,
     addressbook_drag_types, N_DRAG_TYPES, GDK_ACTION_MOVE | GDK_ACTION_COPY);

  g_signal_connect_after (G_OBJECT (listview), "drag-begin", G_CALLBACK (addressbook_drag_begin), NULL);
  g_signal_connect_after (G_OBJECT (listview), "drag-end", G_CALLBACK (addressbook_drag_end), NULL);
  g_signal_connect (G_OBJECT (listview), "drag-data-get", G_CALLBACK (addressbook_drag_data_get), NULL);
...
#if 0
  g_signal_connect (G_OBJECT (entry), "changed", G_CALLBACK (addressbook_entry_changed), NULL);
#endif
  ...
  /* Button panel */
  hbbox1 = gtk_button_box_new (GTK_ORIENTATION_HORIZONTAL);
  gtk_button_box_set_layout (GTK_BUTTON_BOX (hbbox1), GTK_BUTTONBOX_END);
  gtk_box_set_homogeneous (GTK_BOX (hbbox1), TRUE);
  ...
  to_btn = gtk_button_new_with_label (prefs_common.trans_hdr ? _("To:") : "To:");
  gtk_widget_set_can_default (to_btn, TRUE);
  gtk_box_pack_start (GTK_BOX (hbbox1), to_btn, TRUE, TRUE, 0);
  cc_btn = gtk_button_new_with_label (prefs_common.trans_hdr ? _("Cc:") : "Cc:");
  gtk_widget_set_can_default (cc_btn, TRUE);
  gtk_box_pack_start (GTK_BOX (hbbox1), cc_btn, TRUE, TRUE, 0);
  bcc_btn = gtk_button_new_with_label (prefs_common.trans_hdr ? _("Bcc:") : "Bcc:");
  gtk_widget_set_can_default (bcc_btn, TRUE);
  gtk_box_pack_start (GTK_BOX (hbbox1), bcc_btn, TRUE, TRUE, 0);

  g_signal_connect (G_OBJECT (to_btn), "clicked", G_CALLBACK (addressbook_to_clicked), GINT_TO_POINTER (COMPOSE_ENTRY_TO));
  g_signal_connect (G_OBJECT (cc_btn), "clicked", G_CALLBACK (addressbook_to_clicked), GINT_TO_POINTER (COMPOSE_ENTRY_CC));
  g_signal_connect (G_OBJECT (bcc_btn), "clicked", G_CALLBACK (addressbook_to_clicked), GINT_TO_POINTER (COMPOSE_ENTRY_BCC));

  hbbox2 = gtk_button_box_new (GTK_ORIENTATION_HORIZONTAL);
  gtk_button_box_set_layout (GTK_BUTTON_BOX (hbbox2), GTK_BUTTONBOX_END);
  gtk_box_set_homogeneous (GTK_BOX (hbbox2), TRUE);
  ...
  gtk_button_box_set_child_secondary (GTK_BUTTON_BOX (hbbox2), close_btn, TRUE);
  ...
  stock_pixbuf_gdk (STOCK_PIXMAP_INTERFACE, &interfacepix);
      ...
      if (iface->useInterface)
        {
          ...
          gtk_tree_store_set (tree_store, &iter, COL_FOLDER_NAME, text,
                              COL_OBJ, adapter, COL_PIXBUF, interfacepix, COL_PIXBUF_OPEN, interfacepix, -1);
  ...
  /* Popup menu */
  n_entries = sizeof (addressbook_tree_popup_entries) / sizeof (addressbook_tree_popup_entries[0]);
  tree_popup = menu_create_items (addressbook_tree_popup_entries, n_entries, "<AddressBookTree>", &tree_factory, NULL);
  g_signal_connect (G_OBJECT (tree_popup), "selection_done", G_CALLBACK (addressbook_popup_close), NULL);
  n_entries = sizeof (addressbook_list_popup_entries) / sizeof (addressbook_list_popup_entries[0]);
  list_popup = menu_create_items (addressbook_list_popup_entries, n_entries, "<AddressBookList>", &list_factory, NULL);
...
{
  yam_widget_get_uposition (addrbook.window, &prefs_common.addressbook_x, &prefs_common.addressbook_y);
...
static void
addressbook_status_show (const gchar * msg)
{
...
static void
addressbook_ds_show_message (AddressDataSource * ds)
{
  gint retVal;
  gchar *name;
  ...
  if (ds)
    {
      ...
      if (ds->type == ADDR_IF_BOOK && name && !strcmp (name, ADDR_DS_AUTOREG))
      ...
      if (retVal == MGU_SUCCESS)
        ...
        g_snprintf (msgbuf, sizeof (msgbuf), "%s: %s", name, mgu_error2string (retVal));
    }
...
*/
static void
addressbook_del_clicked (GtkButton * button, gpointer data)
{
  ...
  yam_tree_row_reference_get_iter (model, addrbook.tree_opened, &iter);
  gtk_tree_model_get (model, &iter, COL_OBJ, &pobj, -1);
  g_return_if_fail (pobj != NULL);

  yam_tree_row_reference_get_iter (lmodel, addrbook.list_selected, &list_sel);
  ...
  /* Confirm deletion */
  aval = alertpanel (_("Delete address(es)"), _("Really delete the address(es)?"), "yam-yes", "yam-no", NULL);
...
static void
addressbook_reg_clicked (GtkButton * button, gpointer data)
{
  addressbook_new_address_cb (NULL, 0, NULL);
}

gchar *
addressbook_format_address (AddressObject * obj)
{
  gchar *buf = NULL;
  gchar *name = NULL;
  gchar *address = NULL;
  ...
  if (address)
    {
      if (!prefs_common.always_add_address_only && name && name[0] != '\0')
        {
...
static void
addressbook_to_clicked (GtkButton * button, gpointer data)
{
...
static void
addressbook_menuitem_set_sensitive (void)
{
  GtkTreeView *treeview = GTK_TREE_VIEW (addrbook.treeview);
  GtkTreeView *listview = GTK_TREE_VIEW (addrbook.listview);
  GtkTreeSelection *selection;
  ...
  if (addrbook.list_selected)
    {
      if (yam_tree_row_reference_get_iter (lmodel, addrbook.list_selected, &liter))
...
#endif
        canLookup = TRUE;
      if (ads->subType == ADDR_BOOK && pobj->name && !strcmp (pobj->name, ADDR_DS_AUTOREG))
        canEditTree = FALSE;
    }
  else if (pobj->type == ADDR_ITEM_FOLDER)
    {
...
static void
addressbook_tree_selection_changed (GtkTreeSelection * selection, gpointer data)
{
  GtkTreeModel *model;
  GtkTreeModel *lmodel;
  GtkTreeIter iter;
...
static void
addressbook_list_selection_changed (GtkTreeSelection * selection, gpointer data)
{
  GtkTreeModel *lmodel;
  GtkTreePath *path;
  GList *selected;
...
static void
addressbook_list_select_clear (void)
{
  if (_addressListSelection_)
...
static void
addressbook_list_select_remove (AddressObject * obj)
{
  if (!obj)
    return;
  if (_addressListSelection_)
...
static void
addressbook_list_select_set (GList * row_list)
{
  GtkTreeModel *model;
  GtkTreeIter iter;
  GList *cur;
  ...
  for (cur = row_list; cur != NULL; cur = cur->next)
    {
      ...
      if (obj && (obj->type == ADDR_ITEM_PERSON || obj->type == ADDR_ITEM_EMAIL || obj->type == ADDR_ITEM_GROUP))
        {
...
static void
addressbook_entry_activated (GtkWidget * widget, gpointer data)
{
  addressbook_lup_clicked (NULL, NULL);
}

static gboolean
addressbook_list_button_pressed (GtkWidget * widget, GdkEventButton * event, gpointer data)
{
  ...
  if (event->button == 1)
    ...
    {
      gtk_menu_popup_at_pointer (GTK_MENU (addrbook.list_popup), (GdkEvent *) event);
      gtk_tree_path_free (path);
      if (is_selected)
        return TRUE;
    }
...
static gboolean
addressbook_list_button_released (GtkWidget * widget, GdkEventButton * event, gpointer data)
{
...
static gboolean
addressbook_tree_button_pressed (GtkWidget * widget, GdkEventButton * event, gpointer data)
{
  ...
  if (event->button == 3)
    {
      ...
      gtk_menu_popup_at_pointer (GTK_MENU (addrbook.tree_popup), (GdkEvent *) event);
    }
...
static gboolean
addressbook_tree_button_released (GtkWidget * widget, GdkEventButton * event, gpointer data)
{
  return FALSE;
}

static gboolean
addressbook_obj_is_droppable (GtkTreeModel * model, AddressObject * obj)
{
  ...
  if (obj->type == ADDR_DATASOURCE)
    {
      ...
      if (ads && ads->subType == ADDR_BOOK && iface && !iface->readOnly)
        acceptable = TRUE;
    }
  ...
  if (acceptable && addrbook.tree_opened)
    {
      ...
      yam_tree_row_reference_get_iter (model, addrbook.tree_opened, &opened);
...
static gboolean
addressbook_drag_motion (GtkWidget * widget, GdkDragContext * context, gint x, gint y, guint time, gpointer data)
{
  GtkTreeModel *model;
  GtkTreePath *path = NULL;
  GtkTreeIter iter;
  ...
  if ((gdk_drag_context_get_actions (context) & (GDK_ACTION_MOVE | GDK_ACTION_COPY)) == 0)
    acceptable = FALSE;
  ...
  if (!acceptable)
    ...
    {
      GdkDragAction tmp_act, action = 0;

      tmp_act = gdk_drag_context_get_actions (context);

      if (!src_ds->iface || (src_ds->iface->readOnly || !src_ds->iface->haveLibrary))
        tmp_act &= ~GDK_ACTION_MOVE;

      if ((tmp_act & GDK_ACTION_MOVE) != 0)
        action = GDK_ACTION_MOVE;
      else if ((tmp_act & GDK_ACTION_COPY) != 0)
...
static void
addressbook_drag_leave (GtkWidget * widget, GdkDragContext * context, guint time, gpointer data)
{
  gtk_tree_view_set_drag_dest_row (GTK_TREE_VIEW (widget), NULL, GTK_TREE_VIEW_DROP_INTO_OR_AFTER);
}

static void
addressbook_drag_received (GtkWidget * widget, GdkDragContext * context, gint x, gint y,
                           GtkSelectionData * data, guint info, guint time, gpointer user_data)
{
  ...
  if ((gdk_drag_context_get_actions (context) & (GDK_ACTION_MOVE | GDK_ACTION_COPY)) == 0)
    acceptable = FALSE;
  is_move = (gdk_drag_context_get_actions (context) & GDK_ACTION_MOVE) != 0;
...
static void
addressbook_folder_resized (GtkWidget * widget, GtkAllocation * allocation, gpointer data)
{
...
static void
addressbook_popup_close (GtkMenuShell * menu_shell, gpointer data)
{
  GtkTreeView *treeview = GTK_TREE_VIEW (addrbook.treeview);
  GtkTreeSelection *selection;
  GtkTreePath *path;
...
static void
addressbook_new_folder_cb (gpointer data, guint action, GtkWidget * widget)
{
  GtkTreeView *treeview = GTK_TREE_VIEW (addrbook.treeview);
  GtkTreeSelection *selection;
  GtkTreeModel *model;
  ...
  if (folder)
    {
      ...
      gtk_tree_path_free (path);
      if (yam_tree_row_reference_equal (addrbook.tree_selected, addrbook.tree_opened))
        addressbook_set_list (obj);
    }
...
static void
addressbook_new_group_cb (gpointer data, guint action, GtkWidget * widget)
{
  GtkTreeView *treeview = GTK_TREE_VIEW (addrbook.treeview);
  GtkTreeSelection *selection;
  GtkTreeModel *model;
...
static void
addressbook_change_node_name (GtkTreeIter * iter, const gchar * name)
{
  GtkTreeView *treeview = GTK_TREE_VIEW (addrbook.treeview);
  GtkTreeStore *store;
...
*/
static gchar *
addressbook_edit_datasource (AddressObject * obj, GtkTreeIter * iter)
{
  gchar *newName = NULL;
  AddressDataSource *ds = NULL;
  AddressInterface *iface = NULL;
...
*/
static void
addressbook_treenode_edit_cb (gpointer data, guint action, GtkWidget * widget)
{
  GtkTreeView *treeview = GTK_TREE_VIEW (addrbook.treeview);
  GtkTreeSelection *selection;
  GtkTreeModel *model;
...
*/
static void
addressbook_treenode_delete_cb (gpointer data, guint action, GtkWidget * widget)
{
  GtkTreeView *treeview = GTK_TREE_VIEW (addrbook.treeview);
  GtkTreeSelection *selection;
  GtkTreeModel *model;
  ...
  if (obj->type == ADDR_ITEM_FOLDER)
    {
      message = g_strdup_printf
        (_("Do you want to delete the folder AND all addresses in `%s' ?\n"
           "If deleting the folder only, addresses will be moved into parent folder."), obj->name);
      aval = alertpanel (_("Delete folder"), message, _("_Folder only"), _("Folder and _addresses"), "yam-cancel");
      g_free (message);
  ...
  else
    {
      ...
      aval = alertpanel (_("Delete"), message, "yam-yes", "yam-no", NULL);
      g_free (message);
      if (aval != G_ALERTDEFAULT)
        return;
    }
...
static void
addressbook_new_address_cb (gpointer data, guint action, GtkWidget * widget)
{
  GtkTreeView *treeview = GTK_TREE_VIEW (addrbook.treeview);
  GtkTreeSelection *selection;
  GtkTreeModel *model;
...
static void
addressbook_compose_to_cb (gpointer data, guint action, GtkWidget * widget)
{
...
*/
static gboolean
addressbook_find_group_node (GtkTreeIter * parent, GtkTreeIter * iter, ItemGroup * group)
{
  GtkTreeModel *model;
  GtkTreeIter iter_;
  gboolean valid = TRUE;
...
static AddressBookFile *
addressbook_get_book_file (void)
{
  GtkTreeView *treeview = GTK_TREE_VIEW (addrbook.treeview);
  GtkTreeSelection *selection;
  GtkTreeIter iter;
...
static void
addressbook_tree_remove_children (GtkTreeModel * model, GtkTreeIter * parent)
{
  GtkTreeIter iter;
...
static void
addressbook_move_nodes_recursive (GtkTreeIter * iter, GtkTreeIter * parent)
{
  GtkTreeView *treeview = GTK_TREE_VIEW (addrbook.treeview);
  GtkTreeModel *model;
  ...
  while (valid)
    {
      ...
      gtk_tree_model_get (model, &child, COL_FOLDER_NAME, &name,
                          COL_OBJ, &obj, COL_PIXBUF, &pixbuf, COL_PIXBUF_OPEN, &pixbuf_open, -1);
      gtk_tree_store_append (GTK_TREE_STORE (model), &new_iter, parent);
      gtk_tree_store_set (GTK_TREE_STORE (model), &new_iter,
                          COL_FOLDER_NAME, name, COL_OBJ, obj, COL_PIXBUF, pixbuf, COL_PIXBUF_OPEN, pixbuf_open, -1);
      g_free (name);
...
static void
addressbook_move_nodes_up (GtkTreeIter * iter)
{
  GtkTreeView *treeview = GTK_TREE_VIEW (addrbook.treeview);
  GtkTreeModel *model;
  GtkTreeIter parent;
...
static void
addressbook_edit_address_cb (gpointer data, guint action, GtkWidget * widget)
{
  GtkTreeView *treeview = GTK_TREE_VIEW (addrbook.treeview);
  GtkTreeSelection *selection;
  GtkTreeModel *model;
  ...
  if (!gtk_tree_selection_get_selected (selection, &model, &iter))
    return;
  gtk_tree_model_get (model, &iter, COL_OBJ, &pobj, -1);
  node_found = yam_tree_model_find_by_column_data (model, &node, &iter, COL_OBJ, obj);
...
static void
addressbook_delete_address_cb (gpointer data, guint action, GtkWidget * widget)
{
  addressbook_del_clicked (NULL, NULL);
}

static void
addressbook_copy_address_cb (gpointer data, guint action, GtkWidget * widget)
{
  AddressObject *obj;
  GList *node;
...
static void
addressbook_paste_address_cb (gpointer data, guint action, GtkWidget * widget)
{
  GtkTreeView *treeview = GTK_TREE_VIEW (addrbook.treeview);
  GtkTreeSelection *selection;
  GtkTreeModel *model;
...
static void
close_cb (gpointer data, guint action, GtkWidget * widget)
{
  addressbook_close ();
}

static void
addressbook_file_save_cb (gpointer data, guint action, GtkWidget * widget)
{
  addressbook_export_to_file ();
}

static void
addressbook_person_expand_node (GtkTreeView * treeview, GtkTreeIter * iter, GtkTreePath * path, gpointer * data)
{
  GtkTreeModel *model;
  ItemPerson *person = NULL;
...
static void
addressbook_person_collapse_node (GtkTreeView * treeview, GtkTreeIter * iter, GtkTreePath * path, gpointer * data)
{
  GtkTreeModel *model;
  ItemPerson *person = NULL;
...
static void
addressbook_drag_begin (GtkWidget * widget, GdkDragContext * drag_context, gpointer data)
{
  list_on_drag = TRUE;
...
static void
addressbook_drag_end (GtkWidget * widget, GdkDragContext * drag_context, gpointer data)
{
}

static void
addressbook_drag_data_get (GtkWidget * widget, GdkDragContext * drag_context,
                           GtkSelectionData * selection_data, guint info, guint time, gpointer data)
{
  if (info == DRAG_TYPE_OBJ)
    gtk_selection_data_set (selection_data, gtk_selection_data_get_target (selection_data), 8, (guchar *) "drag-from-list", 14);
}

static gchar *
addressbook_format_item_list (ItemPerson * person, ItemEMail * email)
{
  gchar *str = NULL;
...
static gboolean
addressbook_match_item (const gchar *name, const gchar *email_alias, const gchar *addr, const gchar *remarks, const gchar *str)
{
  if (!name)
    return FALSE;
  if (!str || str[0] == '\0')
...
static void
addressbook_load_group (ItemGroup * itemGroup)
{
  GtkTreeView *listview = GTK_TREE_VIEW (addrbook.listview);
  GtkTreeModel *model;
  GtkTreeIter iter;
  ...
  for (; items != NULL; items = g_list_next (items))
    {
      ...
      if (!addressbook_match_item (ADDRITEM_NAME (person),
                                   ADDRITEM_NAME (email), email->address, email->remarks, search_str))
        continue;
      ...
      gtk_tree_store_set (GTK_TREE_STORE (model), &iter,
                          ...
                          COL_L_OBJ, email, COL_L_PIXBUF, atci->icon_pixbuf, -1);
      g_free (str);
    }
}

static void
addressbook_folder_load_person (ItemFolder * itemFolder)
{
  GtkTreeView *listview = GTK_TREE_VIEW (addrbook.listview);
  GtkTreeModel *model;
  GList *items;
  ...
  for (; items != NULL; items = g_list_next (items))
    {
      ...
      if (node && node->data)
        {
          email = node->data;
          if (!addressbook_match_item
              (ADDRITEM_NAME (person), ADDRITEM_NAME (email), email->address, email->remarks, search_str))
            continue;
        }
      else
        {
          ...
          if (flgFirst)
            {
              ...
              gtk_tree_store_append (GTK_TREE_STORE (model), &iperson, NULL);
              gtk_tree_store_set (GTK_TREE_STORE (model),
                                  ...
                                  COL_NICKNAME, person->nickName ? person->nickName : "",
                                  COL_REMARKS, email->remarks ? email->remarks : "",
                                  COL_L_OBJ, person, COL_L_PIXBUF, atci->icon_pixbuf, -1);
              g_free (str);
            }
          else
            {
              ...
              name = ADDRITEM_NAME (email);
              gtk_tree_store_append (GTK_TREE_STORE (model), &iemail, &iperson);
              gtk_tree_store_set (GTK_TREE_STORE (model),
                                  &iemail,
                                  COL_NAME, name,
                                  COL_ADDRESS, email->address ? email->address : "",
                                  COL_NICKNAME, "",
                                  COL_REMARKS, email->remarks ? email->remarks : "",
                                  COL_L_OBJ, email, COL_L_PIXBUF, atciMail->icon_pixbuf, -1);
            }
          flgFirst = FALSE;
          haveAddr = TRUE;
        }
      if (!haveAddr)
        {
          ...
          gtk_tree_store_set (GTK_TREE_STORE (model),
                              ...
                              COL_ADDRESS, "",
                              COL_NICKNAME, person->nickName ? person->nickName : "",
                              COL_REMARKS, "", COL_L_OBJ, person, COL_L_PIXBUF, atci->icon_pixbuf, -1);
        }
      if (person->isOpened)
        {
          GtkTreePath *path;
...
static void
addressbook_folder_load_group (ItemFolder * itemFolder)
{
  GtkTreeView *listview = GTK_TREE_VIEW (addrbook.listview);
  GtkTreeModel *model;
  GList *items;
  ...
  for (; items != NULL; items = g_list_next (items))
    {
      ...
      gtk_tree_store_set (GTK_TREE_STORE (model), &iter,
                          ...
                          COL_ADDRESS, "",
                          COL_NICKNAME, "", COL_REMARKS, "", COL_L_OBJ, group, COL_L_PIXBUF, atci->icon_pixbuf, -1);
    }
...
static AddressDataSource *
addressbook_find_datasource (GtkTreeIter * iter)
{
  GtkTreeModel *model;
...
*/
static void
addressbook_set_list (AddressObject * obj)
{
  GtkTreeView *listview = GTK_TREE_VIEW (addrbook.listview);
  GtkTreeModel *model;
  AddressDataSource *ds = NULL;
  ...
  gtk_tree_store_clear (GTK_TREE_STORE (model));
  sorted = gtk_tree_sortable_get_sort_column_id (GTK_TREE_SORTABLE (model), &address_list_sort_id, &order);
  if (!sorted)
    {
      address_list_sort_id = COL_NAME;
      order = GTK_SORT_ASCENDING;
    }
  yam_tree_sortable_unset_sort_column_id (GTK_TREE_SORTABLE (model));
  ...
  gtk_tree_sortable_set_sort_column_id (GTK_TREE_SORTABLE (model), address_list_sort_id, order);
}
...
*/
static void
addressbook_free_adapter (GtkTreeIter * iter)
{
  GtkTreeModel *model;
...
*/
static void
addressbook_free_child_adapters (GtkTreeIter * iter)
{
  GtkTreeModel *model;
  GtkTreeIter child;
  gboolean valid;
...
AdapterDSource *
addressbook_create_ds_adapter (AddressDataSource * ds, AddressObjectType otype, gchar * name)
{
...
void
addressbook_ads_set_name (AdapterDSource * adapter, gchar * value)
{
...
addressbook_load_tree (void)
{
  ...
  while (nodeIf)
    {
      adapter = nodeIf->data;
      yam_tree_row_reference_get_iter (model, adapter->tree_row, &iter);
      iface = adapter->iface;
      atci = adapter->atci;
      if (iface)
        {
 ...
 */
static gboolean
addressbook_convert (AddressIndex * addrIndex)
{
  ...
  if (errFlag)
    {
      debug_print ("Error\n%s\n", msg);
      alertpanel (_("Address book conversion error"), msg, "yam-close", NULL, NULL);
    }
  else if (msg)
    {
      debug_print ("Warning\n%s\n", msg);
      alertpanel (_("Address book conversion"), msg, "yam-close", NULL, NULL);
    }
  ...
  if (addrIndex->retVal == MGU_NO_FILE)
  ...
  else
    {
      ...
      g_snprintf (msg, sizeof (msg),
                  ...
                  addrIndex->filePath, G_DIR_SEPARATOR, addrIndex->fileName);
...
#if 0
void
addressbook_read_file_old (void)
{
  AddressIndex *addrIndex = NULL;
  gboolean errFlag = TRUE;
  gchar *msg = NULL;
          ...
          if (addrIndex->wasConverted)
            {
              ...
              if (addrIndex->retVal == MGU_SUCCESS)
                ...
                {
                  msg = g_strdup (_("Old address book converted, " "could not save new address index file"));
                }
            }
          else
            {
              ...
              if (addrIndex->retVal == MGU_SUCCESS)
                {
                  ...
                  if (addrIndex->retVal == MGU_SUCCESS)
                    {
                      msg = g_strdup (_("Could not convert address book, "
                                        "but created empty new address book files."));
                      errFlag = FALSE;
                    }
                  else
                    {
                      msg = g_strdup (_("Could not convert address book, " "could not create new address book files."));
                    }
                }
              else
                {
                  msg = g_strdup (_("Could not convert address book " "and could not create new address book files."));
                }
            }
        }
    }
...
*/
static gboolean
addressbook_add_object (GtkTreeIter * iter, GtkTreeIter * new_iter, AddressObject * obj)
{
  GtkTreeView *treeview = GTK_TREE_VIEW (addrbook.treeview);
  GtkTreeModel *model;
  GtkTreeIter added;
  ...
  if (atci && atci->showInTree)
    {
      ...
      if (otype == ADDR_BOOK && obj->name && !strcmp (obj->name, ADDR_DS_AUTOREG))
        name = _("Auto-registered address");
      else
        name = obj->name;
      ...
      gtk_tree_store_set (GTK_TREE_STORE (model), &added,
                          ...
                          COL_OBJ, obj, COL_PIXBUF, atci->icon_pixbuf, COL_PIXBUF_OPEN, atci->icon_open_pixbuf, -1);
      if (new_iter)
        *new_iter = added;
      return TRUE;
    }
...
*/
static gboolean
addressbook_node_add_group (GtkTreeIter * iter, AddressDataSource * ds, ItemGroup * itemGroup, GtkTreeIter * new_iter)
{
  GtkTreeView *treeview = GTK_TREE_VIEW (addrbook.treeview);
  GtkTreeStore *store;
  ...
  gtk_tree_store_set (store, &new_iter_,
                      COL_FOLDER_NAME, name,
                      COL_OBJ, adapter, COL_PIXBUF, atci->icon_pixbuf, COL_PIXBUF_OPEN, atci->icon_open_pixbuf, -1);
  if (new_iter)
    *new_iter = new_iter_;
  return TRUE;
}
...
*/
static gboolean
addressbook_node_add_folder (GtkTreeIter * iter, AddressDataSource * ds, ItemFolder * itemFolder,
                             AddressObjectType otype, GtkTreeIter * new_iter)
{
  GtkTreeView *treeview = GTK_TREE_VIEW (addrbook.treeview);
  GtkTreeStore *store;
  GtkTreeIter new_iter_;
...
#if 0
static void
addressbook_delete_object (AddressObject * obj)
{
  AdapterDSource *ads = NULL;
  AddressDataSource *ds = NULL;
  if (!obj)
...
void
addressbook_export_to_file (void)
{
  if (_addressIndex_)
    {
...
static void
size_allocated (GtkWidget * widget, GtkAllocation * allocation, gpointer data)
{
...
*/
static gint
addressbook_tree_compare (GtkTreeModel * model, GtkTreeIter * a, GtkTreeIter * b, gpointer data)
{
  gchar *name1 = NULL, *name2 = NULL;
  AddressObject *obj1 = NULL, *obj2 = NULL;
  gint ret;

  gtk_tree_model_get (model, a, COL_FOLDER_NAME, &name1, COL_OBJ, &obj1, -1);
  gtk_tree_model_get (model, b, COL_FOLDER_NAME, &name2, COL_OBJ, &obj2, -1);
...
static gint
addressbook_list_col_compare (GtkTreeModel * model, GtkTreeIter * a, GtkTreeIter * b, gint col)
{
  gchar *name1 = NULL, *name2 = NULL;
  gint ret;
...
static gint
addressbook_list_name_compare (GtkTreeModel * model, GtkTreeIter * a, GtkTreeIter * b, gpointer data)
{
  gint ret;
...
static gint
addressbook_list_address_compare (GtkTreeModel * model, GtkTreeIter * a, GtkTreeIter * b, gpointer data)
{
  gint ret;
...
static gint
addressbook_list_nickname_compare (GtkTreeModel * model, GtkTreeIter * a, GtkTreeIter * b, gpointer data)
{
  gint ret;
...
static gint
addressbook_list_remarks_compare (GtkTreeModel * model, GtkTreeIter * a, GtkTreeIter * b, gpointer data)
{
  gint ret;
...
static gboolean
addressbook_list_select_func (GtkTreeSelection * selection,
...
/* static */
gint
addressbook_obj_name_compare (gconstpointer a, gconstpointer b)
{
...
#if 0
static void
addressbook_book_show_message (AddressBookFile * abf)
{
  gchar msgbuf[ADDRESSBOOK_MSGBUF_SIZE] = "";
...
static void
addressbook_new_book_cb (gpointer data, guint action, GtkWidget * widget)
{
  GtkTreeModel *model;
  GtkTreeIter iter;
  AdapterDSource *ads;
  ...
  if (adapter == NULL)
    return;
  if (!yam_tree_row_reference_equal (addrbook.tree_selected, adapter->tree_row))
    return;

  model = gtk_tree_view_get_model (GTK_TREE_VIEW (addrbook.treeview));
  yam_tree_row_reference_get_iter (model, addrbook.tree_selected, &iter);
...
static void
addressbook_new_vcard_cb (gpointer data, guint action, GtkWidget * widget)
{
  GtkTreeModel *model;
  GtkTreeIter iter;
  AdapterDSource *ads;
...
#ifdef USE_LDAP
static void
addressbook_new_ldap_cb (gpointer data, guint action, GtkWidget * widget)
{
  GtkTreeModel *model;
  GtkTreeIter iter;
  AdapterDSource *ads;
...
static void
addressbook_ldap_show_message (SyldapServer * svr)
{
  gchar msgbuf[ADDRESSBOOK_MSGBUF_SIZE] = "";
...
static void
ldapsearch_callback (SyldapServer * sls)
{
  GtkTreeView *treeview = GTK_TREE_VIEW (addrbook.treeview);
  GtkTreeSelection *selection;
  GtkTreeModel *model;
 ...
 */
static void
addressbook_lup_clicked (GtkButton * button, gpointer data)
{
  GtkTreeView *treeview = GTK_TREE_VIEW (addrbook.treeview);
  GtkTreeSelection *selection;
  GtkTreeModel *model;
...
static void
addressbook_close_clicked (GtkButton * button, gpointer data)
{
  addressbook_close ();
}
...
*/
void
addrbookctl_build_map (GtkWidget * window)
{
  AddressTypeControlItem *atci;

  /* Build icons */
  stock_pixbuf_gdk (STOCK_PIXMAP_FOLDER_CLOSE, &folderpix);
  stock_pixbuf_gdk (STOCK_PIXMAP_FOLDER_OPEN, &folderopenpix);
  stock_pixbuf_gdk (STOCK_PIXMAP_GROUP, &grouppix);
  stock_pixbuf_gdk (STOCK_PIXMAP_VCARD, &vcardpix);
  stock_pixbuf_gdk (STOCK_PIXMAP_BOOK, &bookpix);
  stock_pixbuf_gdk (STOCK_PIXMAP_PERSON, &personpix);
  stock_pixbuf_gdk (STOCK_PIXMAP_ADDRESS, &addresspix);
  stock_pixbuf_gdk (STOCK_PIXMAP_CATEGORY, &categorypix);
  stock_pixbuf_gdk (STOCK_PIXMAP_LDAP, &ldappix);
...
*/
AddressTypeControlItem *
addrbookctl_lookup_iface (AddressIfType ifType)
{
...
static void
addrbookctl_free_address (AddressObject * obj)
{
  g_free (obj->name);
  obj->type = ADDR_NONE;
  obj->name = NULL;
...
static void
addrbookctl_free_interface (AdapterInterface * adapter)
{
...
static void
addrbookctl_free_datasource (AdapterDSource * adapter)
{
  addrbookctl_free_address (ADDRESS_OBJECT (adapter));
...
static void
addrbookctl_free_folder (AdapterFolder * adapter)
{
  addrbookctl_free_address (ADDRESS_OBJECT (adapter));
...
static void
addrbookctl_free_group (AdapterGroup * adapter)
{
  addrbookctl_free_address (ADDRESS_OBJECT (adapter));
...
void
addrbookctl_free_selection (GList * list)
{
  GList *node = list;
  while (node)
    {
...
*/
AdapterInterface *
addrbookctl_find_interface (AddressIfType ifType)
{
...
*/
void
addrbookctl_build_ifselect (void)
{
  GList *newList = NULL;
  gchar *selectStr;
  gchar **splitStr;
 ...
 */
gboolean
addressbook_add_contact (const gchar * name, const gchar * address, const gchar * remarks)
{
 ...
 */
gboolean
addressbook_add_contact_autoreg (const gchar * name, const gchar * address, const gchar * remarks)
{
...
*/
gboolean
addressbook_load_completion_full (AddressBookCompletionFunc func)
{
...
static gint
wrapper_func (const gchar * name, const gchar * firstname, const gchar * lastname, const gchar * nickname,
              const gchar * address)
{
...
gboolean
addressbook_load_completion (gint (*callBackFunc) (const gchar *, const gchar *, const gchar *))
{
  gboolean ret;
...
*/
static void
addressbook_import_ldif_cb (void)
{
  AddressDataSource *ds = NULL;
  AdapterDSource *ads = NULL;
  AddressBookFile *abf = NULL;
...
gboolean
addressbook_import_ldif_file (const gchar * file, const gchar * book_name)
{
  AddressBookFile *abf;
...
*/
static void
addressbook_import_csv_cb (void)
{
  AddressDataSource *ds = NULL;
  AdapterDSource *ads = NULL;
  AddressBookFile *abf = NULL;
...
*/
static void
addressbook_export_csv_cb (void)
{
  GtkTreeView *treeview = GTK_TREE_VIEW (addrbook.treeview);
  GtkTreeModel *model;
  GtkTreeIter iter;
...
static gint
load_address (const gchar * name, const gchar * address, const gchar * nickname)
{
  gchar *addr;
...
gboolean
addressbook_has_address (const gchar * address)
{
  GSList *list, *cur;
  gchar *addr;
