...
static gint import_progress_delete_event (GtkWidget * widget, GdkEventAny * event, gpointer data);

static gboolean
import_mbox_func (Folder * folder, FolderItem * item, guint count, guint total, gpointer data)
{
  gchar str[64];
...
gint
import_mail (FolderItem * default_dest)
{
  gint ok = 0;
  gchar *dest_id = NULL;
...
import_do (void)
{
  ...
  buf = g_path_get_basename (utf8filename);
  msg = g_strdup_printf (_("Importing %s ..."), buf);
  g_free (buf);
  ...
  g_signal_connect (G_OBJECT (progress->cancel_btn), "clicked", G_CALLBACK (import_progress_cancel_cb), NULL);
  g_signal_connect (G_OBJECT (progress->window), "delete_event", G_CALLBACK (import_progress_delete_event), NULL);
...
static gint
import_eml_folder (FolderItem * dest, const gchar * path)
{
  GDir *dir;
  ...
  while ((dir_name = g_dir_read_name (dir)) != NULL)
    {
      if (((p = strrchr (dir_name, '.')) && !g_ascii_strcasecmp (p + 1, "eml")) || to_number (dir_name) > 0)
        {
          file = g_strconcat (path, G_DIR_SEPARATOR_S, dir_name, NULL);
...
static gint32
read_dword (FILE * fp, off_t offset)
{
  gint32 dw;
...
static void
get_dbx_index (FILE * fp, gint32 table_pos, GArray * array)
{
  ...
  for (i = 0; i < num_elems; i++)
    {
      ...
      if (data_pos == 0)
        {
          g_warning ("get_dbx_index: null data_pos at %08x", table_pos);
          break;
        }
...
static gint
get_dbx_data (FILE * fp, gint32 data_pos, FolderItem * dest)
{
  gchar *tmp;
  FILE *outfp;
...
static gint
import_dbx (FolderItem * dest, const gchar * file)
{
  FILE *fp;
  gint32 dw;
  gint32 table_pos;
...
gint
import_dbx_folders (FolderItem * dest, const gchar * path)
{
  GDir *dir;
  const gchar *dir_name, *p;
  ...
  while ((dir_name = g_dir_read_name (dir)) != NULL)
    {
      if ((p = strrchr (dir_name, '.')) && !g_ascii_strcasecmp (p + 1, "dbx"))
        {
          file = g_strconcat (path, G_DIR_SEPARATOR_S, dir_name, NULL);
          orig_name = g_strndup (dir_name, p - dir_name);
          if (!g_file_test (file, G_FILE_TEST_IS_REGULAR) ||
              !g_ascii_strcasecmp (orig_name, "Folders") ||
              !g_ascii_strcasecmp (orig_name, "Offline") || !g_ascii_strcasecmp (orig_name, "Pop3uidl"))
            {
              g_free (orig_name);
              g_free (file);
              continue;
...
import_create (void)
{
  ...
  desc_label = gtk_label_new (_("Specify source file and destination folder."));
  ...
  gtk_widget_set_hexpand (format_optmenu, TRUE);
  ...
  gtk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (format_optmenu), _("eml (folder)"));
  gtk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (format_optmenu), _("Outlook Express (dbx)"));

  gtk_combo_box_set_active (GTK_COMBO_BOX (format_optmenu), 0);

  g_signal_connect (G_OBJECT (format_optmenu), "changed", G_CALLBACK (import_format_menu_cb), NULL);
  ...
  gtk_grid_attach (GTK_GRID (table), dest_entry, 2, 1, 1, 1);

  file_button = gtk_button_new_with_label (_(" Select... "));
  gtk_grid_attach (GTK_GRID (table), file_button, 1, 2, 1, 1);
  ...
  gtk_grid_attach (GTK_GRID (table), dest_button, 2, 2, 1, 1);
...
static void
import_format_menu_cb (GtkWidget * widget, gpointer data)
{
  gint type = gtk_combo_box_get_active (GTK_COMBO_BOX (widget));

  if (type == IMPORT_EML_FOLDER)
    gtk_label_set_text (GTK_LABEL (desc_label), _("Specify source folder including eml files and destination folder."));
  else
    gtk_label_set_text (GTK_LABEL (desc_label), _("Specify source file and destination folder."));
}

static void
import_ok_cb (GtkWidget * widget, gpointer data)
{
...
static void
import_cancel_cb (GtkWidget * widget, gpointer data)
{
  import_finished = TRUE;
  import_ack = FALSE;
}
...
static void
import_filesel_cb (GtkWidget * widget, gpointer data)
{
  gchar *filename;
  gchar *utf8_filename;
  gint type;
  ...
  if (type == IMPORT_EML_FOLDER)
    filename = filesel_select_file (_("Select importing folder"), NULL, GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER);
  else
    filename = filesel_select_file (_("Select importing file"), NULL, GTK_FILE_CHOOSER_ACTION_OPEN);
  if (!filename)
    return;
...
static void
import_destsel_cb (GtkWidget * widget, gpointer data)
{
  FolderItem *dest;
  gchar *dest_id;
...
static void
import_progress_cancel_cb (GtkWidget * widget, gpointer data)
{
...
static gint
import_progress_delete_event (GtkWidget * widget, GdkEventAny * event, gpointer data)
{
