...
static gboolean window_deleted (GtkWidget * widget, GdkEventAny * event, gpointer data);
static gboolean key_pressed (GtkWidget * widget, GdkEventKey * event, gpointer data);

static void view_destroyed (GtkWidget * widget, gpointer data);

void
message_search (MessageView * messageview)
{
  if (!search_window.window)
    message_search_create ();

  search_window.messageview = messageview;
  g_signal_handlers_disconnect_by_func (GTK_WIDGET_PTR (messageview), view_destroyed, messageview);
  g_signal_connect (G_OBJECT (GTK_WIDGET_PTR (messageview)), "destroy", G_CALLBACK (view_destroyed), messageview);
...
message_search_create (void)
{
  ...
  g_signal_connect (G_OBJECT (window), "delete_event", G_CALLBACK (window_deleted), NULL);
  ...
  g_signal_connect (G_OBJECT (body_entry), "activate", G_CALLBACK (body_activated), NULL);
  ...
  yam_stock_button_set_create (&confirm_area, &next_btn, "yam-go-forward", &prev_btn, "yam-go-back", &close_btn, "yam-close");
  ...
  g_signal_connect (G_OBJECT (close_btn), "clicked", G_CALLBACK (close_clicked), NULL);
...
static void
message_search_execute (gboolean backward)
{
  ...
  for (;;)
    {
      ...
      if (backward)
        {
          if (messageview_search_string_backward (messageview, body_str, case_sens) == TRUE)
            break;
        }
      else
        {
          if (messageview_search_string (messageview, body_str, case_sens) == TRUE)
            break;
        }

      if (all_searched)
        {
          alertpanel_message (_("Search failed"), _("Search string not found."), ALERT_WARNING);
          break;
        }
      ...
      val = alertpanel (_("Search finished"), str, "yam-yes", "yam-no", NULL);
      if (G_ALERTDEFAULT == val)
        {
          manage_window_focus_in (search_window.window, NULL, NULL);
          messageview_set_position (messageview, backward ? -1 : 0);
        }
      else
        break;
    }
...
static void
message_search_prev_clicked (GtkButton * button, gpointer data)
{
  message_search_execute (TRUE);
}

static void
message_search_next_clicked (GtkButton * button, gpointer data)
{
...
static void
view_destroyed (GtkWidget * widget, gpointer data)
{
