...
static void plugin_manager_set_list_row (GtkTreeIter * iter, YamPluginInfo * info, const gchar * filename);
static gint plugin_manager_deleted (GtkWidget * widget, GdkEventAny * event, gpointer data);
static gboolean key_pressed (GtkWidget * widget, GdkEventKey * event, gpointer data);

void
plugin_manager_open (void)
{
  GSList *list, *cur;
  YamPluginInfo *info;
  GModule *module;
  ...
  list = yam_plugin_get_module_list ();
  ...
  for (cur = list; cur != NULL; cur = cur->next)
    {
      ...
      info = yam_plugin_get_info (module);
      if (info)
        {
          debug_print ("------------------------------\n");
  ...
  yam_plugin_signal_emit ("plugin-manager-open", pm_window.window);
}

static void
plugin_manager_create (void)
{
  GtkWidget *window;
  GtkWidget *vbox;
  GtkWidget *close_btn;
  ...
  g_signal_connect (G_OBJECT (window), "delete_event", G_CALLBACK (plugin_manager_deleted), NULL);
  g_signal_connect (G_OBJECT (window), "key_press_event", G_CALLBACK (key_pressed), NULL);
  g_signal_connect (G_OBJECT (close_btn), "clicked", G_CALLBACK (plugin_manager_deleted), NULL);
  ...
  column = gtk_tree_view_column_new_with_attributes (_("Plug-in information"), renderer, "text", COL_INFO, NULL);
...
static void
plugin_manager_set_list_row (GtkTreeIter * iter, YamPluginInfo * info, const gchar * filename)
{
...
static gint
plugin_manager_deleted (GtkWidget * widget, GdkEventAny * event, gpointer data)
{
