...
static void
edit_group_status_show (gchar * msg)
{
...
static void
edit_group_ok (GtkWidget * widget, gboolean * cancelled)
{
  gchar *sName;
...
static void
edit_group_cancel (GtkWidget * widget, gboolean * cancelled)
{
  *cancelled = TRUE;
  gtk_main_quit ();
}
...
static gint
edit_group_delete_event (GtkWidget * widget, GdkEventAny * event, gboolean * cancelled)
{
  *cancelled = TRUE;
  gtk_main_quit ();
  return TRUE;
...
static gchar *
edit_group_format_item (ItemPerson * person, ItemEMail * email)
{
  gchar *str = NULL;
...
static void
edit_group_list_add_email (GtkTreeStore * store, GtkTreeIter * parent, ItemEMail * email)
{
  ...
  gtk_tree_store_set (store, &iter,
                      GROUP_COL_NAME, name,
                      GROUP_COL_EMAIL, email->address, GROUP_COL_REMARKS, email->remarks, GROUP_COL_DATA, email, -1);
  if (parent && GTK_TREE_MODEL (store) == gtk_tree_view_get_model (groupeditdlg.treeview_avail))
    yam_tree_view_expand_parent_all (groupeditdlg.treeview_avail, &iter);
...
*/
static void
edit_group_load_available_list_recurse (ItemFolder * folder, GtkTreeIter * parent, GHashTable * table)
{
  GtkTreeModel *model;
  GtkTreeStore *store;
  GtkTreeIter iter;
  ...
  if (ADDRITEM_PARENT (folder))
    {
      ...
      gtk_tree_store_set (store, &iter,
                          ...
                          GROUP_COL_PIXBUF, folderpix, GROUP_COL_PIXBUF_OPEN, folderopenpix, -1);
    }
...
static void
edit_group_load_available_list (AddressBookFile * abf, ItemGroup * group)
{
  GHashTable *table;
...
static void
edit_group_load_group_list (GtkTreeView * treeview, GList * listEMail)
{
...
static gboolean
edit_group_find_parent_folder (GtkTreeModel * model, GtkTreeIter * parent, ItemEMail * email)
{
  ItemPerson *person;
  ItemFolder *folder;
  ...
  if (yam_tree_model_find_by_column_data (model, parent, NULL, GROUP_COL_DATA, folder))
    return TRUE;
...
static gboolean
edit_group_find_next_selection (GtkTreeModel * model, GtkTreeIter * next)
{
  gboolean valid;
  GtkTreeIter iter = *next;
  AddrItemObject *obj;

  while ((valid = yam_tree_model_next (model, &iter)) != FALSE)
    {
...
static gboolean
edit_group_find_last_selection (GtkTreeModel * model, GtkTreeIter * next)
{
  gboolean valid;
  GtkTreeIter iter;
  GtkTreeIter last;
...
static void
edit_group_move_email (GtkTreeView * treeview_from, GtkTreeView * treeview_to)
{
  GtkTreeModel *model;
  ...
  for (cur = rows; cur != NULL; cur = cur->next)
    {
      ...
      if (obj && ADDRITEM_TYPE (obj) == ITEMTYPE_FOLDER)
        continue;
      email = (ItemEMail *) obj;
      if (treeview_to == groupeditdlg.treeview_avail && edit_group_find_parent_folder (model_to, &parent, email))
...
static void
edit_group_to_group (GtkWidget * widget, gpointer data)
{
  edit_group_move_email (groupeditdlg.treeview_avail, groupeditdlg.treeview_group);
}

static void
edit_group_to_avail (GtkWidget * widget, gpointer data)
{
  edit_group_move_email (groupeditdlg.treeview_group, groupeditdlg.treeview_avail);
}

static gboolean
edit_group_list_group_button (GtkWidget * treeview, GdkEventButton * event, gpointer data)
{
  if (!event)
    return FALSE;
  if (event->button == 1)
...
static gboolean
edit_group_list_avail_button (GtkWidget * treeview, GdkEventButton * event, gpointer data)
{
  if (!event)
    return FALSE;
  if (event->button == 1)
...
static gint
edit_group_list_col_compare (GtkTreeModel * model, GtkTreeIter * a, GtkTreeIter * b, gint col)
{
...
static gint
edit_group_list_name_compare_func (GtkTreeModel * model, GtkTreeIter * a, GtkTreeIter * b, gpointer data)
{
  gint ret;
...
static gint
edit_group_list_email_compare_func (GtkTreeModel * model, GtkTreeIter * a, GtkTreeIter * b, gpointer data)
{
  gint ret;
...
static gint
edit_group_list_remarks_compare_func (GtkTreeModel * model, GtkTreeIter * a, GtkTreeIter * b, gpointer data)
{
  gint ret;
...
static void
addressbook_edit_group_create (gboolean * cancelled)
{
  GtkWidget *window;
  GtkWidget *vbox;
  GtkWidget *vbox1;
  ...
  /* FIXME: too much layout containers !!! */
  ...
  /* List area */
  hboxl = gtk_box_new (GTK_ORIENTATION_HORIZONTAL, 8);
  ...
  gtk_scrolled_window_set_policy (GTK_SCROLLED_WINDOW (treeview_swin), GTK_POLICY_AUTOMATIC, GTK_POLICY_ALWAYS);
  ...
  gtk_tree_sortable_set_sort_func (GTK_TREE_SORTABLE (store), GROUP_COL_NAME, edit_group_list_name_compare_func, NULL, NULL);
  gtk_tree_sortable_set_sort_func (GTK_TREE_SORTABLE (store), GROUP_COL_EMAIL, edit_group_list_email_compare_func, NULL, NULL);
  gtk_tree_sortable_set_sort_func (GTK_TREE_SORTABLE (store), GROUP_COL_REMARKS, edit_group_list_remarks_compare_func, NULL, NULL);
  ...
  gtk_tree_view_column_set_attributes (column, renderer, "pixbuf", GROUP_COL_PIXBUF, "pixbuf-expander-open",
                                       GROUP_COL_PIXBUF_OPEN, "pixbuf-expander-closed", GROUP_COL_PIXBUF, NULL);
  ...
  renderer = gtk_cell_renderer_text_new ();
  g_object_set (renderer, "ellipsize", PANGO_ELLIPSIZE_END, "ypad", 0, NULL);
  column = gtk_tree_view_column_new_with_attributes (_("E-Mail Address"), renderer, "text", GROUP_COL_EMAIL, NULL);
  gtk_tree_view_column_set_spacing (column, 1);
  gtk_tree_view_column_set_sizing (column, GTK_TREE_VIEW_COLUMN_FIXED);
  ...
  renderer = gtk_cell_renderer_text_new ();
  g_object_set (renderer, "ellipsize", PANGO_ELLIPSIZE_END, "ypad", 0, NULL);
  column = gtk_tree_view_column_new_with_attributes (_("Remarks"), renderer, "text", GROUP_COL_REMARKS, NULL);
  gtk_tree_view_column_set_spacing (column, 1);
  gtk_tree_view_column_set_sizing (column, GTK_TREE_VIEW_COLUMN_FIXED);
  ...
  gtk_tree_sortable_set_sort_column_id (GTK_TREE_SORTABLE (store), GROUP_COL_NAME, GTK_SORT_ASCENDING);

  /* Add/Remove button */
  vboxb = gtk_box_new (GTK_ORIENTATION_VERTICAL, 0);
  gtk_box_pack_start (GTK_BOX (hboxl), vboxb, FALSE, FALSE, 0);

  vboxb1 = gtk_box_new (GTK_ORIENTATION_VERTICAL, 8);
  ...
  renderer = gtk_cell_renderer_text_new ();
  g_object_set (renderer, "ellipsize", PANGO_ELLIPSIZE_END, "ypad", 0, NULL);
  column = gtk_tree_view_column_new_with_attributes (_("Name"), renderer, "text", GROUP_COL_NAME, NULL);
  gtk_tree_view_column_set_spacing (column, 1);
  gtk_tree_view_column_set_sizing (column, GTK_TREE_VIEW_COLUMN_FIXED);
  gtk_tree_view_column_set_fixed_width (column, GROUP_COL_WIDTH_NAME);
  gtk_tree_view_column_set_resizable (column, TRUE);
  ...
  gtk_tree_sortable_set_sort_column_id (GTK_TREE_SORTABLE (store), GROUP_COL_NAME, GTK_SORT_ASCENDING);
  /* Button panel */
  hboxb = gtk_box_new (GTK_ORIENTATION_HORIZONTAL, 8);
  ...
  /* Event handlers */
  g_signal_connect (G_OBJECT (button_add), "clicked", G_CALLBACK (edit_group_to_group), NULL);
  g_signal_connect (G_OBJECT (button_remove), "clicked", G_CALLBACK (edit_group_to_avail), NULL);
  g_signal_connect (G_OBJECT (treeview_avail), "button_press_event", G_CALLBACK (edit_group_list_avail_button), NULL);
  g_signal_connect (G_OBJECT (treeview_group), "button_press_event", G_CALLBACK (edit_group_list_group_button), NULL);
...
*/
ItemGroup *
addressbook_edit_group (AddressBookFile * abf, ItemFolder * parent, ItemGroup * group)
{
...
*/
ItemFolder *
addressbook_edit_folder (AddressBookFile * abf, ItemFolder * parent, ItemFolder * folder)
{
  gchar *name = NULL;
