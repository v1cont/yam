...
static gint
get_header_data (MsgPrintInfo * mpinfo, PrintData * print_data)
{
  MsgInfo *msginfo;
  FILE *fp;
  GPtrArray *headers;
...
static gint
layout_set_headers (PangoLayout * layout, MsgPrintInfo * mpinfo, PrintData * print_data)
{
  ...
  debug_print ("layout_set_headers: using font %s (style %d, %g pt)\n", pango_font_description_get_family (desc),
               pango_font_description_get_style (desc), (gdouble) size / PANGO_SCALE);
...
static gint
message_count_page (MsgPrintInfo * mpinfo, GtkPrintContext * context, PrintData * print_data)
{
  cairo_t *cr;
  ...
  if (print_data->partinfo)
    {
      ...
      if ((msgfp = procmsg_open_message (mpinfo->msginfo)) == NULL)
        return -1;
      fp = procmime_get_text_content (print_data->partinfo, msgfp, NULL);
      fclose (msgfp);
    }
  else
    {
...
begin_print (GtkPrintOperation * operation, GtkPrintContext * context, gpointer data)
{
  ...
  for (i = 0; i < print_data->n_msgs; i++)
    {
      n_pages += message_count_page (&print_data->msgs[i], context, print_data);
    }
...
static void
draw_page (GtkPrintOperation * operation, GtkPrintContext * context, gint page_nr, gpointer data)
{
  PrintData *print_data = data;
  PageInfo *pinfo;
  MsgPrintInfo *mpinfo;
  ...
  debug_print ("draw_page: using font %s (style %d, %g pt)\n", pango_font_description_get_family (desc),
               pango_font_description_get_style (desc), (gdouble) font_size / PANGO_SCALE);
  ...
  while (count < pinfo->lines)
    {
      ...
      for (i = 0; i < lines; i++)
        {
          ...
          cairo_move_to (cr, 0, y + (gdouble) baseline / PANGO_SCALE);
      ...
      pango_layout_get_size (layout, NULL, &layout_h);
      cairo_move_to (cr, 0, y + (gdouble) layout_h / PANGO_SCALE + (SPACING / 72 * dpi));
    }
  debug_print ("count = %d\n", count);
...
gint
printing_print_messages_gtk (GSList * mlist, MimeInfo * partinfo, gboolean all_headers)
{
  ...
  g_signal_connect (op, "begin-print", G_CALLBACK (begin_print), print_data);
  ...
  res = gtk_print_operation_run
    (op, GTK_PRINT_OPERATION_ACTION_PRINT_DIALOG, GTK_WINDOW (main_window_get ()->window), NULL);
...
printing_page_setup_gtk (void)
{
  ...
  new_page_setup = gtk_print_run_page_setup_dialog (GTK_WINDOW (main_window_get ()->window), page_setup, settings);
...
static gint
check_command_line (const gchar * cmdline)
{
  gchar *msg;

  msg = g_strconcat
    (_("The message will be printed with the following command:"),
     "\n\n", cmdline ? cmdline : _("(Default print command)"), NULL);
  if (alertpanel (_("Print"), msg, "yam-ok", "yam-cancel", NULL) != G_ALERTDEFAULT)
    {
      g_free (msg);
      return -2;
    }
...
gint
printing_print_messages_with_command (GSList * mlist, gboolean all_headers, const gchar * cmdline)
{
  MsgInfo *msginfo;
  GSList *cur;
...
gint
printing_print_messages (GSList * mlist, gboolean all_headers)
{
  if (!prefs_common.use_print_cmd)
    return printing_print_messages_gtk (mlist, NULL, all_headers);
  else
    return printing_print_messages_with_command (mlist, all_headers, prefs_common.print_cmd);
}

gint
printing_print_message (MsgInfo * msginfo, gboolean all_headers)
{
  GSList mlist;
...
gint
printing_print_message_part (MsgInfo * msginfo, MimeInfo * partinfo)
{
  if (!prefs_common.use_print_cmd)
    {
      GSList mlist;
  ...
  procmsg_print_message_part (msginfo, partinfo, prefs_common.print_cmd, FALSE);
