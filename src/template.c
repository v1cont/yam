...
static Template *
template_load (gchar * filename, guint tmplid)
{
  Template *tmpl;
  FILE *fp;
  gchar buf[BUFFSIZE];
...
void
template_clear_config (GSList * tmpl_list)
{
  GSList *cur;
  Template *tmpl;
...
static gint
template_compare_id (gconstpointer a, gconstpointer b)
{
  ...
  while ((dir_name = g_dir_read_name (dir)) != NULL)
    {
      ...
      filename = g_strconcat (path, G_DIR_SEPARATOR_S, dir_name, NULL);
      ...
      if (tmpl)
        tmpl_list = g_slist_insert_sorted (tmpl_list, tmpl, template_compare_id);
...
void
template_write_config (GSList * tmpl_list)
{
  const gchar *path;
  GSList *cur;
  Template *tmpl;
  ...
  for (cur = tmpl_list; cur != NULL; cur = cur->next)
    {
      ...
      filename = g_strconcat (path, G_DIR_SEPARATOR_S, itos (tmpl->tmplid), NULL);
...
void
template_set_config (GSList * tmpl_list)
{
