...
static GSList *mh_get_msg_list (Folder * folder, FolderItem * item, gboolean use_cache);
...
static gint mh_add_msgs (Folder * folder, FolderItem * dest, GSList * file_list, gboolean remove_source, gint * first);
static gint mh_add_msg_msginfo (Folder * folder, FolderItem * dest, MsgInfo * msginfo, gboolean remove_source);
...
static gint mh_scan_folder_full (Folder * folder, FolderItem * item, gboolean count_sum);
...
static gint mh_move_folder (Folder * folder, FolderItem * item, FolderItem * new_parent);
...
static Folder *
mh_folder_new (const gchar * name, const gchar * path)
{
  Folder *folder;
...
static void
mh_folder_destroy (Folder * folder)
{
  folder_local_folder_destroy (LOCAL_FOLDER (folder));
}

static void
mh_folder_init (Folder * folder, const gchar * name, const gchar * path)
{
...
static GSList *
mh_get_msg_list_full (Folder * folder, FolderItem * item, gboolean use_cache, gboolean uncached_only)
{
  GSList *mlist;
...
static GSList *
mh_get_msg_list (Folder * folder, FolderItem * item, gboolean use_cache)
{
  return mh_get_msg_list_full (folder, item, use_cache, FALSE);
}

static GSList *
mh_get_uncached_msg_list (Folder * folder, FolderItem * item)
{
  return mh_get_msg_list_full (folder, item, TRUE, TRUE);
}

static gchar *
mh_fetch_msg (Folder * folder, FolderItem * item, gint num)
{
  gchar *path;
  gchar *file;
  gchar buf[16];
...
static MsgInfo *
mh_get_msginfo (Folder * folder, FolderItem * item, gint num)
{
  MsgInfo *msginfo;
  gchar *file;
...
static gchar *
mh_get_new_msg_filename (FolderItem * dest)
{
  gchar *destfile;
  gchar *destpath;
...
static gint
mh_add_msg (Folder * folder, FolderItem * dest, const gchar * file, MsgFlags * flags, gboolean remove_source)
{
  GSList file_list;
...
static gint
mh_add_msgs (Folder * folder, FolderItem * dest, GSList * file_list, gboolean remove_source, gint * first)
{
  gchar *destfile;
  GSList *cur;
  MsgFileInfo *fileinfo;
  ...
  for (cur = file_list; cur != NULL; cur = cur->next)
    {
      ...
      if (yam_link (fileinfo->file, destfile) < 0)
        {
      ...
      if (yam_app_get ())
        g_signal_emit_by_name (yam_app_get (), "add-msg", dest, destfile, dest->last_num + 1);
...
static gint
mh_add_msg_msginfo (Folder * folder, FolderItem * dest, MsgInfo * msginfo, gboolean remove_source)
{
  GSList msglist;
...
static gint
mh_add_msgs_msginfo (Folder * folder, FolderItem * dest, GSList * msglist, gboolean remove_source, gint * first)
{
  GSList *cur;
  MsgInfo *msginfo;
  gchar *srcfile;
  ...
  for (cur = msglist; cur != NULL; cur = cur->next)
    {
      ...
      if (yam_link (srcfile, destfile) < 0)
        {
      ...
      if (MSG_IS_RECEIVED (msginfo->flags))
        {
          ...
          if (dest->unmarked_num == 0)
            dest->new = 0;
          dest->unmarked_num++;
          procmsg_add_mark_queue (dest, dest->last_num, msginfo->flags);
        }
      else
        {
          SET_DEST_MSG_FLAGS (fp, dest, dest->last_num, msginfo->flags);
        }
      procmsg_add_cache_queue (dest, dest->last_num, msginfo);
...
static gint
mh_do_move_msgs (Folder * folder, FolderItem * dest, GSList * msglist)
{
  FolderItem *src;
  gchar *srcfile;
  gchar *destfile;
  ...
  for (cur = msglist; cur != NULL; cur = cur->next)
    {
      ...
      /* g_signal_emit_by_name(yam_app_get(), "remove-msg", src, srcfile, msginfo->msgnum); */
...
static gint
mh_move_msg (Folder * folder, FolderItem * dest, MsgInfo * msginfo)
{
  GSList msglist;
...
static gint
mh_move_msgs (Folder * folder, FolderItem * dest, GSList * msglist)
{
  MsgInfo *msginfo;
  gint ret = 0;
  gint first;
...
static gint
mh_copy_msg (Folder * folder, FolderItem * dest, MsgInfo * msginfo)
{
  GSList msglist;
...
static gint
mh_copy_msgs (Folder * folder, FolderItem * dest, GSList * msglist)
{
  gchar *srcfile;
  gchar *destfile;
  GSList *cur;
...
static gint
mh_remove_msg (Folder * folder, FolderItem * item, MsgInfo * msginfo)
{
  gchar *file;
  ...
  if (yam_app_get ())
    g_signal_emit_by_name (yam_app_get (), "remove-msg", item, file, msginfo->msgnum);
...
static gint
mh_remove_all_msg (Folder * folder, FolderItem * item)
{
  gchar *path;
  gint val;
...
static gboolean
mh_is_msg_changed (Folder * folder, FolderItem * item, MsgInfo * msginfo)
{
  GStatBuf s;
  gchar buf[16];

  if (g_stat (utos_buf (buf, msginfo->msgnum), &s) < 0 || msginfo->size != s.st_size || msginfo->mtime != s.st_mtime)
    return TRUE;
...
static gint
mh_close (Folder * folder, FolderItem * item)
{
  return 0;
}

static gint
mh_scan_folder_full (Folder * folder, FolderItem * item, gboolean count_sum)
{
  gchar *path;
  DIR *dp;
  struct dirent *d;
  ...
  if (n_msg == 0)
  ...
  else if (count_sum)
    {
      ...
      procmsg_get_mark_sum (item, &new, &unread, &total, &min, &max_, 0);
...
static gint
mh_scan_folder (Folder * folder, FolderItem * item)
{
  return mh_scan_folder_full (folder, item, TRUE);
}

static gint
mh_scan_tree (Folder * folder)
{
  FolderItem *item;
  gchar *rootpath;
...
static FolderItem *
mh_create_folder (Folder * folder, FolderItem * parent, const gchar * name)
{
  gchar *path;
  gchar *fs_name;
  gchar *fullpath;
...
static gint
mh_move_folder_real (Folder * folder, FolderItem * item, FolderItem * new_parent, const gchar * name)
{
  gchar *rootpath;
  gchar *oldpath;
  gchar *newpath;
  ...
  if (new_parent)
    {
      if (name)
      ...
      else
        {
          ...
          utf8_name = g_filename_to_utf8 (name_, -1, NULL, NULL, NULL);
          if (!utf8_name)
  ...
  else
    {
      ...
      newpath = g_strconcat (dirname, G_DIR_SEPARATOR_S, name_ ? name_ : name, NULL);
      g_free (dirname);
    }
  g_free (name_);
  ...
  if (new_parent)
    {
      ...
      if (new_parent->path != NULL)
        {
          newpath = g_strconcat (new_parent->path, "/", utf8_name, NULL);
          g_free (utf8_name);
        }
      else
        newpath = utf8_name;
...
static gint
mh_move_folder (Folder * folder, FolderItem * item, FolderItem * new_parent)
{
  return mh_move_folder_real (folder, item, new_parent, NULL);
}

static gint
mh_rename_folder (Folder * folder, FolderItem * item, const gchar * name)
{
  return mh_move_folder_real (folder, item, NULL, name);
}

static gint
mh_remove_folder (Folder * folder, FolderItem * item)
{
  gchar *path;
...
static GSList *
mh_get_uncached_msgs (GHashTable * msg_table, FolderItem * item)
{
  gchar *path;
  GDir *dp;
      ...
      while ((dir_name = g_dir_read_name (dp)) != NULL)
        {
          ...
          msginfo = g_hash_table_lookup (msg_table, GUINT_TO_POINTER (num));
  ...
  if (newlist && item->sort_key == SORT_BY_NONE)
    {
      ...
      newlist = g_slist_sort (newlist, (GCompareFunc) procmsg_cmp_msgnum_for_sort);
      debug_print ("done.\n");
    }
...
static MsgInfo *
mh_parse_msg (const gchar * file, FolderItem * item)
{
  MsgInfo *msginfo;
  MsgFlags flags;
...
static gboolean
mh_remove_missing_folder_items_func (GNode * node, gpointer data)
{
  FolderItem *item;
  gchar *path;
...
static void
mh_remove_missing_folder_items (Folder * folder)
{
...
static void
mh_scan_tree_recursive (FolderItem * item)
{
  Folder *folder;
  DIR *dp;
  struct dirent *d;
  ...
  fs_path = item->path ? g_filename_from_utf8 (item->path, -1, NULL, NULL, NULL) : g_strdup (".");
  if (!fs_path)
  ...
  while ((d = readdir (dp)) != NULL)
    {
      ...
      if (item->path)
        {
          utf8entry = g_strconcat (item->path, "/", utf8name, NULL);
        }
      else
      ...
      if (
           ...
           d->d_type == DT_DIR || ((d->d_type == DT_UNKNOWN || d->d_type == DT_LNK) &&
#endif
          ...
          if (!item->path)
            {
              ...
              else if (!folder_get_junk (folder) && !strcmp (dir_name, JUNK_DIR))
                {
  ...
  if (item->path)
    {
      ...
      procmsg_get_mark_sum (item, &new, &unread, &total, &min, &max, 0);
      if (n_msg > total)
        {
          new += n_msg - total;
          unread += n_msg - total;
