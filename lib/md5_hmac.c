...
*/
static SMD5 *
md5_hmac_get (const guchar * text, gint text_len, const guchar * key, gint key_len)
{
  SMD5 *md5;
  ...
  if (key_len > 64)
    {
      ...
      memcpy (k_ipad, s_gnet_md5_get_digest (tmd5), S_GNET_MD5_HASH_LENGTH);
      memcpy (k_opad, s_gnet_md5_get_digest (tmd5), S_GNET_MD5_HASH_LENGTH);
...
void
md5_hmac (guchar * digest, const guchar * text, gint text_len, const guchar * key, gint key_len)
{
  SMD5 *md5;
...
void
md5_hex_hmac (gchar * hexdigest, const guchar * text, gint text_len, const guchar * key, gint key_len)
{
  SMD5 *md5;
