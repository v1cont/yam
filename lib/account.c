...
void
account_write_config_all (void)
{
  prefs_account_write_config_all (account_list);
}

PrefsAccount *
account_get_current_account (void)
{
  return cur_account;
}

PrefsAccount *
account_find_from_smtp_server (const gchar * address, const gchar * smtp_server)
{
  GList *cur;
  PrefsAccount *ac;
  ...
  for (cur = account_list; cur != NULL; cur = cur->next)
    {
      ...
      if (!strcmp2 (address, ac->address) && !strcmp2 (smtp_server, ac->smtp_server))
        return ac;
    }
 ...
 */
PrefsAccount *
account_find_from_address (const gchar * address)
{
  GList *cur;
  PrefsAccount *ac;
...
PrefsAccount *
account_find_from_item (FolderItem * item)
{
  PrefsAccount *ac;
...
PrefsAccount *
account_find_from_item_property (FolderItem * item)
{
  PrefsAccount *ac;
...
PrefsAccount *
account_find_from_message_file (const gchar * file)
{
  static HeaderEntry hentry[] = { {"From:", NULL, FALSE},
  {"X-YAM-Account-Id:", NULL, FALSE},
  {"AID:", NULL, FALSE},
  {NULL, NULL, FALSE}
  };
  ...
  while ((hnum = procheader_get_one_field (buf, sizeof (buf), fp, hentry)) != -1)
    {
      ...
      else if (hnum == H_X_YAM_ACCOUNT_ID || hnum == H_AID)
        {
          PrefsAccount *tmp_ac;
...
PrefsAccount *
account_find_from_msginfo (MsgInfo * msginfo)
{
  gchar *file;
  PrefsAccount *ac;
...
gboolean
account_address_exist (const gchar * address)
{
  if (!address_table)
    {
      GList *cur;
      ...
      for (cur = account_list; cur != NULL; cur = cur->next)
        {
          ...
          if (ac->address)
            g_hash_table_insert (address_table, ac->address, GINT_TO_POINTER (1));
        }
    }
...
void
account_foreach (AccountFunc func, gpointer user_data)
{
  GList *cur;
...
void
account_append (PrefsAccount * ac_prefs)
{
  account_list = g_list_append (account_list, ac_prefs);
  account_updated ();
}
...
void
account_set_as_default (PrefsAccount * ac_prefs)
{
  PrefsAccount *ap;
  GList *cur;
...
FolderItem *
account_get_special_folder (PrefsAccount * ac_prefs, SpecialFolderItemType type)
{
...
void
account_destroy (PrefsAccount * ac_prefs)
{
...
void
account_update_lock (void)
{
  account_update_lock_count++;
}

void
account_update_unlock (void)
{
...
account_updated (void)
{
  ...
  if (yam_app_get ())
    g_signal_emit_by_name (yam_app_get (), "account-updated");
}
