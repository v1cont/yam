...
gint
procheader_get_one_field (gchar * buf, size_t len, FILE * fp, HeaderEntry hentry[])
{
  gint nexthead;
  gint hnum = 0;
  HeaderEntry *hp = NULL;
          ...
          if (nexthead == ' ' || nexthead == '\t')
          ...
          else if (folded == TRUE)
            {
              ...
              *bufp = '\0';
              ...
              if (fgets (bufp, len - (bufp - buf), fp) == NULL)
                break;
              bufp += strlen (bufp);
...
gchar *
procheader_get_unfolded_line (gchar * buf, size_t len, FILE * fp)
{
  gboolean folded = FALSE;
  gint nexthead;
  gchar *bufp;
...
GSList *
procheader_get_header_list_from_file (const gchar * file)
{
  FILE *fp;
  GSList *hlist;
...
GSList *
procheader_get_header_list (FILE * fp)
{
  gchar buf[BUFFSIZE];
  gchar *p;
  GSList *hlist = NULL;
...
GSList *
procheader_get_header_list_from_msginfo (MsgInfo * msginfo)
{
  GSList *hlist = NULL;
  ...
  if (msginfo->subject)
    hlist = procheader_add_header_list (hlist, "Subject", msginfo->subject);
  if (msginfo->from)
    hlist = procheader_add_header_list (hlist, "From", msginfo->from);
  if (msginfo->to)
    hlist = procheader_add_header_list (hlist, "To", msginfo->to);
  if (msginfo->cc)
    hlist = procheader_add_header_list (hlist, "Cc", msginfo->cc);
  if (msginfo->newsgroups)
    hlist = procheader_add_header_list (hlist, "Newsgroups", msginfo->newsgroups);
  if (msginfo->date)
    hlist = procheader_add_header_list (hlist, "Date", msginfo->date);

  return hlist;
}

GSList *
procheader_add_header_list (GSList * hlist, const gchar * header_name, const gchar * body)
{
  Header *header;
...
GSList *
procheader_copy_header_list (GSList * hlist)
{
  ...
  for (cur = hlist; cur != NULL; cur = cur->next)
    {
      ...
      newlist = procheader_add_header_list (newlist, header->name, header->body);
    }
...
GSList *
procheader_merge_header_list (GSList * hlist1, GSList * hlist2)
{
  GSList *cur;
...
GSList *
procheader_merge_header_list_dup (GSList * hlist1, GSList * hlist2)
{
  GSList *list, *cur;
  ...
  for (cur = hlist2; cur != NULL; cur = cur->next)
    {
      ...
      if (procheader_find_header_list (list, header->name) < 0)
        list = procheader_add_header_list (list, header->name, header->body);
    }
...
gint
procheader_find_header_list (GSList * hlist, const gchar * header_name)
{
  GSList *cur;
  gint index = 0;
  Header *header;
...
GPtrArray *
procheader_get_header_array (FILE * fp, const gchar * encoding)
{
  gchar buf[BUFFSIZE];
  gchar *p;
  GPtrArray *headers;
...
GPtrArray *
procheader_get_header_array_asis (FILE * fp, const gchar * encoding)
{
  gchar buf[BUFFSIZE];
  gchar *p;
  GPtrArray *headers;
...
GPtrArray *
procheader_get_header_array_for_display (FILE * fp, const gchar * encoding)
{
...
void
procheader_header_list_destroy (GSList * hlist)
{
  Header *header;
...
void
procheader_header_array_destroy (GPtrArray * harray)
{
  gint i;
  Header *header;
...
void
procheader_header_free (Header * header)
{
  if (!header)
    return;
...
void
procheader_get_header_fields (FILE * fp, HeaderEntry hentry[])
{
  gchar buf[BUFFSIZE];
  HeaderEntry *hp;
  gint hnum;
  ...
  while ((hnum = procheader_get_one_field (buf, sizeof (buf), fp, hentry)) != -1)
    {
      ...
      if (hp->body == NULL)
        hp->body = g_strdup (p);
      else if (!g_ascii_strcasecmp (hp->name, "To:") || !g_ascii_strcasecmp (hp->name, "Cc:"))
        {
          gchar *tp = hp->body;
...
MsgInfo *
procheader_parse_file (const gchar * file, MsgFlags flags, gboolean full)
{
  GStatBuf s;
  FILE *fp;
  MsgInfo *msginfo;
...
MsgInfo *
procheader_parse_str (const gchar * str, MsgFlags flags, gboolean full)
{
  FILE *fp;
  MsgInfo *msginfo;
...
MsgInfo *
procheader_parse_stream (FILE * fp, MsgFlags flags, gboolean full)
{
        ...
        case H_CONTENT_TYPE:
          if (!g_ascii_strncasecmp (hp, "multipart", 9))
          ...
          else
            {
              ...
              if (!charset)
                {
                  procmime_scan_content_type_str (hp, NULL, &charset, NULL, NULL);
                }
            }
          break;
        case H_SEEN:
  ...
  if (MSG_IS_MIME (msginfo->flags))
    {
      ...
      while (part)
        {
          if (part->mime_type != MIME_TEXT && part->mime_type != MIME_TEXT_HTML && part->mime_type != MIME_MULTIPART)
            break;
...
gchar *
procheader_get_fromname (const gchar * str)
{
  gchar *tmp, *name;
...
gchar *
procheader_get_toname (const gchar * str)
{
...
procheader_scan_date_string (const gchar * str,
                             ...
                             gchar * month, gint * year, gint * hh, gint * mm, gint * ss, gchar * zone)
{
  gint result;

  *zone = '\0';
  result = sscanf (str, "%10s %d %9s %d %2d:%2d:%2d %5s", weekday, day, month, year, hh, mm, ss, zone);
  if (result >= 7)
    return 0;

  result = sscanf (str, "%3s,%d %9s %d %2d:%2d:%2d %5s", weekday, day, month, year, hh, mm, ss, zone);
  if (result >= 7)
    return 0;
  ...
  result = sscanf (str, "%d-%2s-%2d %2d:%2d:%2d", year, month, day, hh, mm, ss);
  if (result == 6)
    return 0;
...
stime_t
procheader_date_parse (gchar * dest, const gchar * src, gint len)
{
...
void
procheader_date_get_localtime (gchar * dest, gint len, const stime_t timer)
{
  time_t timer_ = timer;
  struct tm *lt;
