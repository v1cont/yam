...
void
procmime_mimeinfo_free_all (MimeInfo * mimeinfo)
{
  while (mimeinfo != NULL)
    {
      MimeInfo *next;
...
MimeInfo *
procmime_mimeinfo_insert (MimeInfo * parent, MimeInfo * mimeinfo)
{
...
MimeInfo *
procmime_mimeinfo_next (MimeInfo * mimeinfo)
{
  if (!mimeinfo)
    return NULL;
...
MimeInfo *
procmime_scan_message (MsgInfo * msginfo)
{
  FILE *fp;
  MimeInfo *mimeinfo;
  ...
  if (mimeinfo)
    {
      ...
      if (mimeinfo->mime_type == MIME_MULTIPART || mimeinfo->mime_type == MIME_MESSAGE_RFC822)
...
MimeInfo *
procmime_scan_message_stream (FILE * fp)
{
  MimeInfo *mimeinfo;
  glong fpos;
...
void
procmime_scan_multipart_message (MimeInfo * mimeinfo, FILE * fp)
{
  gchar *p;
  gchar *boundary;
  gint boundary_len = 0;
  ...
  g_return_if_fail (mimeinfo->mime_type == MIME_MULTIPART || mimeinfo->mime_type == MIME_MESSAGE_RFC822);
  ...
  for (;;)
    {
      ...
      if (partinfo->mime_type == MIME_MULTIPART || partinfo->mime_type == MIME_MESSAGE_RFC822)
        {
      ...
      if (is_base64)
        partinfo->content_size = b64_content_len / 4 * 3 - b64_pad_len;
      else
...
void
procmime_scan_encoding (MimeInfo * mimeinfo, const gchar * encoding)
{
  gchar *enc;
...
void
procmime_scan_content_type (MimeInfo * mimeinfo, const gchar * content_type)
{
  g_free (mimeinfo->content_type);
  g_free (mimeinfo->charset);
  g_free (mimeinfo->name);
  ...
  procmime_scan_content_type_str (content_type, &mimeinfo->content_type,
                                  &mimeinfo->charset, &mimeinfo->name, &mimeinfo->boundary);
...
static gchar *
procmime_find_parameter_delimiter (const gchar * param, const gchar ** eq)
{
...
static gchar *
procmime_convert_value (const gchar * value, const gchar * charset)
{
  if (charset)
    {
      gchar *utf8_value;
...
static MimeParams *
procmime_parse_mime_parameter (const gchar * str)
{
  ConvADType ad_type;
  ...
  if ((ad_type == C_AD_JAPANESE || (ad_type == C_AD_BY_LOCALE && conv_is_ja_locale ())) && strstr (p, "\033$") != NULL)
    {
                  ...
                  if (sq2 > sq1 + 1)
                    {
                      if (lang)
                        g_free (lang);
                      lang = g_strndup (sq1 + 1, sq2 - sq1 - 1);
                    }
                  begin = sq2 + 1;
                }
            }
          ...
          if (encoded)
            {
              dec_value = g_malloc (strlen (begin) + 1);
              decode_xdigit_encoded_str (dec_value, begin);
              mparam->value = procmime_convert_value (dec_value, charset);
              g_free (dec_value);
            }
          else
            {
              if (!ast && (!g_ascii_strcasecmp (name, "name") || !g_ascii_strcasecmp (name, "filename")))
                mparam->value = conv_unmime_header (begin, NULL);
              else
...
static void
procmime_mime_params_free (MimeParams * mparams)
{
  GSList *cur;
...
void
procmime_scan_content_type_str (const gchar * content_type,
                                gchar ** mime_type, gchar ** charset, gchar ** name, gchar ** boundary)
{
  MimeParams *mparams;
  GSList *cur;
  ...
  for (cur = mparams->plist; cur != NULL; cur = cur->next)
    {
      ...
      else if (boundary && !g_ascii_strcasecmp (param->name, "boundary"))
        {
...
void
procmime_scan_content_type_partial (const gchar * content_type, gint * total, gchar ** part_id, gint * number)
{
  MimeParams *mparams;
  GSList *cur;
  gchar *id_str = NULL;
  ...
  *number = 0;
  ...
  if (!mparams->hvalue || g_ascii_strcasecmp (mparams->hvalue, "message/partial") != 0)
    {
      procmime_mime_params_free (mparams);
      return;
    }
...
void
procmime_scan_content_disposition (MimeInfo * mimeinfo, const gchar * content_disposition)
{
  MimeParams *mparams;
  GSList *cur;
...
procmime_scan_mime_header (FILE * fp)
{
  static HeaderEntry hentry[] = { 
    { "Content-Transfer-Encoding:", NULL, FALSE },
    { "Content-Type:", NULL, TRUE },
    { "Content-Disposition:", NULL, TRUE },
    { NULL, NULL, FALSE }
  };
  gchar buf[BUFFSIZE];
  gint hnum;
  ...
  if (mimeinfo->mime_type == MIME_APPLICATION_OCTET_STREAM && (mimeinfo->filename || mimeinfo->name))
    {
      const gchar *type;
      type = procmime_get_mime_type (mimeinfo->filename ? mimeinfo->filename : mimeinfo->name);
      if (type)
...
static gint
procmime_normalize_lbreak (FILE * infp, FILE * outfp)
{
  gchar buf[BUFFSIZE];
  gint len;
...
FILE *
procmime_decode_content (FILE * outfp, FILE * infp, MimeInfo * mimeinfo)
{
  gchar buf[BUFFSIZE];
  gchar *boundary = NULL;
  gint boundary_len = 0;
  ...
  if (mimeinfo->encoding_type == ENC_QUOTED_PRINTABLE)
    {
      ...
      while (fgets (buf, sizeof (buf), infp) != NULL && (!boundary || !IS_BOUNDARY (buf, boundary, boundary_len)))
        {
          gint len;
  ...
  else if (mimeinfo->encoding_type == ENC_BASE64)
    {
      ...
      while (fgets (buf, sizeof (buf), infp) != NULL && (!boundary || !IS_BOUNDARY (buf, boundary, boundary_len)))
        {
          len = base64_decoder_decode (decoder, buf, (guchar *) outbuf);
          if (len < 0)
            {
...
gint
procmime_get_part (const gchar * outfile, const gchar * infile, MimeInfo * mimeinfo)
{
  FILE *infp;
  gint ret;
...
gint
procmime_get_part_fp (const gchar * outfile, FILE * infp, MimeInfo * mimeinfo)
{
  FILE *outfp;
  gchar buf[BUFFSIZE];
...
FILE *
procmime_get_part_fp_fp (FILE * outfp, FILE * infp, MimeInfo * mimeinfo)
{
  gchar buf[BUFFSIZE];
...
gint
procmime_get_all_parts (const gchar * dir, const gchar * infile, MimeInfo * mimeinfo)
{
  FILE *fp;
  MimeInfo *partinfo;
  gchar *base, *filename;
      ...
      if (partinfo->filename || partinfo->name)
        {
          ...
          filename = g_strconcat (dir, G_DIR_SEPARATOR_S, base, NULL);
          ...
          while (is_file_entry_exist (filename))
            {
              ...
              g_free (filename);
              filename = g_strconcat (dir, G_DIR_SEPARATOR_S, base_alt, NULL);
              g_free (base_alt);
            }
...
FILE *
procmime_get_text_content (MimeInfo * mimeinfo, FILE * infp, const gchar * encoding)
{
  FILE *tmpfp, *outfp;
  ...
  g_return_val_if_fail (mimeinfo != NULL, NULL);
  g_return_val_if_fail (infp != NULL, NULL);
  g_return_val_if_fail (mimeinfo->mime_type == MIME_TEXT || mimeinfo->mime_type == MIME_TEXT_HTML, NULL);
  ...
  src_encoding = prefs_common.force_charset ? prefs_common.force_charset
    : mimeinfo->charset ? mimeinfo->charset : prefs_common.default_encoding;
...
FILE *
procmime_get_first_text_content (MsgInfo * msginfo, const gchar * encoding)
{
...
gboolean
procmime_find_string_part (MimeInfo * mimeinfo, const gchar * filename, const gchar * str, StrFindFunc find_func)
{
  ...
  g_return_val_if_fail (mimeinfo->mime_type == MIME_TEXT || mimeinfo->mime_type == MIME_TEXT_HTML, FALSE);
...
gboolean
procmime_find_string (MsgInfo * msginfo, const gchar * str, StrFindFunc find_func)
{
  MimeInfo *mimeinfo;
  MimeInfo *partinfo;
  gchar *filename;
  ...
  for (partinfo = mimeinfo; partinfo != NULL; partinfo = procmime_mimeinfo_next (partinfo))
    {
      if (partinfo->mime_type == MIME_TEXT || partinfo->mime_type == MIME_TEXT_HTML)
        {
          if (procmime_find_string_part (partinfo, filename, str, find_func) == TRUE)
            {
              found = TRUE;
              break;
            }
...
gchar *
procmime_get_part_file_name (MimeInfo * mimeinfo)
{
  gchar *base, *base_;

  base_ = g_path_get_basename (mimeinfo->filename ? mimeinfo->filename : mimeinfo->name ? mimeinfo->name : "mimetmp");
  if (*base_ == '\0')
    {
      g_free (base_);
      base_ = g_strdup ("mimetmp");
    }
...
gchar *
procmime_get_tmp_file_name (MimeInfo * mimeinfo)
{
  static guint32 id = 0;
  gchar *base;
  gchar *filename = NULL;
...
gchar *
procmime_get_tmp_file_name_for_user (MimeInfo * mimeinfo)
{
  gchar *base;
  gchar *filename;
  gint count = 1;
  ...
  filename = g_strconcat (get_mime_tmp_dir (), G_DIR_SEPARATOR_S, base, NULL);
  while (is_file_entry_exist (filename))
    {
      gchar *base_alt;
      ...
      base_alt = get_alt_filename (base, count++);
      g_free (filename);
      filename = g_strconcat (get_mime_tmp_dir (), G_DIR_SEPARATOR_S, base_alt, NULL);
      g_free (base_alt);
    }
...
ContentType
procmime_scan_mime_type (const gchar * mime_type)
{
  ContentType type;
  ...
  if (!g_ascii_strncasecmp (mime_type, "text/html", 9))
    ...
    type = MIME_TEXT;
  else if (!g_ascii_strncasecmp (mime_type, "application/octet-stream", 24))
...
gchar *
procmime_get_mime_type (const gchar * filename)
{
  ...
  if (!mime_type_list)
    {
      ...
      mime_type_list = procmime_get_mime_type_list (SYSCONFDIR "/mime.types");
      if (!mime_type_list)
        mime_type_list = procmime_get_mime_type_list ("/etc/mime.types");
      dir = g_strconcat (get_rc_dir (), G_DIR_SEPARATOR_S, "mime.types", NULL);
...
static GList *
procmime_get_mime_type_list (const gchar * file)
{
  GList *list = NULL;
  FILE *fp;
  gchar buf[BUFFSIZE];
...
static GList *
procmime_parse_mailcap (const gchar * file)
{
  GList *list = NULL;
  FILE *fp;
  gchar buf[BUFFSIZE];
  ...
  while (fgets (buf, sizeof (buf), fp) != NULL)
    {
      ...
      if (!strv[0] || *strv[0] == '\0' || !strv[1] || *strv[1] == '\0')
        {
          g_strfreev (strv);
          continue;
        }
...
gint
procmime_execute_open_file (const gchar * file, const gchar * mime_type)
{
  ...
  if (!mailcap_list_init && !mailcap_list)
    {
      ...
      path = g_build_filename (get_rc_dir (), "mailcap", NULL);
      mailcap_list = procmime_parse_mailcap (path);
      g_free (path);
      if (!mailcap_list)
        {
          path = g_build_filename (g_get_home_dir (), ".mailcap", NULL);
          mailcap_list = procmime_parse_mailcap (path);
          g_free (path);
        }
  ...
  for (cur = mailcap_list; cur != NULL; cur = cur->next)
    {
      ...
      if (str_find_format_times (mailcap->cmdline_fmt, 's') == 1)
        ...
        cmdline = g_strconcat (mailcap->cmdline_fmt, " \"", file, "\"", NULL);
...
EncodingType
procmime_get_encoding_for_charset (const gchar * charset)
{
  if (!charset)
    return ENC_8BIT;
    ...
    return ENC_7BIT;
           ...
           !g_ascii_strncasecmp (charset, "KOI8-", 5) || !g_ascii_strcasecmp (charset, "Windows-1251"))
    return ENC_8BIT;
...
EncodingType
procmime_get_encoding_for_text_file (const gchar * file)
{
  FILE *fp;
  guchar buf[BUFFSIZE];
  size_t len;
...
EncodingType
procmime_get_encoding_for_str (const gchar * str)
{
  const guchar *p;
  size_t octet_chars = 0;
  size_t total_len = 0;
