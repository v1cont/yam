...
void
slist_free_strings (GSList * list)
{
  while (list != NULL)
    {
      g_free (list->data);
...
static void
hash_free_strings_func (gpointer key, gpointer value, gpointer data)
{
  g_free (key);
}

void
hash_free_strings (GHashTable * table)
{
  g_hash_table_foreach (table, hash_free_strings_func, NULL);
}

static void
hash_free_value_mem_func (gpointer key, gpointer value, gpointer data)
{
  g_free (value);
}

void
hash_free_value_mem (GHashTable * table)
{
  g_hash_table_foreach (table, hash_free_value_mem_func, NULL);
}

gint
str_case_equal (gconstpointer v, gconstpointer v2)
{
...
void
ptr_array_free_strings (GPtrArray * array)
{
  gint i;
  gchar *str;
...
gboolean
str_find (const gchar * haystack, const gchar * needle)
{
  return strstr (haystack, needle) != NULL ? TRUE : FALSE;
}

gboolean
str_case_find (const gchar * haystack, const gchar * needle)
{
  return strcasestr (haystack, needle) != NULL ? TRUE : FALSE;
}

gboolean
str_find_equal (const gchar * haystack, const gchar * needle)
{
  return strcmp (haystack, needle) == 0;
}

gboolean
str_case_find_equal (const gchar * haystack, const gchar * needle)
{
  return g_ascii_strcasecmp (haystack, needle) == 0;
}

gint
to_number (const gchar * nstr)
{
...
gchar *
to_human_readable_buf (gchar * buf, size_t bufsize, gint64 size)
{
  if (size < 1024)
...
gint
strcmp2 (const gchar * s1, const gchar * s2)
{
  if (s1 == NULL || s2 == NULL)
    return -1;
  else
...
/* compare paths */
gint
path_cmp (const gchar * s1, const gchar * s2)
{
  gint len1, len2;
...
gboolean
is_path_parent (const gchar * parent, const gchar * child)
{
  gint plen;
  const gchar *base;
...
gchar *
strtailchomp (gchar * str, gchar tail_char)
{
  register gchar *s;
...
gpointer
my_memmem (gconstpointer haystack, size_t haystacklen, gconstpointer needle, size_t needlelen)
{
  ...
  while ((haystack_cur = memchr (haystack_cur, *needle_, haystack_left)) != NULL)
    {
...
gchar *
strncpy2 (gchar * dest, const gchar * src, size_t n)
{
...
gboolean
str_has_suffix_case (const gchar * str, const gchar * suffix)
{
  size_t len, s_len;
...
gint
str_find_format_times (const gchar * haystack, gchar ch)
{
  gint n = 0;
...
gboolean
is_next_nonascii (const gchar * s)
{
  const gchar *p;
...
gint
get_next_word_len (const gchar * s)
{
  const gchar *p = s;
  gboolean in_quote = FALSE;
...
/* compare subjects */
gint
subject_compare (const gchar * s1, const gchar * s2)
{
  gchar *str1, *str2;
...
gint
subject_compare_for_sort (const gchar * s1, const gchar * s2)
{
  gchar *str1, *str2;
...
void
trim_subject_for_compare (gchar * str)
{
  gchar *srcp;
...
void
trim_subject_for_sort (gchar * str)
{
  gchar *srcp;
...
void
eliminate_parenthesis (gchar * str, gchar op, gchar cl)
{
  register gchar *srcp, *destp;
  gint in_brace;
...
void
extract_parenthesis (gchar * str, gchar op, gchar cl)
{
  register gchar *srcp, *destp;
  gint in_brace;
...
void
extract_parenthesis_with_escape (gchar * str, gchar op, gchar cl)
{
  register gchar *srcp, *destp;
  gint in_brace;
...
void
extract_parenthesis_with_skip_quote (gchar * str, gchar quote_chr, gchar op, gchar cl)
{
  register gchar *srcp, *destp;
  gint in_brace;
  gboolean in_quote = FALSE;
...
void
eliminate_quote (gchar * str, gchar quote_chr)
{
  register gchar *srcp, *destp;
...
void
extract_quote_with_escape (gchar * str, gchar quote_chr)
{
...
void
eliminate_address_comment (gchar * str)
{
  register gchar *srcp, *destp;
  gint in_brace;
...
gchar *
strchr_with_skip_quote (const gchar * str, gint quote_chr, gint c)
{
  gboolean in_quote = FALSE;
...
gchar *
strrchr_with_skip_quote (const gchar * str, gint quote_chr, gint c)
{
  gboolean in_quote = FALSE;
  const gchar *p;
...
gchar *
extract_addresses (const gchar * str)
{
  GString *new_str;
...
gchar *
normalize_address_field (const gchar * str)
{
  GString *new_str;
  GSList *addr_list, *cur;
  gchar *addr, *p, *q, *r;
              ...
              if (*r)
                {
                  ...
                  if (new_str->len > 0 && !g_ascii_isspace (new_str->str[new_str->len - 1]))
                    g_string_append_c (new_str, ' ');
                }
...
gboolean
address_equal (const gchar * addr1, const gchar * addr2)
{
  gchar *addr1_, *addr2_;
...
GSList *
address_list_append_orig (GSList * addr_list, const gchar * str)
{
  const gchar *p = str, *q;
  gchar *addr;
...
GSList *
address_list_append (GSList * addr_list, const gchar * str)
{
  gchar *work;
  gchar *workp;
...
GSList *
references_list_prepend (GSList * msgid_list, const gchar * str)
{
  const gchar *strp;
...
GSList *
references_list_append (GSList * msgid_list, const gchar * str)
{
  GSList *list;
...
GList *
add_history (GList * list, const gchar * str)
{
  GList *old;
...
void
subst_char (gchar * str, gchar orig, gchar subst)
{
  register gchar *p = str;
...
void
subst_chars (gchar * str, gchar * orig, gchar subst)
{
  register gchar *p = str;
...
void
subst_null (gchar * str, gint len, gchar subst)
{
  register gchar *p = str;
...
void
subst_control (gchar * str, gchar subst)
{
  register gchar *p = str;
...
void
subst_for_filename (gchar * str)
{
  subst_chars (str, " \t\r\n\"'\\/:;*?<>|", '_');
}

gchar *
get_alt_filename (const gchar * filename, gint count)
{
  const gchar *ext;
  gchar *alt_filename;
...
gboolean
is_header_line (const gchar * str)
{
  if (str[0] == ':')
    return FALSE;
...
gint
get_quote_level (const gchar * str)
{
  ...
  while (p <= last_pos)
    {
      ...
      if (*p == '>')
      ...
      else if (*p != '-' && !g_ascii_isspace (*p) && p <= last_pos)
        {
          ...
          while (*p != '-' && *p != '>' && !g_ascii_isspace (*p) && p < last_pos)
            p++;
          if (*p == '>')
            quote_level++;
          else
...
gint
check_line_length (const gchar * str, gint max_chars, gint * line)
{
  const gchar *p = str, *q;
  gint cur_line = 0, len;
...
gchar *
strstr_with_skip_quote (const gchar * haystack, const gchar * needle)
{
...
gchar *
strcasestr_with_skip_quote (const gchar * haystack, const gchar * needle)
{
  register guint haystack_len, needle_len;
  gboolean in_squote = FALSE, in_dquote = FALSE;
  ...
  while (haystack_len >= needle_len)
    {
      if (!in_squote && !in_dquote && !g_ascii_strncasecmp (haystack, needle, needle_len))
        return (gchar *) haystack;
...
gchar *
strchr_parenthesis_close (const gchar * str, gchar op, gchar cl)
{
  const gchar *p;
  gchar quote_chr = '"';
  gint in_brace;
...
gchar **
strsplit_parenthesis (const gchar * str, gchar op, gchar cl, gint max_tokens)
{
      ...
      do
        {
          ...
          if (*str != op)
            {
              string_list = g_slist_prepend (string_list, g_strdup (""));
              n++;
              s_op = strchr_with_skip_quote (str, '"', op);
              if (!--max_tokens || !s_op)
                break;
...
gchar **
strsplit_with_quote (const gchar * str, const gchar * delim, gint max_tokens)
{
  GSList *string_list = NULL, *slist;
...
gchar **
strsplit_csv (const gchar * str, gchar delim, gint max_tokens)
{
  GSList *string_list = NULL, *slist;
  gchar **str_array, *s, *new_str;
  gchar *tmp, *tmpp, *p;
 ...
 */
gchar *
strconcat_csv (gchar delim, const gchar * field1, ...)
{
  va_list args;
  gchar *s;
  GString *csv;
...
gchar *
get_abbrev_newsgroup_name (const gchar * group, gint len)
{
  gchar *abbrev_group;
  gchar *ap;
  const gchar *p = group;
...
gchar *
trim_string (const gchar * str, gint len)
{
  const gchar *p = str;
  gint mb_len;
  gchar *new_str;
...
gchar *
trim_string_before (const gchar * str, gint len)
{
  const gchar *p = str;
  gint mb_len;
  gint new_len;
...
GList *
uri_list_extract_filenames (const gchar * uri_list)
{
  GList *result = NULL;
  gchar *file;
...
static void
get_hex_str (gchar * out, guchar ch)
{
  gchar hex;
...
gboolean
is_uri_string (const gchar * str)
{
  return (g_ascii_strncasecmp (str, "http://", 7) == 0 ||
          g_ascii_strncasecmp (str, "https://", 8) == 0 ||
          g_ascii_strncasecmp (str, "ftp://", 6) == 0 || g_ascii_strncasecmp (str, "www.", 4) == 0);
}

gchar *
get_uri_path (const gchar * uri)
{
 ...
 */
void
decode_uri (gchar * decoded_uri, const gchar * encoded_uri)
{
  ...
  while (*enc)
    {
      if (*enc == '%')
        {
          enc++;
          if (g_ascii_isxdigit ((guchar) enc[0]) && g_ascii_isxdigit ((guchar) enc[1]))
            {
              *dec = axtoi (enc);
              dec++;
              enc += 2;
...
void
decode_xdigit_encoded_str (gchar * decoded, const gchar * encoded)
{
  gchar *dec = decoded;
...
gchar *
encode_uri (const gchar * filename)
{
  gchar *uri;
...
gchar *
uriencode_for_filename (const gchar * filename)
{
...
gchar *
uriencode_for_mailto (const gchar * mailto)
{
...
gint
scan_mailto_url (const gchar * mailto, gchar ** to, gchar ** cc, gchar ** bcc,
  ...
  if (dir)
    {
      if (g_path_is_absolute (dir))
        ...
        rc_dir = g_strconcat (get_startup_dir (), G_DIR_SEPARATOR_S, dir, NULL);
    }
  else
    rc_dir = NULL;
}
...
{
  if (!rc_dir)
    rc_dir = g_build_filename  (g_get_user_data_dir (),  RC_DIR, NULL);
  return rc_dir;
}
...
off_t
get_file_size (const gchar * file)
{
  GStatBuf s;
...
off_t
get_file_size_as_crlf (const gchar * file)
{
  FILE *fp;
  off_t size = 0;
  gchar buf[BUFFSIZE];
...
gint
get_last_empty_line_size (FILE * fp, off_t size)
{
  glong pos;
  gint lsize = 0;
  gchar buf[4];
...
gboolean
is_file_entry_exist (const gchar * file)
{
  if (file == NULL)
    return FALSE;
...
gboolean
dirent_is_regular_file (struct dirent *d)
{
...
gboolean
dirent_is_directory (struct dirent *d)
{
#ifdef HAVE_DIRENT_D_TYPE
  if (d->d_type == DT_DIR)
    return TRUE;
...
gint
remove_all_files (const gchar * dir)
{
  GDir *dp;
...
gint
remove_numbered_files (const gchar * dir, guint first, guint last)
{
  GDir *dp;
  const gchar *dir_name;
  gchar *prev_dir;
...
gint
remove_all_numbered_files (const gchar * dir)
{
  return remove_numbered_files (dir, 0, UINT_MAX);
}

gint
remove_expired_files (const gchar * dir, guint hours)
{
  GDir *dp;
  const gchar *dir_name;
  GStatBuf s;
...
static gint
remove_dir_recursive_real (const gchar * dir)
{
  GStatBuf s;
  GDir *dp;
  const gchar *dir_name;
...
gint
rename_force (const gchar * oldpath, const gchar * newpath)
{
  return g_rename (oldpath, newpath);
}

gint
copy_file (const gchar * src, const gchar * dest, gboolean keep_backup)
{
  gint srcfd, destfd;
  gint n_read;
  gchar buf[BUFFSIZE];
...
gint
copy_dir (const gchar * src, const gchar * dest)
{
  GDir *dir;
  const gchar *dir_name;
  gchar *src_file;
  ...
  while ((dir_name = g_dir_read_name (dir)) != NULL)
    {
      ...
      dest_file = g_strconcat (dest, G_DIR_SEPARATOR_S, dir_name, NULL);
...
gint
move_file (const gchar * src, const gchar * dest, gboolean overwrite)
{
...
gint
append_file_part (FILE * fp, off_t offset, size_t length, FILE * dest_fp)
{
  gint n_read;
...
gint
copy_file_part (FILE * fp, off_t offset, size_t length, const gchar * dest)
{
  FILE *dest_fp;
...
gint
copy_file_stream (FILE * fp, FILE * dest_fp)
{
  gint n_read;
  gchar buf[BUFFSIZE];
 ...
 */
gchar *
canonicalize_str (const gchar * str)
{
  const gchar *p;
  guint new_len = 0;
  gchar *out, *outp;
...
gint
canonicalize_file (const gchar * src, const gchar * dest)
{
  FILE *src_fp, *dest_fp;
  gchar buf[BUFFSIZE];
  gint len;
...
FILE *
canonicalize_file_stream (FILE * src_fp, gint * length)
{
  FILE *dest_fp;
  gchar buf[BUFFSIZE];
  gint len;
...
gint
uncanonicalize_file (const gchar * src, const gchar * dest)
{
  FILE *src_fp, *dest_fp;
  gchar buf[BUFFSIZE];
  gboolean err = FALSE;
...
gint
uncanonicalize_file_replace (const gchar * file)
{
  gchar *tmp_file;
...
gchar *
normalize_newlines (const gchar * str)
{
  const gchar *p = str;
  gchar *out, *outp;
  ...
  while (*p != '\0')
    {
      ...
      if (newline)
        {
          for (last = newline; p < last && g_ascii_isspace (*(last - 1)); --last)
            ;
        ...
        {
          for (last = p + strlen (p); p < last && g_ascii_isspace (*(last - 1)); --last)
            ;
          strncpy (outp, p, last - p);
          outp += last - p;
          break;
...
FILE *
get_outgoing_rfc2822_file (FILE * fp)
{
  gchar buf[BUFFSIZE];
  FILE *outfp;
...
gchar *
generate_mime_boundary (const gchar * prefix)
{
  ...
  return g_strdup_printf ("%s=_%s_%s", prefix ? prefix : "Multipart", buf_date, buf_uniq);
}
...
gint
str_write_to_file (const gchar * str, const gchar * file)
{
  FILE *fp;
  size_t len;
...
execute_async (gchar * const argv[])
{
  ...
  if (g_spawn_async (NULL, (gchar **) argv, NULL, G_SPAWN_SEARCH_PATH, NULL, NULL, NULL, NULL) == FALSE)
    {
...
gint
execute_sync (gchar * const argv[])
{
  gint status;

  g_return_val_if_fail (argv != NULL && argv[0] != NULL, -1);

  if (g_spawn_sync (NULL, (gchar **) argv, NULL, G_SPAWN_SEARCH_PATH, NULL, NULL, NULL, NULL, &status, NULL) == FALSE)
    {
      g_warning ("Can't execute command: %s\n", argv[0]);
      return -1;
    }
...
gint
execute_command_line (const gchar * cmdline, gboolean async)
{
  gchar **argv;
  gint ret;
  ...
  if (debug_mode)
    {
      ...
      utf8_cmdline = g_filename_to_utf8 (cmdline, -1, NULL, NULL, NULL);
...
static gpointer
execute_command_line_async_func (gpointer data)
{
...
gint
execute_command_line_async_wait (const gchar * cmdline)
{
  ...
  thread = g_thread_new ("exec", execute_command_line_async_func, &data);
  if (!thread)
    return -1;
...
gint
execute_open_file (const gchar * file, const gchar * content_type)
{
...
gint
execute_print_file (const gchar * file)
{
  g_return_val_if_fail (file != NULL, -1);
...
gchar *
get_command_output (const gchar * cmdline)
{
  gchar *child_stdout;
  gint status;
...
gint
open_uri (const gchar * uri, const gchar * cmdline)
{
  gchar buf[BUFFSIZE];
...
stime_t
remote_tzoffset_sec (const gchar * zone)
{
  ...
  else if (!strncmp (zone, "UT", 2) || !strncmp (zone, "GMT", 2))
    {
      remoteoffset = 0;
    }
...
gchar *
tzoffset_buf (gchar * buf, stime_t * now)
{
  time_t now_ = *now;
  struct tm gmt, *tmp, *lt;
  gint off;
...
void
get_rfc822_date (gchar * buf, gint len)
{
  struct tm *lt;
  time_t t;
  stime_t t_;
  ...
  g_snprintf (buf, len, "%s, %d %s %d %02d:%02d:%02d %s", day, dd, mon, yyyy, hh, mm, ss, tzoffset_buf (off, &t_));
}

/* just a wrapper to suppress the warning of gcc about %c */
size_t
my_strftime (gchar * s, size_t max, const gchar * format, const struct tm *tm)
{
...
void
set_ui_update_func (UIUpdateFunc func)
{
  ui_update_func = func;
}
...
void
set_event_loop_func (EventLoopFunc func)
{
  event_loop_func = func;
}

void
event_loop_iterate (void)
{
  if (event_loop_func)
    event_loop_func ();
  else
...
void
set_progress_func (ProgressFunc func)
{
  progress_func = func;
}

void
progress_show (gint cur, gint total)
{
  if (progress_func)
...
void
set_input_query_password_func (QueryPasswordFunc func)
{
  query_password_func = func;
}

gchar *
input_query_password (const gchar * server, const gchar * user)
{
...
void
set_log_file (const gchar * filename)
{
  S_LOCK (log_fp);
  if (!log_fp)
    {
...
void
set_log_verbosity (gboolean verbose)
{
  if (verbose)
    log_verbosity_count++;
...
void
set_debug_mode (gboolean enable)
{
  debug_mode = enable;
}

static void
log_dummy_func (const gchar * str)
{
}

static void
log_dummy_flush_func (void)
{
}
...
void
set_log_ui_func (LogFunc print_func, LogFunc message_func, LogFunc warning_func, LogFunc error_func)
{
...
void
set_log_ui_func_full (LogFunc print_func, LogFunc message_func,
                      LogFunc warning_func, LogFunc error_func, LogFlushFunc flush_func)
{
...
void
set_log_show_status_func (LogFunc status_func)
{
  log_show_status_func = status_func;
}

void
debug_print (const gchar * format, ...)
{
  va_list args;
  gchar buf[BUFFSIZE];
...
void
status_print (const gchar * format, ...)
{
  va_list args;
  gchar buf[BUFFSIZE];
...
void
log_write (const gchar * str, const gchar * prefix)
{
  S_LOCK (log_fp);
