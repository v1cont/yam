...
XMLFile *
xml_open_file (const gchar * path)
{
  XMLFile *newfile;
...
static GNode *
xml_build_tree (XMLFile * file, GNode * parent, guint level)
{
  GNode *node = NULL;
  XMLNode *xmlnode;
  XMLTag *tag;
...
GNode *
xml_parse_file (const gchar * path)
{
  XMLFile *file;
  GNode *node;
...
xml_get_dtd (XMLFile * file)
{
  ...
  if ((*bufp++ == '?') &&
      (bufp = strcasestr (bufp, "xml")) && (bufp = strcasestr (bufp + 3, "version")) && (bufp = strchr (bufp + 7, '?')))
    {
...
gint
xml_parse_next_tag (XMLFile * file)
{
  gchar buf[XMLBUFSIZE];
  gchar *bufp = buf;
  gchar *tag_str;
...
void
xml_push_tag (XMLFile * file, XMLTag * tag)
{
...
XMLTag *
xml_get_current_tag (XMLFile * file)
{
  if (file->tag_stack)
...
GList *
xml_get_current_tag_attr (XMLFile * file)
{
  XMLTag *tag;
...
gboolean
xml_compare_tag (XMLFile * file, const gchar * name)
{
  XMLTag *tag;
...
XMLNode *
xml_node_new (XMLTag * tag, const gchar * text)
{
  XMLNode *node;
...
void
xml_tag_add_attr (XMLTag * tag, XMLAttr * attr)
{
  tag->attr = g_list_append (tag->attr, attr);
}

XMLTag *
xml_copy_tag (XMLTag * tag)
{
  XMLTag *new_tag;
  XMLAttr *attr;
  GList *list;
...
XMLAttr *
xml_copy_attr (XMLAttr * attr)
{
  return xml_attr_new (attr->name, attr->value);
}

gint
xml_unescape_str (gchar * str)
{
  gchar *start;
  gchar *end;
  gchar *p = str;
...
gchar *
xml_escape_str (const gchar * str)
{
  GString *estr;
  const gchar *p;
...
gint
xml_file_put_escape_str (FILE * fp, const gchar * str)
{
  const gchar *p;
...
gint
xml_file_put_node (FILE * fp, XMLNode * node)
{
  GList *cur;
...
static gboolean
xml_free_func (GNode * node, gpointer data)
{
...
static gint
xml_get_parenthesis (XMLFile * file, gchar * buf, gint len)
{
  gchar *start;
  gchar *end;
