...
Folder *
folder_new (FolderType type, const gchar * name, const gchar * path)
{
  Folder *folder = NULL;
...
static void
folder_init (Folder * folder, const gchar * name)
{
  FolderItem *item;
...
void
folder_local_folder_init (Folder * folder, const gchar * name, const gchar * path)
{
...
void
folder_remote_folder_init (Folder * folder, const gchar * name, const gchar * path)
{
  folder_init (folder, name);
...
void
folder_local_folder_destroy (LocalFolder * lfolder)
{
...
void
folder_remote_folder_destroy (RemoteFolder * rfolder)
{
...
gint
folder_remote_folder_destroy_all_sessions (void)
{
  GList *list;
  Folder *folder;
      ...
      if (FOLDER_IS_REMOTE (folder))
        {
          ...
          if (rfolder->session && !folder_remote_folder_is_session_active (rfolder))
            {
              session_destroy (rfolder->session);
...
gboolean
folder_remote_folder_is_session_active (RemoteFolder * rfolder)
{
...
gboolean
folder_remote_folder_active_session_exist (void)
{
  GList *list;
  Folder *folder;
  RemoteFolder *rfolder;
...
gint
folder_scan_tree (Folder * folder)
{
...
gint
folder_create_tree (Folder * folder)
{
  g_return_val_if_fail (folder != NULL, -1);
...
FolderItem *
folder_item_new (const gchar * name, const gchar * path)
{
  FolderItem *item;
...
void
folder_item_append (FolderItem * parent, FolderItem * item)
{
...
FolderItem *
folder_item_copy (FolderItem * item)
{
  FolderItem *new_item;
...
static gboolean
folder_item_remove_func (GNode * node, gpointer data)
{
...
void
folder_item_remove (FolderItem * item)
{
  GNode *node;
  ...
  g_node_traverse (node, G_POST_ORDER, G_TRAVERSE_ALL, -1, folder_item_remove_func, NULL);
  g_node_destroy (node);
}

void
folder_item_remove_children (FolderItem * item)
{
  GNode *node, *next;
...
void
folder_item_destroy (FolderItem * item)
{
  Folder *folder;
...
gint
folder_item_compare (FolderItem * item_a, FolderItem * item_b)
{
  gint ret;
  gchar *str_a, *str_b;
...
void
folder_set_ui_func (Folder * folder, FolderUIFunc func, gpointer data)
{
  g_return_if_fail (folder != NULL);
...
void
folder_set_ui_func2 (Folder * folder, FolderUIFunc2 func, gpointer data)
{
  FolderPrivData *priv;
...
FolderUIFunc2
folder_get_ui_func2 (Folder * folder)
{
  FolderPrivData *priv;
...
gboolean
folder_call_ui_func2 (Folder * folder, FolderItem * item, guint count, guint total)
{
  FolderPrivData *priv;
  ...
  if (priv && priv->ui_func2)
    {
      return priv->ui_func2 (folder, item, count, total, priv->ui_func2_data);
    }
...
void
folder_tree_destroy (Folder * folder)
{
  g_return_if_fail (folder != NULL);
  ...
  for (i = 0, cur = folder_list; cur != NULL; cur = cur->next, i++)
    {
      ...
      if (FOLDER_TYPE (folder) == F_MH)
        ...
        {
          if (FOLDER_TYPE (cur_folder) != F_MH && FOLDER_TYPE (cur_folder) != F_IMAP)
            break;
        }
      else if (FOLDER_TYPE (folder) == F_NEWS)
        {
          if (FOLDER_TYPE (cur_folder) != F_MH &&
              FOLDER_TYPE (cur_folder) != F_IMAP && FOLDER_TYPE (cur_folder) != F_NEWS)
            break;
        }
    }
...
folder_read_list (void)
{
  ...
  g_node_traverse (node, G_PRE_ORDER, G_TRAVERSE_ALL, 2, folder_read_folder_func, NULL);
...
folder_write_list (void)
{
  ...
  if (yam_app_get ())
    g_signal_emit_by_name (yam_app_get (), "folderlist-updated");
}
...
static gboolean
folder_get_status_full_all_func (GNode * node, gpointer data)
{
  FolderItem *item;
...
static void
folder_get_status_full_all (GString * str, guint * new, guint * unread, guint * total)
{
  GList *list;
  Folder *folder;
...
gchar *
folder_get_status (GPtrArray * folders, gboolean full)
{
  ...
  if (folders)
    {
      for (i = 0; i < folders->len; i++)
        {
          FolderItem *item = g_ptr_array_index (folders, i);
          new += item->new;
          unread += item->unread;
          total += item->total;
...
Folder *
folder_find_from_path (const gchar * path)
{
  GList *list;
  Folder *folder;
  ...
  for (list = folder_list; list != NULL; list = list->next)
    {
      folder = list->data;
      if (FOLDER_TYPE (folder) == F_MH && !path_cmp (LOCAL_FOLDER (folder)->rootpath, path))
        return folder;
    }
...
Folder *
folder_find_from_name (const gchar * name, FolderType type)
{
  GList *list;
  Folder *folder;
...
static gboolean
folder_item_find_func (GNode * node, gpointer data)
{
...
FolderItem *
folder_find_item_from_path (const gchar * path)
{
  Folder *folder;
  gpointer d[2];
...
FolderItem *
folder_find_child_item_by_name (FolderItem * item, const gchar * name)
{
  GNode *node;
  FolderItem *child;
  gchar *base;
...
static gchar *
folder_get_type_string (FolderType type)
{
  gint i;

  for (i = 0; i < sizeof (type_str_table) / sizeof (type_str_table[0]); i++)
    {
...
static FolderType
folder_get_type_from_string (const gchar * str)
{
  gint i;
...
gchar *
folder_get_identifier (Folder * folder)
{
  gchar *type_str;
...
gchar *
folder_item_get_identifier (FolderItem * item)
{
  gchar *id;
  gchar *folder_id;
...
FolderItem *
folder_find_item_from_identifier (const gchar * identifier)
{
  Folder *folder;
  gpointer d[2];
  gchar *str;
...
FolderItem *
folder_find_item_and_num_from_id (const gchar * identifier, gint * num)
{
  gchar *id;
  gchar *msg;
  FolderItem *item;
...
static FolderPrivData *
folder_get_priv (Folder * folder)
{
  FolderPrivData *priv;
  GList *cur;
  ...
  g_warning ("folder_get_priv: private data for Folder (%p) not found.", folder);

  return NULL;
}

FolderItem *
folder_get_junk (Folder * folder)
{
  FolderPrivData *priv;
...
void
folder_set_junk (Folder * folder, FolderItem * item)
{
  FolderPrivData *priv;
...
gboolean
folder_item_is_trash (FolderItem * item)
{
  PrefsAccount *ac;
  FolderItem *trash;
...
void
folder_set_missing_folders (void)
{
  Folder *folder;
  FolderItem *rootitem;
  FolderItem *item;
...
static gboolean
folder_unref_account_func (GNode * node, gpointer data)
{
  FolderItem *item = node->data;
...
void
folder_unref_account_all (PrefsAccount * account)
{
  Folder *folder;
  GList *list;
...
folder_get_path (Folder * folder)
{
  ...
  if (FOLDER_TYPE (folder) == F_MH)
    {
      path = g_filename_from_utf8 (LOCAL_FOLDER (folder)->rootpath, -1, NULL, NULL, NULL);
      if (!path)
        {
      ...
      if (!g_path_is_absolute (path))
        {
          ...
          path_ = g_strconcat (get_mail_base_dir (), G_DIR_SEPARATOR_S, path, NULL);
          g_free (path);
          path = path_;
        }
    }
  else if (FOLDER_TYPE (folder) == F_IMAP)
    {
      ...
      path = g_strconcat (get_imap_cache_dir (), G_DIR_SEPARATOR_S, server, G_DIR_SEPARATOR_S, uid, NULL);
      g_free (uid);
      g_free (server);
    }
  else if (FOLDER_TYPE (folder) == F_NEWS)
    {
      ...
      path = g_strconcat (get_news_cache_dir (), G_DIR_SEPARATOR_S, server, NULL);
      g_free (server);
    }
  else
    path = NULL;
...
gchar *
folder_item_get_path (FolderItem * item)
{
  gchar *folder_path;
  ...
  if (item->path)
    {
      item_path = g_filename_from_utf8 (item->path, -1, NULL, NULL, NULL);
      if (!item_path)
        {
...
gint
folder_item_scan (FolderItem * item)
{
  Folder *folder;
...
static void
folder_item_scan_foreach_func (gpointer key, gpointer val, gpointer data)
{
  folder_item_scan (FOLDER_ITEM (key));
}

void
folder_item_scan_foreach (GHashTable * table)
{
  g_hash_table_foreach (table, folder_item_scan_foreach_func, NULL);
}

GSList *
folder_item_get_msg_list (FolderItem * item, gboolean use_cache)
{
  Folder *folder;
  ...
  if (item->stype == F_VIRTUAL)
    return virtual_get_class ()->get_msg_list (folder, item, use_cache);

  return folder->klass->get_msg_list (folder, item, use_cache);
}

GSList *
folder_item_get_uncached_msg_list (FolderItem * item)
{
  Folder *folder;

  g_return_val_if_fail (item != NULL, NULL);
  g_return_val_if_fail (item->folder->klass->get_uncached_msg_list != NULL, NULL);
...
gchar *
folder_item_fetch_msg (FolderItem * item, gint num)
{
  Folder *folder;
...
gint
folder_item_fetch_all_msg (FolderItem * item)
{
  Folder *folder;
  GSList *mlist;
  GSList *cur;
  ...
  if (folder->ui_func)
    folder->ui_func (folder, item, folder->ui_func_data ? folder->ui_func_data : GINT_TO_POINTER (num));
...
MsgInfo *
folder_item_get_msginfo (FolderItem * item, gint num)
{
  Folder *folder;
...
gint
folder_item_add_msg (FolderItem * dest, const gchar * file, MsgFlags * flags, gboolean remove_source)
{
  Folder *folder;
...
gint
folder_item_add_msgs (FolderItem * dest, GSList * file_list, gboolean remove_source, gint * first)
{
  Folder *folder;
  ...
  return folder->klass->add_msgs (folder, dest, file_list, remove_source, first);
}

gint
folder_item_add_msg_msginfo (FolderItem * dest, MsgInfo * msginfo, gboolean remove_source)
{
  Folder *folder;
  ...
  return folder->klass->add_msg_msginfo (folder, dest, msginfo, remove_source);
}

gint
folder_item_add_msgs_msginfo (FolderItem * dest, GSList * msglist, gboolean remove_source, gint * first)
{
  Folder *folder;
  ...
  return folder->klass->add_msgs_msginfo (folder, dest, msglist, remove_source, first);
}
...
gint
folder_item_move_msg (FolderItem * dest, MsgInfo * msginfo)
{
  Folder *folder;
...
gint
folder_item_move_msgs (FolderItem * dest, GSList * msglist)
{
  Folder *folder;
  MsgInfo *msginfo;
...
gint
folder_item_copy_msg (FolderItem * dest, MsgInfo * msginfo)
{
  Folder *folder;
...
gint
folder_item_copy_msgs (FolderItem * dest, GSList * msglist)
{
  Folder *folder;
  MsgInfo *msginfo;
...
gint
folder_item_remove_msg (FolderItem * item, MsgInfo * msginfo)
{
  Folder *folder;
...
gint
folder_item_remove_msgs (FolderItem * item, GSList * msglist)
{
  Folder *folder;
  gint ret = 0;
...
gint
folder_item_remove_all_msg (FolderItem * item)
{
  Folder *folder;
...
gboolean
folder_item_is_msg_changed (FolderItem * item, MsgInfo * msginfo)
{
  Folder *folder;

  g_return_val_if_fail (item != NULL, FALSE);
  g_return_val_if_fail (item->folder->klass->is_msg_changed != NULL, FALSE);
...
gint
folder_item_close (FolderItem * item)
{
  Folder *folder;
...
gchar *
folder_item_get_cache_file (FolderItem * item)
{
  gchar *path;
  gchar *file;
...
gchar *
folder_item_get_mark_file (FolderItem * item)
{
  gchar *path;
  gchar *file;
...
static gboolean
folder_build_tree (GNode * node, gpointer data)
{
  ...
  gboolean no_sub = FALSE, no_select = FALSE, collapsed = FALSE, threaded = TRUE, ac_apply_sub = FALSE;
  ...
  gchar *auto_to = NULL, *auto_cc = NULL, *auto_bcc = NULL, *auto_replyto = NULL;
  ...
  for (; list != NULL; list = list->next)
    {
      ...
      if (!strcmp (attr->name, "type"))
        ...
        use_auto_to_on_reply = *attr->value == '1' ? TRUE : FALSE;
        ...
        trim_summary_subject = *attr->value == '1' ? TRUE : FALSE;
      else if (!strcmp (attr->name, "trim_compose_subject"))
        trim_compose_subject = *attr->value = '1' ? TRUE : FALSE;
    }
...
static gboolean
folder_read_folder_func (GNode * node, gpointer data)
{
  Folder *folder;
  FolderItem *item;
  XMLNode *xmlnode;
  ...
  g_node_traverse (node, G_PRE_ORDER, G_TRAVERSE_ALL, -1, folder_build_tree, folder);
...
static void
folder_write_list_recursive (GNode * node, gpointer data)
{
  FILE *fp = (FILE *) data;
  FolderItem *item;
  gint i, depth;
  static gchar *folder_type_str[] = { "mh", "mbox", "maildir", "imap", "news", "unknown" };
  static gchar *folder_item_stype_str[] = { "normal", "inbox", "outbox", "draft", "queue", "trash", "junk", "virtual" };
  static gchar *sort_key_str[] = { "none", "number", "size", "date", "thread-date", "from", "subject", "score",
                                   "label", "mark", "unread", "mime", "to" };
  static gchar *qsearch_cond_str[] = { "all", "unread", "mark", "clabel", "mime", "w1day", "last5", "last7",
  ...
  if (depth == 1)
    {
      ...
      if (FOLDER_TYPE (folder) == F_MH)
        PUT_ESCAPE_STR (fp, "path", LOCAL_FOLDER (folder)->rootpath);
      if (item->collapsed && node->children)
        fputs (" collapsed=\"1\"", fp);
      if (folder->account)
        fprintf (fp, " account_id=\"%d\"", folder->account->account_id);
      else if (item->account)
        fprintf (fp, " account_id=\"%d\"", item->account->account_id);
      if (item->ac_apply_sub)
        fputs (" account_apply_sub=\"1\"", fp);
    }
  else
    {
      fprintf (fp, "<folderitem type=\"%s\"", folder_item_stype_str[item->stype]);
      if (item->name)
      ...
      if (item->sort_key != SORT_BY_NONE)
        {
          fprintf (fp, " sort_key=\"%s\"", sort_key_str[item->sort_key]);
          if (item->sort_type == SORT_ASCENDING)
            fprintf (fp, " sort_type=\"ascending\"");
          else
            fprintf (fp, " sort_type=\"descending\"");
        }
      if (item->qsearch_cond_type > 0 && item->qsearch_cond_type < 10)
        fprintf (fp, " qsearch_cond=\"%s\"", qsearch_cond_str[item->qsearch_cond_type]);

      fprintf (fp, " mtime=\"%lld\" new=\"%d\" unread=\"%d\" total=\"%d\"",
