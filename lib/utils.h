...
typedef void (*ProgressFunc) (gint cur, gint total);
typedef gchar *(*QueryPasswordFunc) (const gchar * server, const gchar * user);
...
typedef gboolean (*StrFindFunc) (const gchar * haystack, const gchar * needle);
...
strconcat_csv (gchar delim, const gchar * field1, ...)
     ...
     gint scan_mailto_url (const gchar * mailto,
                           gchar ** to, gchar ** cc, gchar ** bcc, gchar ** subject, gchar ** inreplyto, gchar ** body);
...
#define is_file_or_fifo_exist(file)	file_exist(file, TRUE)
     ...
     FILE *canonicalize_file_stream (FILE * fp, gint * length);
...
/* time functions */
     ...
     size_t my_strftime (gchar * s, size_t max, const gchar * format, const struct tm *tm);
...
/* logging */
     ...
     void set_log_ui_func (LogFunc print_func, LogFunc message_func, LogFunc warning_func, LogFunc error_func);
