...
static GSList *news_get_article_list (Folder * folder, FolderItem * item, gboolean use_cache);
...
static Session *news_session_new (const gchar * server,
                                  gushort port,
                                  SocksInfo * socks_info, const gchar * userid, const gchar * passwd, SSLType ssl_type);
#else
static Session *news_session_new (const gchar * server,
                                  gushort port, SocksInfo * socks_info, const gchar * userid, const gchar * passwd);
#endif

static gint news_get_article_cmd (NNTPSession * session, const gchar * cmd, gint num, gchar * filename);
...
static gint news_select_group (NNTPSession * session, const gchar * group, gint * num, gint * first, gint * last);
static GSList *news_get_uncached_articles (NNTPSession * session,
                                           FolderItem * item, gint cache_last, gint * rfirst, gint * rlast);
static MsgInfo *news_parse_xover (const gchar * xover_str);
static gchar *news_parse_xhdr (const gchar * xhdr_str, MsgInfo * msginfo);
static GSList *news_delete_old_articles (GSList * alist, FolderItem * item, gint first);
...
static Folder *
news_folder_new (const gchar * name, const gchar * path)
{
  Folder *folder;
...
static void
news_folder_destroy (Folder * folder)
{
  if (REMOTE_FOLDER (folder)->remove_cache_on_destroy)
    {
      ...
      dir = g_strconcat (get_news_cache_dir (), G_DIR_SEPARATOR_S, server, NULL);
      if (is_dir_exist (dir))
        g_rmdir (dir);
      g_free (dir);
      g_free (server);
...
static void
news_folder_init (Folder * folder, const gchar * name, const gchar * path)
{
...
#if USE_SSL
static Session *
news_session_new (const gchar * server, gushort port,
                  SocksInfo * socks_info, const gchar * userid, const gchar * passwd, SSLType ssl_type)
#else
static Session *
news_session_new (const gchar * server, gushort port,
                  SocksInfo * socks_info, const gchar * userid, const gchar * passwd)
#endif
{
...
static Session *
news_session_new_for_folder (Folder * folder)
{
  Session *session;
  PrefsAccount *ac;
  ...
  if (ac->use_socks && ac->use_socks_for_recv && ac->proxy_host)
    {
      socks_info =
        socks_info_new (ac->socks_type, ac->proxy_host, ac->proxy_port, ac->use_proxy_auth ? ac->proxy_name : NULL,
                        ac->use_proxy_auth ? ac->proxy_pass : NULL);
    }

#if USE_SSL
  port = ac->set_nntpport ? ac->nntpport : ac->ssl_nntp ? NNTPS_PORT : NNTP_PORT;
  session = news_session_new (ac->nntp_server, port, socks_info, userid, passwd, ac->ssl_nntp);
#else
  port = ac->set_nntpport ? ac->nntpport : NNTP_PORT;
  session = news_session_new (ac->nntp_server, port, socks_info, userid, passwd);
#endif
...
static NNTPSession *
news_session_get (Folder * folder)
{
  ...
  if (nntp_mode (NNTP_SESSION (rfolder->session), FALSE) != NN_SUCCESS)
    {
      log_warning (_("NNTP connection to %s:%d has been"
                     " disconnected. Reconnecting...\n"),
                   folder->account->nntp_server, folder->account->set_nntpport ? folder->account->nntpport : NNTP_PORT);
...
static GSList *
news_get_article_list (Folder * folder, FolderItem * item, gboolean use_cache)
{
  GSList *alist;
  NNTPSession *session;
  ...
  if (!session)
  ...
  else if (use_cache)
    {
      ...
      newlist = news_get_uncached_articles (session, item, cache_last, &first, &last);
      if (newlist)
  ...
  else
    {
      ...
      alist = news_get_uncached_articles (session, item, 0, NULL, &last);
      news_delete_all_articles (item);
      item->last_num = last;
      item->cache_dirty = TRUE;
    }
...
static gchar *
news_fetch_msg (Folder * folder, FolderItem * item, gint num)
{
  gchar *path, *filename;
  NNTPSession *session;
  gchar nstr[16];
  ...
  if (!is_dir_exist (path))
    make_dir_hier (path);
  filename = g_strconcat (path, G_DIR_SEPARATOR_S, utos_buf (nstr, num), NULL);
  g_free (path);
  ...
  ok = news_get_article (NNTP_SESSION (REMOTE_FOLDER (folder)->session), num, filename);
  if (ok != NN_SUCCESS)
    {
...
static MsgInfo *
news_get_msginfo (Folder * folder, FolderItem * item, gint num)
{
  MsgInfo *msginfo;
  MsgFlags flags = { 0, 0 };
  gchar *file;
...
static gint
news_close (Folder * folder, FolderItem * item)
{
  return 0;
}

static gint
news_scan_group (Folder * folder, FolderItem * item)
{
  NNTPSession *session;
...
static NewsGroupInfo *
news_group_info_new (const gchar * name, gint first, gint last, gchar type)
{
  NewsGroupInfo *ginfo;
...
static void
news_group_info_free (NewsGroupInfo * ginfo)
{
  g_free (ginfo->name);
  g_free (ginfo);
}
...
static gint
news_group_info_compare (NewsGroupInfo * ginfo1, NewsGroupInfo * ginfo2)
{
  return g_ascii_strcasecmp (ginfo1->name, ginfo2->name);
}

GSList *
news_get_group_list (Folder * folder)
{
  gchar *path, *filename;
  FILE *fp;
  GSList *list = NULL;
...
void
news_group_list_free (GSList * group_list)
{
  GSList *cur;
...
void
news_remove_group_list_cache (Folder * folder)
{
  gchar *path, *filename;
...
gint
news_post (Folder * folder, const gchar * file)
{
  FILE *fp;
  gint ok;
...
gint
news_post_stream (Folder * folder, FILE * fp)
{
  NNTPSession *session;
  gint ok;
...
static gint
news_get_article_cmd (NNTPSession * session, const gchar * cmd, gint num, gchar * filename)
{
  gchar *msgid;
  gint ok;
...
static gint
news_get_article (NNTPSession * session, gint num, gchar * filename)
{
...
#if 0
static gint
news_get_header (NNTPSession * session, gint num, gchar * filename)
{
 ...
 **/
static gint
news_select_group (NNTPSession * session, const gchar * group, gint * num, gint * first, gint * last)
{
  gint ok;
...
static GSList *
news_get_uncached_articles (NNTPSession * session, FolderItem * item, gint cache_last, gint * rfirst, gint * rlast)
{
  gint ok;
...
static MsgInfo *
news_parse_xover (const gchar * xover_str)
{
  MsgInfo *msginfo;
...
static gchar *
news_parse_xhdr (const gchar * xhdr_str, MsgInfo * msginfo)
{
  gchar *p;
  gchar *tmp;
  gint num;
...
static GSList *
news_delete_old_articles (GSList * alist, FolderItem * item, gint first)
{
  GSList *cur, *next;
  MsgInfo *msginfo;
  gchar *dir;
...
static void
news_delete_all_articles (FolderItem * item)
{
  gchar *dir;
...
static void
news_delete_expired_caches (GSList * alist, FolderItem * item)
{
  gchar *dir;
