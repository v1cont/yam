...
gchar *
unmime_header (const gchar * encoded_str)
{
      ...
      if (encoding == 'B')
        {
          ...
          len = base64_decode ((guchar *) decoded_text, text_begin_p + 1, eword_end_p - (text_begin_p + 1));
          decoded_text[len] = '\0';
        }
      else if (encoding == 'Q')
        {
          decoded_text = g_malloc (eword_end_p - (text_begin_p + 1) + 1);
          len = qp_decode_q_encoding ((guchar *) decoded_text, text_begin_p + 1, eword_end_p - (text_begin_p + 1));
        }
      else
        {
