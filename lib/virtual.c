...
static void virtual_folder_init (Folder * folder, const gchar * name, const gchar * path);

static GHashTable *virtual_read_search_cache (FolderItem * item);
static void virtual_write_search_cache (FILE * fp, FolderItem * item, MsgInfo * msginfo, gint matched);
...
static GSList *virtual_get_msg_list (Folder * folder, FolderItem * item, gboolean use_cache);
...
static gint virtual_rename_folder (Folder * folder, FolderItem * item, const gchar * name);
static gint virtual_move_folder (Folder * folder, FolderItem * item, FolderItem * new_parent);
...
FolderClass *
virtual_get_class (void)
{
  return &virtual_class;
}

static Folder *
virtual_folder_new (const gchar * name, const gchar * path)
{
  Folder *folder;
...
static void
virtual_folder_destroy (Folder * folder)
{
  folder_local_folder_destroy (LOCAL_FOLDER (folder));
}

static void
virtual_folder_init (Folder * folder, const gchar * name, const gchar * path)
{
...
guint
sinfo_hash (gconstpointer key)
{
  const SearchCacheInfo *sinfo = key;
  gulong h;

  h = (gulong) sinfo->folder;
  h ^= sinfo->msgnum;
  h ^= (gulong) sinfo->size;
  h ^= (gulong) sinfo->mtime;
  /* h ^= (guint)sinfo->flags.tmp_flags; */
  h ^= (gulong) sinfo->flags.perm_flags;
...
gint
sinfo_equal (gconstpointer v, gconstpointer v2)
{
  const SearchCacheInfo *s1 = v;
  const SearchCacheInfo *s2 = v2;

  return (s1->folder == s2->folder && s1->msgnum == s2->msgnum && s1->size == s2->size && s1->mtime == s2->mtime &&
...
static GHashTable *
virtual_read_search_cache (FolderItem * item)
{
  GHashTable *table;
  gchar *path, *file;
  FILE *fp;
  ...
  fp = procmsg_open_data_file (file, SEARCH_CACHE_VERSION, DATA_READ, NULL, 0);
  g_free (file);
  g_free (path);
  if (!fp)
    return NULL;
          ...
          if (folder)
            {
              ...
              sinfo->mtime = mtime;
              sinfo->flags = flags;
              g_hash_table_insert (table, sinfo, GINT_TO_POINTER (matched));
              ++count;
            }
        }
    }
...
static void
virtual_write_search_cache (FILE * fp, FolderItem * item, MsgInfo * msginfo, gint matched)
{
  if (!item && !msginfo)
    {
  ...
  if (msginfo)
    {
      ...
      WRITE_CACHE_DATA_INT ((msginfo->flags.tmp_flags & MSG_CACHED_FLAG_MASK), fp);
...
static void
search_cache_free_func (gpointer key, gpointer value, gpointer data)
{
  g_free (key);
}

static void
virtual_search_cache_free (GHashTable * table)
{
  if (table)
    {
...
static GSList *
virtual_search_folder (VirtualSearchInfo * info, FolderItem * item)
{
  ...
  for (cur = mlist; cur != NULL; cur = cur->next)
    {
      ...
      if (tv_cur.tv_sec > tv_prev.tv_sec || tv_cur.tv_nsec - tv_prev.tv_nsec > PROGRESS_UPDATE_INTERVAL * 1000)
        {
          status_print (_("Searching %s (%d / %d)..."), item->path, count, total);
          tv_prev = tv_cur;
        }
      ++count;
      ...
      if (info->search_cache_table)
        {
          ...
          matched = (gulong) g_hash_table_lookup (info->search_cache_table, &sinfo);
          if (matched == SCACHE_MATCHED)
            {
              match_list = g_slist_prepend (match_list, msginfo);
              cur->data = NULL;
              virtual_write_search_cache (info->fp, NULL, msginfo, matched);
              ++ncachehit;
              continue;
            }
      ...
      if (filter_match_rule (info->rule, msginfo, hlist, &fltinfo))
        {
          match_list = g_slist_prepend (match_list, msginfo);
          cur->data = NULL;
          virtual_write_search_cache (info->fp, NULL, msginfo, SCACHE_MATCHED);
        }
      else
        {
          virtual_write_search_cache (info->fp, NULL, msginfo, SCACHE_NOT_MATCHED);
        }
...
static gboolean
virtual_search_recursive_func (GNode * node, gpointer data)
{
...
static GSList *
virtual_get_msg_list (Folder * folder, FolderItem * item, gboolean use_cache)
{
  GSList *mlist = NULL;
  GSList *flist;
  GSList *cur;
  ...
  info.fp = procmsg_open_data_file (cache_file, SEARCH_CACHE_VERSION, DATA_WRITE, NULL, 0);
  g_free (cache_file);
  g_free (path);
  if (!info.fp)
    goto finish;

  info.requires_full_headers = filter_rule_requires_full_headers (rule);
  ...
  if (rule->recursive)
    {
      g_node_traverse (target->node, G_PRE_ORDER, G_TRAVERSE_ALL, -1, virtual_search_recursive_func, &info);
      mlist = info.mlist;
    }
  else
...
static gchar *
virtual_fetch_msg (Folder * folder, FolderItem * item, gint num)
{
  return NULL;
}

static MsgInfo *
virtual_get_msginfo (Folder * folder, FolderItem * item, gint num)
{
  return NULL;
}

static gint
virtual_close (Folder * folder, FolderItem * item)
{
  return 0;
}

static gint
virtual_scan_folder (Folder * folder, FolderItem * item)
{
  return 0;
}

static gint
virtual_rename_folder (Folder * folder, FolderItem * item, const gchar * name)
{
...
static gint
virtual_move_folder (Folder * folder, FolderItem * item, FolderItem * new_parent)
{
  g_return_val_if_fail (item != NULL, -1);
  g_return_val_if_fail (item->stype == F_VIRTUAL, -1);
...
static gint
virtual_remove_folder (Folder * folder, FolderItem * item)
{
  gchar *path;
