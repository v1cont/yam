...
void
qp_encode_line (gchar * out, const guchar * in)
{
  const guchar *inp = in;
  gchar *outp = out;
  guchar ch;
  ...
  while (*inp != '\0')
    {
      if (*inp == '=')
        {
          if (inp[1] && inp[2] && get_hex_value ((guchar *) outp, inp[1], inp[2]) == TRUE)
            {
              inp += 3;
            }
...
gint
qp_decode_q_encoding (guchar * out, const gchar * in, gint inlen)
{
  const gchar *inp = in;
  guchar *outp = out;
...
gint
qp_get_q_encoding_len (const guchar * str)
{
...
void
qp_q_encode (gchar * out, const guchar * in)
{
  const guchar *inp = in;
  gchar *outp = out;
...
static gboolean
get_hex_value (guchar * out, gchar c1, gchar c2)
{
  gint hi, lo;
