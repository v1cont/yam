...
void
custom_header_read_config (PrefsAccount * ac)
{
  gchar *rcpath;
  FILE *fp;
...
void
custom_header_write_config (PrefsAccount * ac)
{
  gchar *rcpath;
  PrefFile *pfile;
  GSList *cur;
  ...
  for (cur = all_hdrs; cur != NULL; cur = cur->next)
    {
      ...
      if (fputs (chstr, pfile->fp) == EOF || fputc ('\n', pfile->fp) == EOF)
        {
...
gchar *
custom_header_get_str (CustomHeader * ch)
{
  return g_strdup_printf ("%i:%s: %s", ch->account_id, ch->name, ch->value ? ch->value : "");
}

CustomHeader *
custom_header_read_str (const gchar * buf)
{
  CustomHeader *ch;
...
CustomHeader *
custom_header_find (GSList * header_list, const gchar * header)
{
  GSList *cur;
  CustomHeader *chdr;
...
void
custom_header_free (CustomHeader * ch)
{
  if (!ch)
    return;
