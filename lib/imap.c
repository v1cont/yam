...
typedef gint (*IMAPThreadFunc) (IMAPSession * session, gpointer data);
typedef gint (*IMAPProgressFunc) (IMAPSession * session, gint count, gint total, gpointer data);
...
static gint imap_fetch_flags (IMAPSession * session, GArray ** uids, GHashTable ** flags_table);

static GSList *imap_get_msg_list (Folder * folder, FolderItem * item, gboolean use_cache);
...
static gint imap_add_msg (Folder * folder,
                          FolderItem * dest, const gchar * file, MsgFlags * flags, gboolean remove_source);
static gint imap_add_msgs (Folder * folder,
                           FolderItem * dest, GSList * file_list, gboolean remove_source, gint * first);
static gint imap_add_msg_msginfo (Folder * folder, FolderItem * dest, MsgInfo * msginfo, gboolean remove_source);
static gint imap_add_msgs_msginfo (Folder * folder,
                                   FolderItem * dest, GSList * msglist, gboolean remove_source, gint * first);
...
static gint imap_move_folder (Folder * folder, FolderItem * item, FolderItem * new_parent);
...
static gint imap_scan_tree_recursive (IMAPSession * session, FolderItem * item, GSList * item_list);
static GSList *imap_get_folder_list (IMAPSession * session, FolderItem * item);
static GSList *imap_parse_list (IMAPSession * session, const gchar * real_path, gchar * separator);
...
static gint imap_do_copy_msgs (Folder * folder, FolderItem * dest, GSList * msglist, gboolean remove_source);
static gint imap_remove_msgs_by_seq_set (Folder * folder, FolderItem * item, GSList * seq_list);

static GSList *imap_get_uncached_messages (IMAPSession * session,
                                           FolderItem * item,
                                           guint32 first_uid, guint32 last_uid, gint exists, gboolean update_count);
static void imap_delete_cached_message (FolderItem * item, guint32 uid);
static GSList *imap_delete_cached_messages (GSList * mlist, FolderItem * item, guint32 first_uid, guint32 last_uid);
static void imap_delete_all_cached_messages (FolderItem * item);

#if USE_SSL
static SockInfo *imap_open (const gchar * server, gushort port, SocksInfo * socks_info, SSLType ssl_type);
#else
static SockInfo *imap_open (const gchar * server, gushort port, SocksInfo * socks_info);
#endif

static gint imap_msg_list_change_perm_flags (GSList * msglist, MsgPermFlags flags, gboolean is_set);
static gchar *imap_get_flag_str (IMAPFlags flags);
static gint imap_set_message_flags (IMAPSession * session, const gchar * seq_set, IMAPFlags flags, gboolean is_set);
static gint imap_select (IMAPSession * session,
                         IMAPFolder * folder,
                         const gchar * path, gint * exists, gint * recent, gint * unseen, guint32 * uid_validity);
static gint imap_status (IMAPSession * session,
                         IMAPFolder * folder,
                         const gchar * path,
                         gint * messages, gint * recent, guint32 * uid_next, guint32 * uid_validity, gint * unseen);
...
static IMAPNameSpace *imap_find_namespace (IMAPFolder * folder, const gchar * path);
...
static gchar *imap_parse_atom (IMAPSession * session, gchar * src, gchar * dest, gint dest_len, GString * str);
static MsgFlags imap_parse_flags (const gchar * flag_str);
static IMAPFlags imap_parse_imap_flags (const gchar * flag_str);
static MsgInfo *imap_parse_envelope (IMAPSession * session, FolderItem * item, GString * line_str);

static gboolean imap_has_capability (IMAPSession * session, const gchar * capability);
static void imap_capability_free (IMAPSession * session);

/* low-level IMAP4rev1 commands */
static gint imap_cmd_capability (IMAPSession * session);
static gint imap_cmd_authenticate (IMAPSession * session, const gchar * user, const gchar * pass, IMAPAuthType type);
static gint imap_cmd_login (IMAPSession * session, const gchar * user, const gchar * pass);
...
static gint imap_cmd_list (IMAPSession * session, const gchar * ref, const gchar * mailbox, GPtrArray * argbuf);
static gint imap_cmd_do_select (IMAPSession * session,
                                const gchar * folder,
                                gboolean examine, gint * exists, gint * recent, gint * unseen, guint32 * uid_validity);
static gint imap_cmd_select (IMAPSession * session,
                             const gchar * folder, gint * exists, gint * recent, gint * unseen, guint32 * uid_validity);
static gint imap_cmd_examine (IMAPSession * session,
                              const gchar * folder,
                              gint * exists, gint * recent, gint * unseen, guint32 * uid_validity);
static gint imap_cmd_create (IMAPSession * session, const gchar * folder);
static gint imap_cmd_rename (IMAPSession * session, const gchar * oldfolder, const gchar * newfolder);
...
static gint imap_cmd_envelope (IMAPSession * session, const gchar * seq_set);
...
static gint imap_cmd_append (IMAPSession * session,
                             const gchar * destfolder, const gchar * file, IMAPFlags flags, guint32 * new_uid);
static gint imap_cmd_copy (IMAPSession * session, const gchar * seq_set, const gchar * destfolder);
static gint imap_cmd_store (IMAPSession * session, const gchar * seq_set, const gchar * sub_cmd);
...
static gint imap_thread_run (IMAPSession * session, IMAPThreadFunc func, gpointer data);
static gint imap_thread_run_progress (IMAPSession * session,
                                      IMAPThreadFunc func, IMAPProgressFunc progress_func, gpointer data);
...
static Folder *
imap_folder_new (const gchar * name, const gchar * path)
{
  Folder *folder;
...
static void
imap_folder_destroy (Folder * folder)
{
  ...
  if (REMOTE_FOLDER (folder)->remove_cache_on_destroy)
    {
      ...
      dir = g_strconcat (get_imap_cache_dir (), G_DIR_SEPARATOR_S, server, NULL);
      if (is_dir_exist (dir))
        g_rmdir (dir);
      g_free (dir);
      g_free (server);
...
static void
imap_folder_init (Folder * folder, const gchar * name, const gchar * path)
{
...
static IMAPSession *
imap_session_get (Folder * folder)
{
  ...
  if (!rfolder->session)
    {
      ...
      if (rfolder->session)
        imap_parse_namespace (IMAP_SESSION (rfolder->session), IMAP_FOLDER (folder));
  ...
  if ((ret = imap_cmd_noop (IMAP_SESSION (rfolder->session))) != IMAP_SUCCESS)
    {
      ...
      if (imap_session_reconnect (IMAP_SESSION (rfolder->session)) == IMAP_SUCCESS)
        imap_parse_namespace (IMAP_SESSION (rfolder->session), IMAP_FOLDER (folder));
      else
        {
...
static gint
imap_greeting (IMAPSession * session)
{
  gchar *greeting;
  gint ok;
...
static gint
imap_auth (IMAPSession * session, const gchar * user, const gchar * pass, IMAPAuthType type)
{
  gboolean nologin;
  ...
  switch (type)
    ...
    case 0:
      if (imap_has_capability (session, "AUTH=CRAM-MD5"))
        ok = imap_cmd_authenticate (session, user, pass, IMAP_AUTH_CRAM_MD5);
      else if (imap_has_capability (session, "AUTH=PLAIN"))
        ok = imap_cmd_authenticate (session, user, pass, IMAP_AUTH_PLAIN);
      else if (nologin)
...
static Session *
imap_session_new (PrefsAccount * account)
{
  IMAPSession *session;
  gushort port;
...
static gint
imap_session_connect (IMAPSession * session)
{
  SockInfo *sock;
  ...
  if (!pass)
    {
      ...
      tmp_pass = input_query_password (account->recv_server, account->userid);
      if (!tmp_pass)
        return IMAP_ERROR;
  ...
  if (account->use_socks && account->use_socks_for_recv && account->proxy_host)
    {
      socks_info =
        socks_info_new (account->socks_type, account->proxy_host, account->proxy_port,
                        account->use_proxy_auth ? account->proxy_name : NULL,
                        account->use_proxy_auth ? account->proxy_pass : NULL);
    }
...
#else
  if ((sock = imap_open (SESSION (session)->server, SESSION (session)->port, socks_info)) == NULL)
#endif
    return IMAP_ERROR;
  ...
  if (!session->authenticated && imap_auth (session, account->userid, pass, account->imap_auth_type) != IMAP_SUCCESS)
    {
      if (account->tmp_pass)
        {
...
static gint
imap_session_reconnect (IMAPSession * session)
{
  g_return_val_if_fail (session != NULL, IMAP_ERROR);
...
static void
imap_session_destroy (Session * session)
{
...
static gint
imap_fetch_flags (IMAPSession * session, GArray ** uids, GHashTable ** flags_table)
{
  gint ok;
  gchar *tmp;
  gchar *cur_pos;
      ...
      if (uid > 0)
        {
          ...
          g_hash_table_insert (*flags_table, GUINT_TO_POINTER (uid), GINT_TO_POINTER (flags));
        }
...
static GSList *
imap_get_msg_list_full (Folder * folder, FolderItem * item, gboolean use_cache, gboolean uncached_only)
{
  GSList *mlist = NULL;
  IMAPSession *session;
  ...
  ok = imap_select (session, IMAP_FOLDER (folder), item->path, &exists, &recent, &unseen, &uid_validity);
  if (ok != IMAP_SUCCESS)
    THROW;
  ...
  if (use_cache)
    {
      ...
      for (cur = mlist; cur != NULL; cur = next)
        {
          ...
          next = cur->next;
          imap_flags = GPOINTER_TO_INT (g_hash_table_lookup (flags_table, GUINT_TO_POINTER (msginfo->msgnum)));

          if (imap_flags == 0)
            {
              debug_print ("imap_get_msg_list: " "message %u has been deleted.\n", msginfo->msgnum);
              imap_delete_cached_message (item, msginfo->msgnum);
          ...
          for (i = 0; i < uids->len; i++)
            {
              ...
              if (g_hash_table_lookup (msg_table, GUINT_TO_POINTER (uid)) == NULL)
                {
                  debug_print ("imap_get_msg_list: " "first new UID: %u\n", uid);
                  begin = uid;
                  break;
                }
            }
      ...
      if (first_uid > 0 && last_uid > 0)
        {
          mlist = imap_delete_cached_messages (mlist, item, 0, first_uid - 1);
          mlist = imap_delete_cached_messages (mlist, item, begin > 0 ? begin : last_uid + 1, UINT_MAX);
        }

      if (begin > 0 && begin <= last_uid)
        {
          newlist = imap_get_uncached_messages (session, item, begin, last_uid, exists - item->total, TRUE);
          if (newlist)
            {
              item->cache_dirty = TRUE;
              item->mark_dirty = TRUE;
    ...
    {
      imap_delete_all_cached_messages (item);
      mlist = imap_get_uncached_messages (session, item, 0, 0, exists, TRUE);
...
static GSList *
imap_get_msg_list (Folder * folder, FolderItem * item, gboolean use_cache)
{
  return imap_get_msg_list_full (folder, item, use_cache, FALSE);
}

static GSList *
imap_get_uncached_msg_list (Folder * folder, FolderItem * item)
{
  return imap_get_msg_list_full (folder, item, TRUE, TRUE);
}

static gchar *
imap_fetch_msg (Folder * folder, FolderItem * item, gint uid)
{
  gchar *path, *filename;
  IMAPSession *session;
  gchar nstr[16];
  ...
  ok = imap_select (session, IMAP_FOLDER (folder), item->path, NULL, NULL, NULL, NULL);
  if (ok != IMAP_SUCCESS)
    {
...
static MsgInfo *
imap_get_msginfo (Folder * folder, FolderItem * item, gint uid)
{
  IMAPSession *session;
  GSList *list;
...
static gint
imap_add_msg (Folder * folder, FolderItem * dest, const gchar * file, MsgFlags * flags, gboolean remove_source)
{
  GSList file_list;
...
static gint
imap_add_msgs (Folder * folder, FolderItem * dest, GSList * file_list, gboolean remove_source, gint * first)
{
  gchar *destdir;
  IMAPSession *session;
  ...
  gint total;
  gint ok;
  struct timespec tv_prev, tv_cur;
  ...
  clock_gettime (CLOCK_MONOTONIC, &tv_prev);
  ui_update ();

  ok = imap_status (session, IMAP_FOLDER (folder), dest->path, &messages, &recent, &uid_next, &uid_validity, &unseen);
  if (ok != IMAP_SUCCESS)
    {
  ...
  for (cur = file_list; cur != NULL; cur = cur->next)
    {
      ...
      if (dest->stype == F_OUTBOX || dest->stype == F_QUEUE || dest->stype == F_DRAFT)
        iflags |= IMAP_FLAG_SEEN;

      clock_gettime (CLOCK_MONOTONIC, &tv_cur);
      if (tv_cur.tv_sec > tv_prev.tv_sec || tv_cur.tv_nsec - tv_prev.tv_nsec > PROGRESS_UPDATE_INTERVAL * 1000)
        {
          status_print (_("Appending messages to %s (%d / %d)"), dest->path, count, total);
      ...
      ok = imap_cmd_append (session, destdir, fileinfo->file, iflags, &new_uid);
      ...
      if (yam_app_get ())
        g_signal_emit_by_name (yam_app_get (), "add-msg", dest, fileinfo->file, new_uid);
...
static gint
imap_add_msg_msginfo (Folder * folder, FolderItem * dest, MsgInfo * msginfo, gboolean remove_source)
{
  GSList msglist;
  ...
  return imap_add_msgs_msginfo (folder, dest, &msglist, remove_source, NULL);
}

static gint
imap_add_msgs_msginfo (Folder * folder, FolderItem * dest, GSList * msglist, gboolean remove_source, gint * first)
{
  GSList *file_list;
  gint ret;
...
static gint
imap_do_copy_msgs (Folder * folder, FolderItem * dest, GSList * msglist, gboolean remove_source)
{
  FolderItem *src;
  gchar *destdir;
  GSList *seq_list, *cur;
  ...
  ok = imap_select (session, IMAP_FOLDER (folder), src->path, NULL, NULL, NULL, NULL);
  if (ok != IMAP_SUCCESS)
    return ok;
  ...
  for (cur = seq_list; cur != NULL; cur = cur->next)
    {
      ...
      if (remove_source)
        {
          status_print (_("Moving messages %s to %s ..."), seq_set, dest->path);
          debug_print ("Moving message %s/[%s] to %s ...\n", src->path, seq_set, dest->path);
        }
      else
        {
          status_print (_("Copying messages %s to %s ..."), seq_set, dest->path);
  ...
  for (cur = msglist; cur != NULL; cur = cur->next)
    {
      ...
      if (yam_app_get ())
        g_signal_emit_by_name (yam_app_get (), "add-msg", dest, NULL, 0);
...
static gint
imap_move_msg (Folder * folder, FolderItem * dest, MsgInfo * msginfo)
{
  GSList msglist;
...
static gint
imap_move_msgs (Folder * folder, FolderItem * dest, GSList * msglist)
{
  MsgInfo *msginfo;
  GSList *file_list;
  gint ret = 0;
...
static gint
imap_copy_msg (Folder * folder, FolderItem * dest, MsgInfo * msginfo)
{
  GSList msglist;
...
static gint
imap_copy_msgs (Folder * folder, FolderItem * dest, GSList * msglist)
{
  MsgInfo *msginfo;
  GSList *file_list;
  gint ret;
...
static gint
imap_remove_msgs_by_seq_set (Folder * folder, FolderItem * item, GSList * seq_list)
{
  gint ok;
  IMAPSession *session;
  GSList *cur;
  ...
  for (cur = seq_list; cur != NULL; cur = cur->next)
    {
      ...
      ok = imap_set_message_flags (session, seq_set, IMAP_FLAG_DELETED, TRUE);
      if (ok != IMAP_SUCCESS)
        {
...
static gint
imap_remove_msg (Folder * folder, FolderItem * item, MsgInfo * msginfo)
{
  GSList msglist;
...
static gint
imap_remove_msgs (Folder * folder, FolderItem * item, GSList * msglist)
{
  gint ok;
  IMAPSession *session;
  GSList *seq_list, *cur;
  ...
  for (cur = msglist; cur != NULL; cur = cur->next)
    {
      ...
      if (yam_app_get ())
        g_signal_emit_by_name (yam_app_get (), "remove-msg", item, NULL, uid);
...
static gint
imap_remove_all_msg (Folder * folder, FolderItem * item)
{
  gint ok;
  IMAPSession *session;
  gchar *dir;
  ...
  if (yam_app_get ())
    g_signal_emit_by_name (yam_app_get (), "remove-all-msg", item);
...
static gboolean
imap_is_msg_changed (Folder * folder, FolderItem * item, MsgInfo * msginfo)
{
...
static gint
imap_close (Folder * folder, FolderItem * item)
{
  gint ok;
  IMAPSession *session;
  ...
  if (session->mbox)
    {
      if (strcmp2 (session->mbox, item->path) != 0)
        return -1;
...
static gint
imap_scan_folder (Folder * folder, FolderItem * item)
{
  IMAPSession *session;
  gint messages, recent, unseen;
  guint32 uid_next, uid_validity;
  ...
  ok = imap_status (session, IMAP_FOLDER (folder), item->path, &messages, &recent, &uid_next, &uid_validity, &unseen);
  if (ok != IMAP_SUCCESS)
    return -1;
...
imap_scan_tree (Folder * folder)
{
  ...
  if (folder->account->imap_dir && *folder->account->imap_dir)
    {
      ...
      subst_char (root_folder, imap_get_path_separator (IMAP_FOLDER (folder), root_folder), '/');
      strtailchomp (root_folder, '/');
      real_path = imap_get_real_path (IMAP_FOLDER (folder), root_folder);
      debug_print ("IMAP root directory: %s\n", real_path);

      /* check if root directory exist */
      argbuf = g_ptr_array_new ();
      ok = imap_cmd_list (session, NULL, real_path, argbuf);
      if (ok != IMAP_SUCCESS || search_array_str (argbuf, "LIST ") == NULL)
        {
  ...
  if (!item || ((item->path || root_folder) && strcmp2 (item->path, root_folder) != 0))
    {
      folder_tree_destroy (folder);
...
static gint
imap_scan_tree_recursive (IMAPSession * session, FolderItem * item, GSList * item_list)
{
  Folder *folder;
  FolderItem *new_item;
      ...
      if (!new_item)
      ...
      else if (old_item->stype == F_VIRTUAL)
        {
          debug_print ("IMAP4 folder found at the location of virtual folder '%s'. removing virtual folder...\n",
                       old_item->path);
          virtual_get_class ()->remove_folder (folder, old_item);
        }
      else
        {
          ...
          if (old_item->no_select == TRUE)
            old_item->new = old_item->unread = old_item->total = 0;
  ...
  for (cur = part_list; cur != NULL; cur = cur->next)
    {
      ...
      for (node = item->node->children; node != NULL; node = node->next)
        {
          if (!strcmp2 (FOLDER_ITEM (node->data)->path, cur_item->path))
            {
      ...
      if (!g_ascii_strcasecmp (new_item->path, "INBOX"))
        ...
        {
          gchar *base = g_path_get_basename (new_item->path);
...
static GSList *
imap_get_folder_list (IMAPSession * session, FolderItem * item)
{
  Folder *folder;
...
static GSList *
imap_parse_list (IMAPSession * session, const gchar * real_path, gchar * separator)
{
  gchar buf[IMAPBUFSIZE];
  gchar flags[256];
  ...
  for (;;)
    {
      ...
      if (buf[0] != '*' || buf[1] != ' ')
        {
          log_print ("IMAP4< %s\n", buf);
          if (sscanf (buf, "%*d %16s", buf) < 1 || strcmp (buf, "OK") != 0)
            log_warning (_("error occurred while getting LIST.\n"));
      ...
      if (buf[0] == '\0')
        continue;
      if (!strcmp (buf, real_path))
        continue;

      if (separator_str[0] != '\0')
        subst_char (buf, separator_str[0], '/');
      name = g_path_get_basename (buf);
      if (name[0] == '.')
        {
          g_free (name);
          continue;
      ...
      if (g_ascii_strcasecmp (buf, "INBOX") != 0 && strcasestr (flags, "\\Noselect") != NULL)
...
static GSList *
imap_add_inter_folders (GSList * item_list, const gchar * root_path)
{
  FolderItem *item;
  GSList *cur;
  ...
  for (cur = item_list; cur != NULL; cur = cur->next)
    {
      ...
      if (root_path_len > 0 && strncmp (root_path, item->path, root_path_len) != 0)
        continue;
      p = item->path + root_path_len;
      if (root_path_len > 0 && *p != '/')
        continue;
      while (*p == '/')
        p++;
      if (*p == '\0')
      ...
      while ((q = strchr (q, '/')) != NULL)
        {
          ...
          if (!g_hash_table_lookup (exist, parent))
            {
              if (root_path_len > 0)
                full_parent = g_strconcat (root_path, "/", parent, NULL);
              else
                full_parent = g_strdup (parent);
              base = g_path_get_basename (parent);
              new_item = folder_item_new (base, full_parent);
              g_free (base);
              new_item->no_select = TRUE;
              ...
              while (*r == '/')
                r++;
              g_hash_table_insert (exist, (gpointer) r, GINT_TO_POINTER (1));
...
static GSList *
imap_get_part_folder_list (GSList * item_list, FolderItem * item)
{
  FolderItem *cur_item;
  ...
  for (cur = item_list; cur != NULL; cur = cur->next)
    {
      ...
      if (!strncmp (cur_item->path, item->path, len) &&
          cur_item->path[len] == '/' && !strchr (cur_item->path + len + 1, '/'))
        {
          part_list = g_slist_prepend (part_list, cur_item);
          debug_print ("append '%s'\n", cur_item->path);
        }
...
static gint
imap_create_tree (Folder * folder)
{
  g_return_val_if_fail (folder != NULL, -1);
...
static void
imap_create_missing_folders (Folder * folder)
{
...
static FolderItem *
imap_create_special_folder (Folder * folder, SpecialFolderItemType stype, const gchar * name)
{
  FolderItem *item;
  FolderItem *new_item;
...
static FolderItem *
imap_create_folder (Folder * folder, FolderItem * parent, const gchar * name)
{
...
static gint
imap_rename_folder_real (Folder * folder, FolderItem * item, FolderItem * new_parent, const gchar * name)
{
  gchar *newpath;
  gchar *real_oldpath;
  gchar *real_newpath;
  ...
  g_free (session->mbox);
  session->mbox = NULL;
  ok = imap_cmd_examine (session, "INBOX", &exists, &recent, &unseen, &uid_validity);
  if (ok != IMAP_SUCCESS)
    {
      g_free (real_oldpath);
      return -1;
  ...
  if (new_parent)
    {
      if (name)
        {
          if (new_parent->path)
            newpath = g_strconcat (new_parent->path, "/", name, NULL);
          else
      ...
      else
        {
          ...
          if (new_parent->path)
            newpath = g_strconcat (new_parent->path, "/", name_, NULL);
          else
  ...
  if (yam_app_get ())
    g_signal_emit_by_name (yam_app_get (), "move-folder", item, old_id, new_id);
  g_free (new_id);
  g_free (old_id);
...
static gint
imap_rename_folder (Folder * folder, FolderItem * item, const gchar * name)
{
  return imap_rename_folder_real (folder, item, NULL, name);
}

static gint
imap_move_folder (Folder * folder, FolderItem * item, FolderItem * new_parent)
{
  return imap_rename_folder_real (folder, item, new_parent, NULL);
}

static gint
imap_remove_folder (Folder * folder, FolderItem * item)
{
  gint ok;
  IMAPSession *session;
  gchar *path;
  ...
  if (yam_app_get ())
    g_signal_emit_by_name (yam_app_get (), "remove-folder", item);
...
static gint
imap_get_uncached_messages_progress_func (IMAPSession * session, gint count, gint total, gpointer data)
{
...
static gint
imap_get_uncached_messages_func (IMAPSession * session, gpointer data)
{
  gchar *tmp;
  GSList *newlist = NULL;
  GSList *llast = NULL;
...
static GSList *
imap_get_uncached_messages (IMAPSession * session,
                            FolderItem * item, guint32 first_uid, guint32 last_uid, gint exists, gboolean update_count)
{
  ...
  if (first_uid == 0 && last_uid == 0)
    ...
    g_snprintf (seq_set, sizeof (seq_set), "%u:%u", first_uid, last_uid);
  ...
  imap_thread_run_progress (session, imap_get_uncached_messages_func,
                            imap_get_uncached_messages_progress_func, &get_data);
...
static void
imap_delete_cached_message (FolderItem * item, guint32 uid)
{
  gchar *dir;
  gchar *file;
...
static GSList *
imap_delete_cached_messages (GSList * mlist, FolderItem * item, guint32 first_uid, guint32 last_uid)
{
  GSList *cur, *next;
  MsgInfo *msginfo;
  gchar *dir;
  ...
  debug_print ("Deleting cached messages %u - %u ... ", first_uid, last_uid);
  ...
  for (cur = mlist; cur != NULL;)
    {
      ...
      msginfo = (MsgInfo *) cur->data;
      if (msginfo != NULL && first_uid <= msginfo->msgnum && msginfo->msgnum <= last_uid)
        {
          procmsg_msginfo_free (msginfo);
          mlist = g_slist_remove (mlist, msginfo);
        }
...
#if USE_SSL
static SockInfo *
imap_open (const gchar * server, gushort port, SocksInfo * socks_info, SSLType ssl_type)
#else
static SockInfo *
imap_open (const gchar * server, gushort port, SocksInfo * socks_info)
#endif
{
  SockInfo *sock = NULL;
  const gchar *server_;
  ...
  if ((conn_id = sock_connect_async_thread (server_, port_)) < 0 || sock_connect_async_thread_wait (conn_id, &sock) < 0)
    {
...
static GList *
imap_parse_namespace_str (gchar * str)
{
  gchar *p = str;
  gchar *name;
  gchar *separator;
...
static void
imap_parse_namespace (IMAPSession * session, IMAPFolder * folder)
{
  gchar *ns_str = NULL;
  gchar **str_array;
  ...
  if (folder->ns_personal != NULL || folder->ns_others != NULL || folder->ns_shared != NULL)
    return;
...
static void
imap_get_namespace_by_list (IMAPSession * session, IMAPFolder * folder)
{
  GSList *item_list, *cur;
  gchar separator = '\0';
  IMAPNameSpace *namespace;
...
imap_find_namespace_from_list (GList * ns_list, const gchar * path)
{
...
static IMAPNameSpace *
imap_find_namespace (IMAPFolder * folder, const gchar * path)
{
  IMAPNameSpace *namespace;
...
static gchar
imap_get_path_separator (IMAPFolder * folder, const gchar * path)
{
  IMAPNameSpace *namespace;
  gchar separator = '/';
...
static gchar *
imap_get_real_path (IMAPFolder * folder, const gchar * path)
{
  gchar *real_path;
  gchar separator;
...
static gchar *
imap_parse_atom (IMAPSession * session, gchar * src, gchar * dest, gint dest_len, GString * str)
{
  gchar *cur_pos = src;
  gchar *nextline;
...
static gchar *
imap_get_header (IMAPSession * session, gchar * cur_pos, gchar ** headers, GString * str)
{
  gchar *nextline;
  gchar buf[IMAPBUFSIZE];
  gint len;
...
static MsgFlags
imap_parse_flags (const gchar * flag_str)
{
  ...
  while (*p != '\0')
    {
      if (g_ascii_strncasecmp (p, "\\Recent", 7) == 0 && MSG_IS_UNREAD (flags))
        {
      ...
      else if (g_ascii_strncasecmp (p, "$label", 6) == 0)
        {
          ...
          if (*(p + 6) >= '1' && *(p + 6) <= '7')
            {
              guint color = *(p + 6) - '1' + 1;
              MSG_UNSET_PERM_FLAGS (flags, MSG_CLABEL_FLAG_MASK);
...
static IMAPFlags
imap_parse_imap_flags (const gchar * flag_str)
{
  const gchar *p = flag_str;
  IMAPFlags flags = 0;
...
static MsgInfo *
imap_parse_envelope (IMAPSession * session, FolderItem * item, GString * line_str)
{
  gchar buf[IMAPBUFSIZE];
  MsgInfo *msginfo = NULL;
  gchar *cur_pos;
  ...
  g_return_val_if_fail (line_str->str[0] == '*' && line_str->str[1] == ' ', NULL);
  ...
  while (*cur_pos != '\0' && *cur_pos != ')')
    {
      ...
      if (!strncmp (cur_pos, "UID ", 4))
      ...
      else if (!strncmp (cur_pos, "RFC822.HEADER", 13))
        {
          ...
          cur_pos += 13;
          cur_pos = imap_get_header (session, cur_pos, &headers, line_str);
          if (cur_pos == NULL)
            {
...
static gint
imap_msg_list_change_perm_flags (GSList * msglist, MsgPermFlags flags, gboolean is_set)
{
  Folder *folder;
  IMAPSession *session;
  IMAPFlags iflags = 0;
  ...
  ok = imap_select (session, IMAP_FOLDER (folder), msginfo->folder->path, NULL, NULL, NULL, NULL);
  if (ok != IMAP_SUCCESS)
    return ok;
  ...
  for (cur = seq_list; cur != NULL; cur = cur->next)
    {
      ...
      if (iflags)
        {
          ok = imap_set_message_flags (session, seq_set, iflags, is_set);
          if (ok != IMAP_SUCCESS)
            break;
        }
      ...
      if (flags & MSG_UNREAD)
        {
          ok = imap_set_message_flags (session, seq_set, IMAP_FLAG_SEEN, !is_set);
          if (ok != IMAP_SUCCESS)
            break;
        }
    }
...
gint
imap_msg_set_perm_flags (MsgInfo * msginfo, MsgPermFlags flags)
{
  GSList msglist;
...
gint
imap_msg_unset_perm_flags (MsgInfo * msginfo, MsgPermFlags flags)
{
  GSList msglist;
...
gint
imap_msg_list_set_perm_flags (GSList * msglist, MsgPermFlags flags)
{
  return imap_msg_list_change_perm_flags (msglist, flags, TRUE);
}

gint
imap_msg_list_unset_perm_flags (GSList * msglist, MsgPermFlags flags)
{
  return imap_msg_list_change_perm_flags (msglist, flags, FALSE);
}

gint
imap_msg_list_set_colorlabel_flags (GSList * msglist, guint color)
{
  Folder *folder;
  IMAPSession *session;
  IMAPFlags iflags = 0;
  ...
  for (cur = seq_list; cur != NULL; cur = cur->next)
    {
      ...
      ok = imap_cmd_store (session, seq_set, "-FLAGS.SILENT ($label1 $label2 $label3 $label4 $label5 $label6 $label7)");
      if (ok != IMAP_SUCCESS)
        break;

      if (iflags)
        {
          ok = imap_set_message_flags (session, seq_set, iflags, TRUE);
          if (ok != IMAP_SUCCESS)
            break;
        }
    }
...
static gchar *
imap_get_flag_str (IMAPFlags flags)
{
  GString *str;
  gchar *ret;
  guint color;
  ...
  if (IMAP_IS_SEEN (flags))
    g_string_append (str, "\\Seen ");
  if (IMAP_IS_ANSWERED (flags))
    g_string_append (str, "\\Answered ");
  if (IMAP_IS_FLAGGED (flags))
    g_string_append (str, "\\Flagged ");
  if (IMAP_IS_DELETED (flags))
    g_string_append (str, "\\Deleted ");
  if (IMAP_IS_DRAFT (flags))
    g_string_append (str, "\\Draft ");
...
static gint
imap_set_message_flags (IMAPSession * session, const gchar * seq_set, IMAPFlags flags, gboolean is_set)
{
  gchar *cmd;
  gchar *flag_str;
  gint ok;
  ...
  cmd = g_strconcat (is_set ? "+FLAGS.SILENT (" : "-FLAGS.SILENT (", flag_str, ")", NULL);
  g_free (flag_str);
...
static gint
imap_select (IMAPSession * session, IMAPFolder * folder,
             const gchar * path, gint * exists, gint * recent, gint * unseen, guint32 * uid_validity)
{
  gchar *real_path;
  gint ok;
  ...
  ok = imap_cmd_select (session, real_path, exists, recent, unseen, uid_validity);
  if (ok != IMAP_SUCCESS)
...
static gint
imap_status (IMAPSession * session, IMAPFolder * folder,
             const gchar * path,
             gint * messages, gint * recent, guint32 * uid_next, guint32 * uid_validity, gint * unseen)
{
  gchar *real_path;
  ...
  ok = imap_cmd_gen_send (session, "STATUS %s " "(MESSAGES RECENT UIDNEXT UIDVALIDITY UNSEEN)", real_path_);
  if (ok != IMAP_SUCCESS)
    {
...
static gboolean
imap_has_capability (IMAPSession * session, const gchar * capability)
{
  gchar **p;
...
static void
imap_capability_free (IMAPSession * session)
{
...
static gint
imap_cmd_capability (IMAPSession * session)
{
  gint ok;
  GPtrArray *argbuf;
  gchar *capability;
  ...
  if ((ok = imap_cmd_gen_send (session, "CAPABILITY")) != IMAP_SUCCESS)
    THROW (ok);
  if ((ok = imap_cmd_ok (session, argbuf)) != IMAP_SUCCESS)
    THROW (ok);
...
static gint
imap_cmd_auth_plain (IMAPSession * session, const gchar * user, const gchar * pass)
{
  gchar *p;
  gchar *response;
  gchar *response64;
...
static gint
imap_cmd_auth_cram_md5 (IMAPSession * session, const gchar * user, const gchar * pass, const gchar * challenge64)
{
  gchar *challenge;
  gint challenge_len;
  gchar hexdigest[33];
  ...
  md5_hex_hmac (hexdigest, (guchar *) challenge, challenge_len, (guchar *) pass, strlen (pass));
  g_free (challenge);
...
static gint
imap_cmd_authenticate (IMAPSession * session, const gchar * user, const gchar * pass, IMAPAuthType type)
{
  gchar *auth_type;
  gint ok;
  gchar *buf = NULL;

  g_return_val_if_fail ((type == 0 || type == IMAP_AUTH_CRAM_MD5 || type == IMAP_AUTH_PLAIN), IMAP_ERROR);
...
static gint
imap_cmd_login (IMAPSession * session, const gchar * user, const gchar * pass)
{
  gchar *user_, *pass_;
  gint ok;
...
static gint
imap_cmd_logout (IMAPSession * session)
{
...
static gint
imap_cmd_noop (IMAPSession * session)
{
  gint ret;
...
#if USE_SSL
static gint
imap_cmd_starttls (IMAPSession * session)
{
...
static gint
imap_cmd_namespace (IMAPSession * session, gchar ** ns_str)
{
  gint ok;
  GPtrArray *argbuf;
  gchar *str;
...
static gint
imap_cmd_list (IMAPSession * session, const gchar * ref, const gchar * mailbox, GPtrArray * argbuf)
{
  gchar *ref_, *mailbox_;
...
static gint
imap_cmd_do_select (IMAPSession * session, const gchar * folder,
                    gboolean examine, gint * exists, gint * recent, gint * unseen, guint32 * uid_validity)
{
  gint ok;
  gchar *resp_str;
  ...
  *exists = *recent = *unseen = *uid_validity = 0;
  ...
  if (resp_str)
    {
      if (sscanf (resp_str, "OK [UIDVALIDITY %u] ", &uid_validity_) != 1)
        {
...
static gint
imap_cmd_select (IMAPSession * session, const gchar * folder,
                 gint * exists, gint * recent, gint * unseen, guint32 * uid_validity)
{
  return imap_cmd_do_select (session, folder, FALSE, exists, recent, unseen, uid_validity);
}

static gint
imap_cmd_examine (IMAPSession * session, const gchar * folder,
                  gint * exists, gint * recent, gint * unseen, guint32 * uid_validity)
{
  return imap_cmd_do_select (session, folder, TRUE, exists, recent, unseen, uid_validity);
}

#undef THROW

static gint
imap_cmd_create (IMAPSession * session, const gchar * folder)
{
  gchar *folder_;
...
static gint
imap_cmd_rename (IMAPSession * session, const gchar * old_folder, const gchar * new_folder)
{
...
static gint
imap_cmd_delete (IMAPSession * session, const gchar * folder)
{
  gchar *folder_;
...
static gint
imap_cmd_subscribe (IMAPSession * session, const gchar * folder)
{
  gchar *folder_;
...
static gint
imap_cmd_fetch_func (IMAPSession * session, gpointer data)
{
...
#undef RETURN_ERROR_IF_FAIL
  ...
  if ((ret = recv_bytes_write_to_file (SESSION (session)->sock, size_num, filename)) != 0)
    {
      if (ret == -2)
        THROW (IMAP_SOCKET);
    }
...
static gint
imap_cmd_fetch (IMAPSession * session, guint32 uid, const gchar * filename)
{
  gint ok;
...
static void
imap_get_date_time (gchar * buf, size_t len, stime_t timer)
{
  ...
  if (lt && lt->tm_mon >= 0 && lt->tm_mon < 12)
    {
      strftime (date_time, sizeof (date_time), "%d-___-%Y %H:%M:%S", lt);
...
static gint
imap_cmd_append (IMAPSession * session, const gchar * destfolder,
                 const gchar * file, IMAPFlags flags, guint32 * new_uid)
{
  ...
  flag_str = imap_get_flag_str (flags);
  if (date_time[0])
    ok = imap_cmd_gen_send (session, "APPEND %s (%s) \"%s\" {%d}", destfolder_, flag_str, date_time, size);
  else
    ok = imap_cmd_gen_send (session, "APPEND %s (%s) {%d}", destfolder_, flag_str, size);
  g_free (flag_str);
  if (ok != IMAP_SUCCESS)
    {
  ...
  if (new_uid != NULL && session->uidplus)
    {
      ...
      if (ok != IMAP_SUCCESS)
      ...
      else if (argbuf->len > 0)
        {
          ...
          if (resp_str && sscanf (resp_str, "%*u OK [APPENDUID %*u %u]", &new_uid_) == 1)
            {
              *new_uid = new_uid_;
            }
        }
...
static gint
imap_cmd_copy (IMAPSession * session, const gchar * seq_set, const gchar * destfolder)
{
  gint ok;
  gchar *destfolder_;
...
gint
imap_cmd_envelope (IMAPSession * session, const gchar * seq_set)
{
  return imap_cmd_gen_send (session, "UID FETCH %s (UID FLAGS RFC822.SIZE RFC822.HEADER)", seq_set);
}

static gint
imap_cmd_store (IMAPSession * session, const gchar * seq_set, const gchar * sub_cmd)
{
  gint ok;
...
static gint
imap_cmd_expunge (IMAPSession * session)
{
  gint ok;
...
static gint
imap_cmd_close (IMAPSession * session)
{
  gint ok;
...
static gint
imap_cmd_ok_real (IMAPSession * session, GPtrArray * argbuf)
{
  gint ok;
  gchar *buf;
  gint cmd_num;
  ...
  while ((ok = imap_cmd_gen_recv (session, &buf)) == IMAP_SUCCESS)
    {
      ...
      else if (sscanf (str->str, "%d %" Xstr (IMAPBUFSIZE) "s", &cmd_num, cmd_status) < 2)
        ok = IMAP_ERROR;
      else if (cmd_num == session->cmd_count && !strcmp (cmd_status, "OK"))
        {
          if (argbuf)
...
static gint
imap_cmd_ok_func (IMAPSession * session, gpointer data)
{
...
static gint
imap_cmd_ok (IMAPSession * session, GPtrArray * argbuf)
{
  return imap_thread_run (session, imap_cmd_ok_func, argbuf);
}

static gint
imap_cmd_gen_send (IMAPSession * session, const gchar * format, ...)
{
  IMAPRealSession *real = (IMAPRealSession *) session;
  gchar buf[IMAPBUFSIZE];
...
static gint
imap_cmd_gen_recv (IMAPSession * session, gchar ** ret)
{
  gint len;
...
static gint
imap_cmd_gen_recv_silent (IMAPSession * session, gchar ** ret)
{
  gint len;
...
static gchar *
strchr_cpy (const gchar * src, gchar ch, gchar * dest, gint len)
{
  gchar *tmp;
...
static gchar *
get_quoted (const gchar * src, gchar ch, gchar * dest, gint len)
{
  const gchar *p = src;
  gint n = 0;
...
static gchar *
search_array_contain_str (GPtrArray * array, gchar * str)
{
  gint i;
...
static gchar *
search_array_str (GPtrArray * array, gchar * str)
{
  gint i;
  gint len;
...
static void
imap_path_separator_subst (gchar * str, gchar separator)
{
  gchar *p;
...
static gchar *
imap_modified_utf7_to_utf8 (const gchar * mutf7_str)
{
  ...
  if (iconv (cd, (gchar **) &norm_utf7_p, &norm_utf7_len, &to_p, &to_len) == -1)
    {
...
static gchar *
imap_utf8_to_modified_utf7 (const gchar * from)
{
  static iconv_t cd = (iconv_t) - 1;
  static gboolean iconv_ok = TRUE;
  ...
  while (from_len > 0)
    {
      if (*from_tmp == '+')
      ...
      else
        {
          ...
          from_len -= conv_len;
          if (iconv (cd, (gchar **) &from_tmp, &conv_len, &norm_utf7_p, &norm_utf7_len) == -1)
            {
...
static GSList *
imap_get_seq_set_from_msglist (GSList * msglist, gint limit)
{
  GString *str;
  ...
  sorted_list = procmsg_sort_msg_list (sorted_list, SORT_BY_NUMBER, SORT_ASCENDING);
...
static gint
imap_seq_set_get_count (const gchar * seq_set)
{
  gint count = 0;
  guint first, last;
  gchar *tmp, *p, *q;
...
static void
imap_seq_set_free (GSList * seq_list)
{
...
static gboolean
imap_rename_folder_func (GNode * node, gpointer data)
{
...
static void
imap_thread_run_proxy (gpointer push_data, gpointer data)
{
...
static gint
imap_thread_run (IMAPSession * session, IMAPThreadFunc func, gpointer data)
{
  IMAPRealSession *real = (IMAPRealSession *) session;
  gint ret;
  ...
  if (!real->pool)
    {
      real->pool = g_thread_pool_new (imap_thread_run_proxy, real, -1, FALSE, NULL);
      if (!real->pool)
        return IMAP_ERROR;
    }
...
static gint
imap_thread_run_progress (IMAPSession * session, IMAPThreadFunc func, IMAPProgressFunc progress_func, gpointer data)
{
  IMAPRealSession *real = (IMAPRealSession *) session;
  gint prev_count = 0;
  gint ret;
  ...
  while (g_atomic_int_get (&real->flag) == 0)
    {
      ...
      if (prev_count != real->prog_count && real->prog_total > 0)
        {
          progress_func (session, real->prog_count, real->prog_total, data);
...
gboolean
imap_is_session_active (IMAPFolder * folder)
{
