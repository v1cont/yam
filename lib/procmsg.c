...
static GMappedFile *procmsg_open_cache_file_mmap (FolderItem * item, DataOpenMode mode);

static gint procmsg_cmp_by_mark (gconstpointer a, gconstpointer b);
static gint procmsg_cmp_by_unread (gconstpointer a, gconstpointer b);
static gint procmsg_cmp_by_mime (gconstpointer a, gconstpointer b);
static gint procmsg_cmp_by_label (gconstpointer a, gconstpointer b);
static gint procmsg_cmp_by_number (gconstpointer a, gconstpointer b);
static gint procmsg_cmp_by_size (gconstpointer a, gconstpointer b);
static gint procmsg_cmp_by_date (gconstpointer a, gconstpointer b);
static gint procmsg_cmp_by_from (gconstpointer a, gconstpointer b);
static gint procmsg_cmp_by_to (gconstpointer a, gconstpointer b);
static gint procmsg_cmp_by_subject (gconstpointer a, gconstpointer b);


GHashTable *
procmsg_msg_hash_table_create (GSList * mlist)
{
...
void
procmsg_msg_hash_table_append (GHashTable * msg_table, GSList * mlist)
{
  GSList *cur;
  MsgInfo *msginfo;

  if (msg_table == NULL || mlist == NULL)
    return;
  ...
  for (cur = mlist; cur != NULL; cur = cur->next)
    {
      ...
      g_hash_table_insert (msg_table, GUINT_TO_POINTER (msginfo->msgnum), msginfo);
    }
}

GHashTable *
procmsg_to_folder_hash_table_create (GSList * mlist)
{
  GHashTable *msg_table;
  GSList *cur;
  MsgInfo *msginfo;
...
gint
procmsg_read_cache_data_str (FILE * fp, gchar ** str)
{
  gchar buf[BUFFSIZE];
  guint32 len;
  gchar *tmp = NULL;
...
static gint
procmsg_read_cache_data_str_mem (const gchar ** p, const gchar * endp, gchar ** str)
{
  guint32 len;
...
GSList *
procmsg_read_cache (FolderItem * item, gboolean scan_file)
{
  GSList *mlist = NULL;
  GSList *last = NULL;
  GMappedFile *mapfile;
  ...
  while (endp - p >= sizeof (num))
    {
      ...
      /* if the message file doesn't exist or is changed,
         don't add the data */
      if ((type == F_MH && scan_file && folder_item_is_msg_changed (item, msginfo)) || msginfo->msgnum == 0)
        {
          procmsg_msginfo_free (msginfo);
          item->cache_dirty = TRUE;
        }
...
static GSList *
procmsg_read_cache_queue (FolderItem * item, gboolean scan_file)
{
  FolderType type;
  MsgInfo *msginfo;
  MsgFlags default_flags;
  ...
  for (cur = item->cache_queue; cur != NULL; cur = cur->next)
    {
      ...
      if ((type == F_MH && scan_file && folder_item_is_msg_changed (item, msginfo)))
        {
          procmsg_msginfo_free (msginfo);
          item->cache_dirty = TRUE;
        }
...
static void
mark_unset_new_func (gpointer key, gpointer value, gpointer data)
{
  MSG_UNSET_PERM_FLAGS (*((MsgFlags *) value), MSG_NEW);
}

void
procmsg_set_flags (GSList * mlist, FolderItem * item)
{
  GSList *cur;
  ...
  if (!mark_queue_exist)
    {
      for (cur = mlist; cur != NULL; cur = cur->next)
        {
          msginfo = (MsgInfo *) cur->data;
          flags = g_hash_table_lookup (mark_table, GUINT_TO_POINTER (msginfo->msgnum));
          if (!flags)
            {
...
static void
mark_all_read_func (gpointer key, gpointer value, gpointer data)
{
  MSG_UNSET_PERM_FLAGS (*((MsgFlags *) value), MSG_NEW | MSG_UNREAD);
}

void
procmsg_mark_all_read (FolderItem * item)
{
  GHashTable *mark_table;
      ...
      for (cur = item->mark_queue; cur != NULL; cur = cur->next)
        {
          ...
          MSG_UNSET_PERM_FLAGS (flaginfo->flags, MSG_NEW | MSG_UNREAD);
        }
      item->mark_dirty = TRUE;
    }
...
GSList *
procmsg_sort_msg_list (GSList * mlist, FolderSortKey sort_key, FolderSortType sort_type)
{
  ...
  switch (sort_key)
    {
    case SORT_BY_MARK:
      cmp_func = procmsg_cmp_by_mark;
      break;
    case SORT_BY_UNREAD:
      cmp_func = procmsg_cmp_by_unread;
      break;
    case SORT_BY_MIME:
      cmp_func = procmsg_cmp_by_mime;
      break;
    case SORT_BY_LABEL:
      cmp_func = procmsg_cmp_by_label;
      break;
    case SORT_BY_NUMBER:
      cmp_func = procmsg_cmp_by_number;
      break;
    case SORT_BY_SIZE:
      cmp_func = procmsg_cmp_by_size;
      break;
    case SORT_BY_DATE:
      cmp_func = procmsg_cmp_by_date;
      break;
    case SORT_BY_FROM:
      cmp_func = procmsg_cmp_by_from;
      break;
    case SORT_BY_SUBJECT:
      cmp_func = procmsg_cmp_by_subject;
      break;
    case SORT_BY_TO:
      cmp_func = procmsg_cmp_by_to;
      break;
    default:
      return mlist;
    }
...
gint
procmsg_get_last_num_in_msg_list (GSList * mlist)
{
  GSList *cur;
  MsgInfo *msginfo;
  gint last = 0;
...
void
procmsg_msg_list_free (GSList * mlist)
{
  GSList *cur;
  MsgInfo *msginfo;
...
void
procmsg_write_cache (MsgInfo * msginfo, FILE * fp)
{
...
void
procmsg_write_flags (MsgInfo * msginfo, FILE * fp)
{
...
void
procmsg_write_cache_list (FolderItem * item, GSList * mlist)
{
  FILE *fp;
  GSList *cur;
...
void
procmsg_write_flags_list (FolderItem * item, GSList * mlist)
{
  FILE *fp;
  GSList *cur;
...
static gint
cmp_by_item (gconstpointer a, gconstpointer b)
{
...
void
procmsg_write_flags_for_multiple_folders (GSList * mlist)
{
  GSList *tmp_list, *cur;
...
void
procmsg_flush_mark_queue (FolderItem * item, FILE * fp)
{
  MsgFlagInfo *flaginfo;
  MsgInfo msginfo = { 0 };
...
void
procmsg_add_mark_queue (FolderItem * item, gint num, MsgFlags flags)
{
  MsgFlagInfo *flaginfo;
...
void
procmsg_flaginfo_list_free (GSList * flaglist)
{
  GSList *cur;
  MsgFlagInfo *flaginfo;
...
void
procmsg_flush_cache_queue (FolderItem * item, FILE * fp)
{
  MsgInfo *msginfo;
  gboolean append = FALSE;
  GSList *qlist, *cur;
...
void
procmsg_add_cache_queue (FolderItem * item, gint num, MsgInfo * msginfo)
{
...
gboolean
procmsg_flush_folder (FolderItem * item)
{
  ...
  if (item->mark_queue && !item->opened)
    flushed = TRUE;
  procmsg_get_mark_sum (item, &n_new, &n_unread, &n_total, &n_min, &n_max, 0);
  item->unmarked_num = 0;
  item->new = n_new;
...
static void
procmsg_flush_folder_foreach_func (gpointer key, gpointer val, gpointer data)
{
  procmsg_flush_folder (FOLDER_ITEM (key));
}

void
procmsg_flush_folder_foreach (GHashTable * folder_table)
{
  g_hash_table_foreach (folder_table, procmsg_flush_folder_foreach_func, NULL);
}

void
procmsg_add_flags (FolderItem * item, gint num, MsgFlags flags)
{
  FILE *fp;
  MsgInfo msginfo;
...
static void
mark_sum_func (gpointer key, gpointer value, gpointer data)
{
...
void
procmsg_get_mark_sum (FolderItem * item, gint * new, gint * unread, gint * total, gint * min, gint * max, gint first)
{
  GHashTable *mark_table;
...
static GHashTable *
procmsg_read_mark_file (FolderItem * item)
{
  FILE *fp;
  ...
  for (cur = item->mark_queue; cur != NULL; cur = cur->next)
    {
      ...
      flags = g_hash_table_lookup (mark_table, GUINT_TO_POINTER (flaginfo->msgnum));
      if (flags != NULL)
        g_free (flags);
      ...
      g_hash_table_insert (mark_table, GUINT_TO_POINTER (flaginfo->msgnum), flags);
...
static void
write_mark_func (gpointer key, gpointer value, gpointer data)
{
  MsgInfo msginfo;
...
static void
procmsg_write_mark_file (FolderItem * item, GHashTable * mark_table)
{
  FILE *fp;
...
FILE *
procmsg_open_data_file (const gchar * file, guint version, DataOpenMode mode, gchar * buf, size_t buf_size)
{
  FILE *fp;
  guint32 data_ver = 0;
...
static GMappedFile *
procmsg_open_cache_file_mmap (FolderItem * item, DataOpenMode mode)
{
  gchar *cachefile;
...
FILE *
procmsg_open_cache_file (FolderItem * item, DataOpenMode mode)
{
  gchar *cachefile;
  FILE *fp;
...
FILE *
procmsg_open_mark_file (FolderItem * item, DataOpenMode mode)
{
  gchar *markfile;
  FILE *fp;
...
void
procmsg_clear_cache (FolderItem * item)
{
  FILE *fp;
...
void
procmsg_clear_mark (FolderItem * item)
{
  FILE *fp;
...
GNode *
procmsg_get_thread_tree (GSList * mlist)
{
  ...
  for (; mlist != NULL; mlist = mlist->next)
    {
      ...
      if ((msgid = msginfo->msgid) && g_hash_table_lookup (table, msgid) == NULL)
      ...
      if (!parent && msginfo->references)
        {
          for (reflist = msginfo->references; reflist != NULL; reflist = reflist->next)
            if ((parent = g_hash_table_lookup (table, reflist->data)) != NULL)
              break;
        }
...
static gboolean
procmsg_thread_date_func (GNode * node, gpointer data)
{
...
guint
procmsg_get_thread_date (GNode * node)
{
  guint tdate = 0;

  g_return_val_if_fail (node != NULL && node->parent != NULL && node->parent->parent == NULL, 0);

  g_node_traverse (node, G_PRE_ORDER, G_TRAVERSE_ALL, -1, procmsg_thread_date_func, &tdate);
...
gint
procmsg_copy_messages (GSList * mlist)
{
...
gint
procmsg_add_messages_from_queue (FolderItem * dest, GSList * mlist, gboolean is_move)
{
  MsgInfo *msginfo;
  GSList *cur;
  gchar *file;
...
gchar *
procmsg_get_message_file_path (MsgInfo * msginfo)
{
  gchar *path, *file;
  ...
  if (msginfo->encinfo && msginfo->encinfo->plaintext_file)
  ...
  else
    {
      ...
      file = g_strconcat (path, G_DIR_SEPARATOR_S, utos_buf (nstr, msginfo->msgnum), NULL);
      g_free (path);
    }
...
gchar *
procmsg_get_message_file (MsgInfo * msginfo)
{
  gchar *filename = NULL;
...
GSList *
procmsg_get_message_file_list (GSList * mlist)
{
...
void
procmsg_message_file_list_free (GSList * file_list)
{
  GSList *cur;
  MsgFileInfo *fileinfo;
...
FILE *
procmsg_open_message (MsgInfo * msginfo)
{
  FILE *fp;
  gchar *file;
...
void
procmsg_set_decrypt_message_func (DecryptMessageFunc func)
{
  decrypt_message_func = func;
}

void
procmsg_set_auto_decrypt_message (gboolean enabled)
{
  auto_decrypt = enabled;
}

FILE *
procmsg_open_message_decrypted (MsgInfo * msginfo, MimeInfo ** mimeinfo)
{
  FILE *fp;
...
gboolean
procmsg_msg_exist (MsgInfo * msginfo)
{
  gchar *path;
  gboolean ret;
...
gboolean
procmsg_trash_messages_exist (void)
{
  FolderItem *trash;
  GList *cur;
...
void
procmsg_empty_trash (FolderItem * trash)
{
  if (!trash)
    return;
...
static gboolean
remove_all_cached_messages_func (GNode * node, gpointer data)
{
  FolderItem *item;
  gchar *dir;
...
void
procmsg_remove_all_cached_messages (Folder * folder)
{
  ...
  g_node_traverse (folder->node, G_PRE_ORDER, G_TRAVERSE_ALL, -1, remove_all_cached_messages_func, NULL);
}

gint
procmsg_save_to_outbox (FolderItem * outbox, const gchar * file)
{
  gint num;
  MsgFlags flag = { 0, 0 };
...
static gint
print_command_exec (const gchar * file, const gchar * cmdline)
{
...
static void
procmsg_write_headers (MsgInfo * msginfo, MimeInfo * partinfo,
                       FILE * fp, FILE * dest_fp, const gchar * encoding, gboolean all_headers)
{
  GPtrArray *headers;
  gint i;
  ...
  for (i = 0; i < headers->len; i++)
    {
      ...
      if (partinfo)
        {
          if (!g_ascii_strcasecmp (hdr->name, "Subject") ||
              ...
              !g_ascii_strcasecmp (hdr->name, "To") || !g_ascii_strcasecmp (hdr->name, "Cc"))
            {
      ...
      if (body && *body != '\0')
        {
          file_str = conv_codeset_strdup (body, CS_INTERNAL, encoding);
          fprintf (dest_fp, "%s: %s\n", hdr->name, file_str ? file_str : body);
          g_free (file_str);
        }
      else
        {
...
void
procmsg_print_message (MsgInfo * msginfo, const gchar * cmdline, gboolean all_headers)
{
  gchar *prtmp;

  g_return_if_fail (msginfo != NULL);

  prtmp = g_strdup_printf ("%s%cprinttmp-%08x.txt", get_mime_tmp_dir (), G_DIR_SEPARATOR, print_id++);

  if (procmsg_save_message_as_text (msginfo, prtmp, conv_get_locale_charset_str (), all_headers) == 0)
...
void
procmsg_print_message_part (MsgInfo * msginfo, MimeInfo * partinfo, const gchar * cmdline, gboolean all_headers)
{
  ...
  if ((tmpfp = procmime_get_text_content (partinfo, msgfp, conv_get_locale_charset_str ())) == NULL)
    {
      fclose (msgfp);
      return;
    }
...
gint
procmsg_save_message_as_text (MsgInfo * msginfo, const gchar * dest, const gchar * encoding, gboolean all_headers)
{
  ...
  while (partinfo != NULL)
    {
      ...
      if (partinfo->mime_type == MIME_TEXT || partinfo->mime_type == MIME_TEXT_HTML)
        {
          if (!partinfo->main && partinfo->parent && partinfo->parent->children != partinfo)
            {
      ...
      if (partinfo->parent && partinfo->parent->content_type &&
          !g_ascii_strcasecmp (partinfo->parent->content_type, "multipart/alternative"))
 ...
 **/
gint
procmsg_concat_partial_messages (GSList * mlist, const gchar * file)
{
  ...
  for (cur = mlist; cur != NULL; cur = cur->next)
    {
      ...
      if (!fp)
        continue;
      if (!mimeinfo->content_type || g_ascii_strcasecmp (mimeinfo->content_type, "message/partial") != 0)
        goto skip;
      ...
      if ((t > 0 && total != t) || (total > 0 && n > total) || strcmp (part_id, cur_id) != 0)
        {
          debug_print ("skip\n");
          g_free (cur_id);
          goto skip;
  ...
  for (i = 0; i < total; i++)
    {
          ...
          while (procheader_get_one_field (buf, sizeof (buf), fp, NULL) != -1)
            {
              if (!g_ascii_strncasecmp (buf, "Content-", 8) ||
                  ...
                  !g_ascii_strncasecmp (buf, "Encrypted", 9) || !g_ascii_strncasecmp (buf, "MIME-Version", 12))
                continue;
              fputs (buf, tmp_fp);
              fputs ("\n", tmp_fp);
            }
      ...
      if (append_file_part (fp, ftell (fp), out_size - empty_line_size, tmp_fp) < 0)
        {
...
static gboolean
procmsg_get_flags (FolderItem * item, gint num, MsgPermFlags * flags)
{
  FILE *fp;
  guint32 idata;
  gint read_num;
...
MsgInfo *
procmsg_get_msginfo (FolderItem * item, gint num)
{
  MsgInfo *msginfo;
  FolderType type;
...
MsgInfo *
procmsg_msginfo_copy (MsgInfo * msginfo)
{
  MsgInfo *newmsginfo;
...
MsgInfo *
procmsg_msginfo_get_full_info (MsgInfo * msginfo)
{
  ...
  if (msginfo->encinfo)
    {
      ...
      full_msginfo->encinfo->plaintext_file = g_strdup (msginfo->encinfo->plaintext_file);
      full_msginfo->encinfo->sigstatus = g_strdup (msginfo->encinfo->sigstatus);
      full_msginfo->encinfo->sigstatus_full = g_strdup (msginfo->encinfo->sigstatus_full);
      full_msginfo->encinfo->decryption_failed = msginfo->encinfo->decryption_failed;
    }
...
gboolean
procmsg_msginfo_equal (MsgInfo * msginfo_a, MsgInfo * msginfo_b)
{
  ...
  if (msginfo_a->folder == msginfo_b->folder &&
      ...
      msginfo_a->size == msginfo_b->size && msginfo_a->mtime == msginfo_b->mtime)
    return TRUE;
...
gint
procmsg_cmp_msgnum_for_sort (gconstpointer a, gconstpointer b)
{
  const MsgInfo *msginfo1 = a;
  const MsgInfo *msginfo2 = b;
...
CMP_FUNC_DEF (procmsg_cmp_by_label,
              ...
              MSG_GET_COLORLABEL (msginfo2->flags)) CMP_FUNC_DEF (procmsg_cmp_by_size, msginfo1->size - msginfo2->size)
#undef CMP_FUNC_DEF
#define CMP_FUNC_DEF(func_name, val)					\
static gint func_name(gconstpointer a, gconstpointer b)			\
{									\
...
{									\
  ...
  CMP_FUNC_DEF (procmsg_cmp_by_from, fromname) CMP_FUNC_DEF (procmsg_cmp_by_to, to)
#undef CMP_FUNC_DEF
     static gint
     procmsg_cmp_by_subject (gconstpointer a, gconstpointer b)
{
  const MsgInfo *msginfo1 = a;
  const MsgInfo *msginfo2 = b;
  gint ret;
  ...
  if (!msginfo1->subject)
    return (msginfo2->subject != NULL) * (cmp_func_sort_type == SORT_ASCENDING ? -1 : 1);
  if (!msginfo2->subject)
    return (cmp_func_sort_type == SORT_ASCENDING ? 1 : -1);
