...
gint
proc_mbox (FolderItem * dest, const gchar * mbox, GHashTable * folder_table)
{
  return proc_mbox_full (dest, mbox, folder_table,
                         folder_table ? TRUE : FALSE,
                         folder_table && prefs_common.enable_junk && prefs_common.filter_junk_on_recv ? TRUE : FALSE);
}

gint
proc_mbox_full (FolderItem * dest, const gchar * mbox,
                GHashTable * folder_table, gboolean apply_filter, gboolean filter_junk)
{
  FILE *mbox_fp;
  ...
  do
    {
      ...
      g_strstrip (rpath);
      g_snprintf (from_line, sizeof (from_line), "Return-Path: %s\n", rpath);
      g_free (rpath);
      ...
      msginfo = procheader_parse_file (tmp_file, fltinfo->flags, FALSE);
      if (!msginfo)
        {
      ...
      if (filter_junk && prefs_common.enable_junk && prefs_common.filter_junk_before && junk_rule)
        {
      ...
      if (!fltinfo->drop_done &&
          filter_junk && prefs_common.enable_junk && !prefs_common.filter_junk_before && junk_rule)
        {
          filter_apply_msginfo (&junk_fltlist, msginfo, fltinfo);
          if (fltinfo->drop_done)
            is_junk = TRUE;
      ...
      if (fltinfo->actions[FLT_ACTION_MOVE] == FALSE && fltinfo->actions[FLT_ACTION_DELETE] == FALSE)
        {
          ...
          fltinfo->dest_list = g_slist_append (fltinfo->dest_list, dest);
        }
      ...
      for (cur = fltinfo->dest_list; cur != NULL; cur = cur->next)
        {
          ...
          if (folder_table)
            {
              val = GPOINTER_TO_INT (g_hash_table_lookup (folder_table, drop_folder));
            }
          if (val == 0)
            {
              if (folder_table)
                {
                  g_hash_table_insert (folder_table, drop_folder, GINT_TO_POINTER (1));
                }
            }
        }

      if (!is_junk && fltinfo->actions[FLT_ACTION_DELETE] == FALSE && fltinfo->actions[FLT_ACTION_MARK_READ] == FALSE)
        new_msgs++;
...
gint
lock_mbox (const gchar * base, LockType type)
{
#ifdef G_OS_UNIX
  gint retval = 0;
      ...
      while (link (lockfile, locklink) < 0)
        {
          ...
          if (retry == 0)
            g_warning (_("mailbox is owned by another" " process, waiting...\n"));
          retry++;
          sleep (5);
        }
      g_unlink (lockfile);
...
gint
unlock_mbox (const gchar * base, gint fd, LockType type)
{
  if (type == LOCK_FILE)
    {
      gchar *lockfile;
...
gint
copy_mbox (const gchar * src, const gchar * dest)
{
  return copy_file (src, dest, TRUE);
}

void
empty_mbox (const gchar * mbox)
{
#if HAVE_TRUNCATE
...
gint
export_to_mbox (FolderItem * src, const gchar * mbox)
{
  GSList *mlist;
  gint ret = 0;
...
gint
export_msgs_to_mbox (FolderItem * src, GSList * mlist, const gchar * mbox)
{
  GSList *cur;
  MsgInfo *msginfo;
  FILE *msg_fp;
  ...
  for (cur = mlist; cur != NULL; cur = cur->next)
    {
      ...
      if (src->folder->ui_func)
        src->folder->ui_func (src->folder, src,
                              src->folder->ui_func_data ? src->folder->ui_func_data : GUINT_TO_POINTER (count));
      ...
      strncpy2 (buf,
                msginfo->from ? msginfo->from : cur_ac && cur_ac->address ? cur_ac->address : "unknown", sizeof (buf));
      extract_address (buf);
