...
HTMLParser *
html_parser_new (FILE * fp, CodeConverter * conv)
{
  HTMLParser *parser;
...
void
html_parser_destroy (HTMLParser * parser)
{
...
static HTMLState
html_read_line (HTMLParser * parser)
{
...
static void
html_append_char (HTMLParser * parser, gchar ch)
{
...
static void
html_append_str (HTMLParser * parser, const gchar * str, gint len)
{
...
static gchar *
html_find_char (HTMLParser * parser, gchar ch)
{
  gchar *p;
...
static gchar *
html_find_str (HTMLParser * parser, const gchar * str)
{
  gchar *p;
...
static gchar *
html_find_str_case (HTMLParser * parser, const gchar * str)
{
  gchar *p;
...
static HTMLState
html_parse_tag (HTMLParser * parser)
{
  gchar buf[HTMLBUFSIZE];
  HTMLTag *tag;
  ...
  if (!strcmp (tag->name, "br"))
  ...
  else if (!strcmp (tag->name, "hr"))
    {
      ...
      parser->state = HTML_HR;
           ...
           !strcmp (tag->name, "tr") || (tag->name[0] == 'h' && g_ascii_isdigit (tag->name[1])))
    {
      if (!parser->newline)
        {
          parser->space = FALSE;
      ...
      parser->state = HTML_NORMAL;
           ...
           (tag->name[0] == '/' && tag->name[1] == 'h' && g_ascii_isdigit (tag->name[1])))
    {
      if (!parser->empty_line)
        {
          parser->space = FALSE;
...
static void
html_parse_special (HTMLParser * parser)
{
  gchar symbol_name[9];
  gint n;
  const gchar *val;
...
static void
html_get_parenthesis (HTMLParser * parser, gchar * buf, gint len)
{
  gchar *p;
...
static gchar *
html_unescape_str (HTMLParser * parser, const gchar * str)
{
  const gchar *p = str;
  gchar symbol_name[9];
  gint n;
