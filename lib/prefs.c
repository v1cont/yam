...
static void prefs_config_parse_one_line (GHashTable * param_table, const gchar * buf);

GHashTable *
prefs_param_table_get (PrefParam * param)
{
  GHashTable *table;
  gint i;
...
void
prefs_param_table_destroy (GHashTable * param_table)
{
  g_hash_table_destroy (param_table);
}

void
prefs_read_config (PrefParam * param, const gchar * label, const gchar * rcfile, const gchar * encoding)
{
  FILE *fp;
...
static void
prefs_config_parse_one_line (GHashTable * param_table, const gchar * buf)
{
  PrefParam *param;
  const gchar *p = buf;
  gchar *name;
  ...
  switch (param->type)
    ...
    case P_INT:
      *((gint *) param->data) = (gint) atoi (value);
      break;
    case P_BOOL:
      *((gboolean *) param->data) = (*value == '0' || *value == '\0') ? FALSE : TRUE;
      break;
    case P_ENUM:
...
void
prefs_write_config (PrefParam * param, const gchar * label, const gchar * rcfile)
{
  FILE *orig_fp;
  PrefFile *pfile;
  gchar *rcpath;
      ...
      while (fgets (buf, sizeof (buf), orig_fp) != NULL)
        {
          ...
          if (buf[0] == '[')
            {
              TRY (fputc ('\n', pfile->fp) != EOF && fputs (buf, pfile->fp) != EOF);
              break;
            }
        }
      while (fgets (buf, sizeof (buf), orig_fp) != NULL)
...
gint
prefs_file_write_param (PrefFile * pfile, PrefParam * param)
{
  gint i;
  gchar buf[PREFSBUFSIZE];
...
PrefFile *
prefs_file_open (const gchar * path)
{
...
void
prefs_file_set_backup_generation (PrefFile * pfile, gint generation)
{
...
gint
prefs_file_get_backup_generation (PrefFile * pfile)
{
  PrefFilePrivate *priv = (PrefFilePrivate *) pfile;
...
gint
prefs_file_close (PrefFile * pfile)
{
  PrefFilePrivate *priv = (PrefFilePrivate *) pfile;
  FILE *fp;
  gchar *path;
      ...
      if (is_file_exist (bakpath))
        {
          ...
          for (i = generation; i > 0; i--)
            {
              bakpath_n = g_strdup_printf ("%s.%d", bakpath, i);
              if (i == 1)
                bakpath_p = g_strdup (bakpath);
              else
                bakpath_p = g_strdup_printf ("%s.%d", bakpath, i - 1);
              if (is_file_exist (bakpath_p))
                {
                  if (rename_force (bakpath_p, bakpath_n) < 0)
                    {
                      FILE_OP_ERROR (bakpath_p, "rename");
                    }
                }
              g_free (bakpath_p);
              g_free (bakpath_n);
...
gint
prefs_file_close_revert (PrefFile * pfile)
{
  gchar *tmppath;
...
void
prefs_set_default (PrefParam * param)
{
  gint i;
              ...
              if (!g_ascii_strncasecmp (param[i].defval, "ENV_", 4))
                {
                  ...
                  if (envstr)
                    {
                      tmp = conv_codeset_strdup (envstr, conv_get_locale_charset_str (), CS_UTF_8);
                      if (!tmp)
                        {
