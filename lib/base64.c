...
static const gchar base64char[64] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
...
void
base64_encode (gchar * out, const guchar * in, gint inlen)
{
  const guchar *inp = in;
  gchar *outp = out;
...
gint
base64_decode (guchar * out, const gchar * in, gint inlen)
{
  const gchar *inp = in;
  guchar *outp = out;
  gchar buf[4];
  ...
  while (inlen >= 4 && *inp != '\0')
    {
      ...
      *outp++ = ((BASE64VAL (buf[0]) << 2) & 0xfc) | ((BASE64VAL (buf[1]) >> 4) & 0x03);
      if (buf[2] != '=')
        {
          *outp++ = ((BASE64VAL (buf[1]) & 0x0f) << 4) | ((BASE64VAL (buf[2]) >> 2) & 0x0f);
          if (buf[3] != '=')
            {
              *outp++ = ((BASE64VAL (buf[2]) & 0x03) << 6) | (BASE64VAL (buf[3]) & 0x3f);
            }
        }
    }
...
gint
base64_decoder_decode (Base64Decoder * decoder, const gchar * in, guchar * out)
{
