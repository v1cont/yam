...
static void
smtp_session_destroy (Session * session)
{
...
smtp_auth (SMTPSession * session)
{
  ...
  if (session->forced_auth_type == SMTPAUTH_CRAM_MD5 ||
      (session->forced_auth_type == 0 && (session->avail_auth_type & SMTPAUTH_CRAM_MD5) != 0))
    smtp_auth_cram_md5 (session);
  else if (session->forced_auth_type == SMTPAUTH_PLAIN ||
           (session->forced_auth_type == 0 && (session->avail_auth_type & SMTPAUTH_PLAIN) != 0))
    smtp_auth_plain (session);
  else if (session->forced_auth_type == SMTPAUTH_LOGIN ||
           (session->forced_auth_type == 0 && (session->avail_auth_type & SMTPAUTH_LOGIN) != 0))
...
static gint
smtp_auth_recv (SMTPSession * session, const gchar * msg)
{
  gchar buf[SMTPBUFSIZE];
  ...
  switch (session->auth_type)
    ...
    case SMTPAUTH_LOGIN:
      ...
      if (!strncmp (msg, "334 ", 4))
        {
          base64_encode (buf, (guchar *) session->user, strlen (session->user));
      ...
      if (!strncmp (msg, "334 ", 4))
        {
          ...
          md5_hex_hmac (hexdigest, challenge, challengelen, (guchar *) buf, strlen (buf));
          g_free (challenge);
          ...
          session_send_msg (SESSION (session), SESSION_MSG_NORMAL, response64);
          log_print ("ESMTP> %s\n", response64);
          g_free (response64);
        }
      else
...
static gint
smtp_auth_login_user_recv (SMTPSession * session, const gchar * msg)
{
  gchar buf[SMTPBUFSIZE];
  ...
  if (!strncmp (msg, "334 ", 4))
    base64_encode (buf, (guchar *) session->pass, strlen (session->pass));
  else
    /* Server rejects AUTH */
...
static gint
smtp_ehlo_recv (SMTPSession * session, const gchar * msg)
{
...
static gint
smtp_starttls (SMTPSession * session)
{
...
static gint
smtp_auth_cram_md5 (SMTPSession * session)
{
  session->state = SMTP_AUTH;
...
static gint
smtp_auth_plain (SMTPSession * session)
{
  gchar *authstr;
  gint authlen;
  gchar *outbuf;
...
static gint
smtp_auth_login (SMTPSession * session)
{
  session->state = SMTP_AUTH;
...
static gint
smtp_send_data (SMTPSession * session)
{
  session->state = SMTP_SEND_DATA;

  session_send_data (SESSION (session), session->send_data_fp, session->send_data_len);
...
static gint
smtp_session_recv_msg (Session * session, const gchar * msg)
{
  SMTPSession *smtp_session = SMTP_SESSION (session);
  gboolean cont = FALSE;
...
static gint
smtp_session_send_data_finished (Session * session, guint len)
{
