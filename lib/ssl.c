...
static gchar *
find_certs_file (const gchar * certs_dir)
{
  gchar *certs_file;
...
ssl_init (void)
{
  ...
  if (ssl_ctx_SSLv23 == NULL)
  ...
  else
    {
      ...
      if ((certs_file || certs_dir) && !SSL_CTX_load_verify_locations (ssl_ctx_SSLv23, certs_file, certs_dir))
  ...
  if (ssl_ctx_TLSv1 == NULL)
  ...
  else
    {
      ...
      /* disable SSLv2/SSLv3 */
      SSL_CTX_set_options (ssl_ctx_TLSv1, SSL_OP_NO_SSLv2 | SSL_OP_NO_SSLv3);
      if ((certs_file || certs_dir) && !SSL_CTX_load_verify_locations (ssl_ctx_TLSv1, certs_file, certs_dir))
  ...
  certs_file = g_strconcat (get_rc_dir (), G_DIR_SEPARATOR_S, "trust.crt", NULL);
...
ssl_done (void)
{
  ...
  if (trust_list)
    {
      trust_file = g_strconcat (get_rc_dir (), G_DIR_SEPARATOR_S, "trust.crt", NULL);
      ...
      for (cur = trust_list; cur != NULL; cur = cur->next)
        {
          if (fp && !PEM_write_X509 (fp, (X509 *) cur->data))
            g_warning ("can't write X509 to PEM file: %s", trust_file);
...
gboolean
ssl_init_socket (SockInfo * sockinfo)
{
  return ssl_init_socket_with_method (sockinfo, SSL_METHOD_SSLv23);
}

static gint
x509_cmp_func (gconstpointer a, gconstpointer b)
{
  const X509 *xa = a;
  const X509 *xb = b;
...
gboolean
ssl_init_socket_with_method (SockInfo * sockinfo, SSLMethod method)
{
  X509 *server_cert;
  gint err, ret;
      ...
      if (verify_result == X509_V_ERR_CERT_HAS_EXPIRED)
        {
          ...
          expired = TRUE;
                                    ...
                                    x509_cmp_func) || g_slist_find_custom (tmp_trust_list, server_cert, x509_cmp_func))
        {
          log_message ("SSL certificate of %s previously accepted\n", sockinfo->hostname);
          X509_free (server_cert);
          return TRUE;
        }
      else if (g_slist_find_custom (reject_list, server_cert, x509_cmp_func))
        {
      ...
      if (verify_result == X509_V_ERR_APPLICATION_VERIFICATION)
      ...
      else
        {
          g_warning ("%s: SSL certificate verify failed (%ld: %s)\n",
                     sockinfo->hostname, verify_result, X509_verify_cert_error_string (verify_result));
        }
      ...
      if (verify_ui_func)
        {
          ...
          res = verify_ui_func (sockinfo, sockinfo->hostname, server_cert, verify_result);
          /* 0: accept 1: temporarily accept -1: reject */
          if (res < 0)
            {
              debug_print ("SSL certificate of %s rejected\n", sockinfo->hostname);
#if 0
              reject_list = g_slist_prepend (reject_list, X509_dup (server_cert));
#endif
              X509_free (server_cert);
              return FALSE;
            }
...
void
ssl_done_socket (SockInfo * sockinfo)
{
  if (sockinfo->ssl)
    {
...
void
ssl_set_verify_func (SSLVerifyFunc func)
{
  verify_ui_func = func;
}
