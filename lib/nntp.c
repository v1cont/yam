...
static gint nntp_gen_command (NNTPSession * session, gchar * argbuf, const gchar * format, ...);


#if USE_SSL
Session *
nntp_session_new_full (const gchar * server, gushort port,
                       SocksInfo * socks_info, gchar * buf,
                       const gchar * userid, const gchar * passwd, SSLType ssl_type)
#else
Session *
nntp_session_new_full (const gchar * server, gushort port,
                       SocksInfo * socks_info, gchar * buf, const gchar * userid, const gchar * passwd)
#endif
{
  NNTPSession *session;
  SockInfo *sock;
  ...
  if (userid && passwd)
    {
      ...
      if (ok == NN_AUTHCONT)
        {
          ok = nntp_gen_send (sock, "AUTHINFO PASS %s", session->passwd);
          if (ok != NN_SUCCESS)
            {
              session_destroy (SESSION (session));
              return NULL;
...
#if USE_SSL
Session *
nntp_session_new (const gchar * server, gushort port, gchar * buf,
                  const gchar * userid, const gchar * passwd, SSLType ssl_type)
{
  return nntp_session_new_full (server, port, NULL, buf, userid, passwd, ssl_type);
}
#else
Session *
nntp_session_new (const gchar * server, gushort port, gchar * buf, const gchar * userid, const gchar * passwd)
{
...
static void
nntp_session_destroy (Session * session)
{
...
gint
nntp_group (NNTPSession * session, const gchar * group, gint * num, gint * first, gint * last)
{
  gint ok;
  gint resp;
  ...
  if (sscanf (buf, "%d %d %d %d", &resp, num, first, last) != 4)
    {
...
gint
nntp_get_article (NNTPSession * session, const gchar * cmd, gint num, gchar ** msgid)
{
  gint ok;
  gchar buf[NNTPBUFSIZE];
...
gint
nntp_article (NNTPSession * session, gint num, gchar ** msgid)
{
  return nntp_get_article (session, "ARTICLE", num, msgid);
}

gint
nntp_body (NNTPSession * session, gint num, gchar ** msgid)
{
  return nntp_get_article (session, "BODY", num, msgid);
}

gint
nntp_head (NNTPSession * session, gint num, gchar ** msgid)
{
  return nntp_get_article (session, "HEAD", num, msgid);
}

gint
nntp_stat (NNTPSession * session, gint num, gchar ** msgid)
{
  return nntp_get_article (session, "STAT", num, msgid);
}

gint
nntp_next (NNTPSession * session, gint * num, gchar ** msgid)
{
  gint ok;
  gint resp;
  gchar buf[NNTPBUFSIZE];
...
gint
nntp_xover (NNTPSession * session, gint first, gint last)
{
  gint ok;
  gchar buf[NNTPBUFSIZE];
...
gint
nntp_xhdr (NNTPSession * session, const gchar * header, gint first, gint last)
{
  gint ok;
  gchar buf[NNTPBUFSIZE];

  ok = nntp_gen_command (session, buf, "XHDR %s %d-%d", header, first, last);
  if (ok != NN_SUCCESS)
    return ok;
...
gint
nntp_list (NNTPSession * session)
{
  return nntp_gen_command (session, NULL, "LIST");
}

gint
nntp_post (NNTPSession * session, FILE * fp)
{
  gint ok;
  gchar buf[NNTPBUFSIZE];
  gchar *msg;
...
gint
nntp_newgroups (NNTPSession * session)
{
  return NN_SUCCESS;
}

gint
nntp_newnews (NNTPSession * session)
{
  return NN_SUCCESS;
}

gint
nntp_mode (NNTPSession * session, gboolean stream)
{
  gint ok;

  ok = nntp_gen_command (session, NULL, "MODE %s", stream ? "STREAM" : "READER");

  return ok;
}

static gint
nntp_ok (SockInfo * sock, gchar * argbuf)
{
  gint ok;
  gchar buf[NNTPBUFSIZE];
  ...
  if ((ok = nntp_gen_recv (sock, buf, sizeof (buf))) == NN_SUCCESS)
    {
      ...
      if ((buf[0] == '1' || buf[0] == '2' || buf[0] == '3') && (buf[3] == ' ' || buf[3] == '\0'))
        {
          if (argbuf)
            strcpy (argbuf, buf);
...
static gint
nntp_gen_send (SockInfo * sock, const gchar * format, ...)
{
  gchar buf[NNTPBUFSIZE];
  va_list args;
...
static gint
nntp_gen_recv (SockInfo * sock, gchar * buf, gint size)
{
...
static gint
nntp_gen_command (NNTPSession * session, gchar * argbuf, const gchar * format, ...)
{
  gchar buf[NNTPBUFSIZE];
  va_list args;
  gint ok;
