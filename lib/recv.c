...
gchar *
recv_bytes (SockInfo * sock, glong size)
{
  gchar *buf;
  glong count = 0;
...
gint
recv_write_to_file (SockInfo * sock, const gchar * filename)
{
  FILE *fp;
  gint ret;
...
gint
recv_bytes_write_to_file (SockInfo * sock, glong size, const gchar * filename)
{
  FILE *fp;
  gint ret;
...
gint
recv_write (SockInfo * sock, FILE * fp)
{
  gchar buf[BUFFSIZE];
  gint len;
  gint count = 0;
  ...
  for (;;)
    {
      ...
      if (len > 1 && buf[0] == '.' && buf[1] == '\r')
        {
          if (recv_ui_func)
            recv_ui_func (sock, count, bytes, recv_ui_func_data);
          break;
        }
      count++;
      bytes += len;
      ...
      if (recv_ui_func)
        {
          ...
          if (tv_cur.tv_sec - tv_prev.tv_sec > 0 || tv_cur.tv_nsec - tv_prev.tv_nsec > UI_REFRESH_INTERVAL)
            {
              gboolean ret;
              ret = recv_ui_func (sock, count, bytes, recv_ui_func_data);
              if (ret == FALSE)
                return -1;
              clock_gettime (CLOCK_MONOTONIC, &tv_prev);
            }
...
gint
recv_bytes_write (SockInfo * sock, glong size, FILE * fp)
{
  gchar *buf;
  gchar *prev, *cur;
  ...
  while ((cur = memchr (prev, '\r', size - (prev - buf))) != NULL)
    {
      ...
      if (fp && (fwrite (prev, sizeof (gchar), cur - prev, fp) == EOF || fwrite ("\n", sizeof (gchar), 1, fp) == EOF))
        {
          perror ("fwrite");
          g_warning (_("Can't write to file.\n"));
          fp = NULL;
  ...
  if (prev - buf < size && fp && fwrite (prev, sizeof (gchar), size - (prev - buf), fp) == EOF)
    {
      perror ("fwrite");
      g_warning (_("Can't write to file.\n"));
      fp = NULL;
...
void
recv_set_ui_func (RecvUIFunc func, gpointer data)
{
  recv_ui_func = func;
