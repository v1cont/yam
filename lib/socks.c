...
SocksInfo *
socks_info_new (SocksType type, const gchar * proxy_host,
                gushort proxy_port, const gchar * proxy_name, const gchar * proxy_pass)
{
...
void
socks_info_free (SocksInfo * socks_info)
{
  if (!socks_info)
    return;
...
gint
socks_connect (SockInfo * sock, const gchar * hostname, gushort port, SocksInfo * socks_info)
{
  ...
  debug_print ("socks_connect: connect to %s:%u via %s:%u\n",
               hostname, port, socks_info->proxy_host, socks_info->proxy_port);

  if (socks_info->type == SOCKS_SOCKS5)
    return socks5_connect (sock, hostname, port, socks_info->proxy_name, socks_info->proxy_pass);
...
gint
socks4_connect (SockInfo * sock, const gchar * hostname, gushort port)
{
  ...
  *((gushort *) (socks_req + 2)) = htons (port);
  ...
  if (socks_req[1] != 90)
    {
      g_warning ("socks4_connect: SOCKS4 connection to %u.%u.%u.%u:%u failed. (%u)", socks_req[4], socks_req[5],
                 socks_req[6], socks_req[7], ntohs (*(gushort *) (socks_req + 2)), socks_req[1]);
      return -1;
    }
...
gint
socks5_connect (SockInfo * sock, const gchar * hostname, gushort port,
                const gchar * proxy_name, const gchar * proxy_pass)
{
  ...
  if (socks_req[1] == 2)
    {
      ...
      if (socks_req[1] != 0)
        {
          g_warning ("socks5_connect: SOCKS5 authentication failed: user: %s (%u %u)",
                     proxy_name ? proxy_name : "(none)", socks_req[0], socks_req[1]);
          return -1;
        }
    }
  ...
  *((gushort *) (socks_req + 5 + len)) = htons (port);
  ...
  if (socks_req[1] != 0)
    {
      g_warning ("socks5_connect: SOCKS5 connection to %u.%u.%u.%u:%u failed. (%u)", socks_req[4], socks_req[5],
                 socks_req[6], socks_req[7], ntohs (*(gushort *) (socks_req + 8)), socks_req[1]);
      return -1;
    }
