...
static gint pop3_session_recv_data_as_file_finished (Session * session, FILE * fp, guint len);


gint
pop3_greeting_recv (Pop3Session * session, const gchar * msg)
{
...
#if USE_SSL
gint
pop3_stls_send (Pop3Session * session)
{
...
gint
pop3_stls_recv (Pop3Session * session)
{
...
gint
pop3_getauth_user_send (Pop3Session * session)
{
...
gint
pop3_getauth_pass_send (Pop3Session * session)
{
...
gint
pop3_getauth_apop_send (Pop3Session * session)
{
  gchar *start, *end;
  gchar *apop_str;
  SMD5 *md5;
...
gint
pop3_getrange_stat_send (Pop3Session * session)
{
...
gint
pop3_getrange_stat_recv (Pop3Session * session, const gchar * msg)
{
...
gint
pop3_getrange_last_send (Pop3Session * session)
{
...
gint
pop3_getrange_last_recv (Pop3Session * session, const gchar * msg)
{
  gint last;
...
gint
pop3_getrange_uidl_send (Pop3Session * session)
{
...
gint
pop3_getrange_uidl_recv (Pop3Session * session, const gchar * data, guint len)
{
  gchar id[IDLEN + 1];
  ...
  while (p < lastp)
    {
      ...
      if (!session->new_msg_exist &&
          (session->ac_prefs->getall || recv_time == RECV_TIME_NONE || session->ac_prefs->rmmail))
        {
          session->cur_msg = num;
          session->new_msg_exist = TRUE;
        }
...
gint
pop3_getsize_list_send (Pop3Session * session)
{
...
gint
pop3_getsize_list_recv (Pop3Session * session, const gchar * data, guint len)
{
  gchar buf[POPBUFSIZE];
  gint buf_len;
  guint num, size;
...
gint
pop3_retr_send (Pop3Session * session)
{
...
gint
pop3_retr_recv (Pop3Session * session, FILE * fp, guint len)
{
  gchar *file;
  gint drop_ok;
  ...
  session->msg[session->cur_msg].recv_time =
    drop_ok == DROP_DONT_RECEIVE ? RECV_TIME_KEEP : drop_ok == DROP_DELETE ? RECV_TIME_DELETE : session->current_time;

  return PS_SUCCESS;
}

gint
pop3_delete_send (Pop3Session * session)
{
...
gint
pop3_delete_recv (Pop3Session * session)
{
...
gint
pop3_logout_send (Pop3Session * session)
{
...
void
pop3_gen_send (Pop3Session * session, const gchar * format, ...)
{
...
Session *
pop3_session_new (PrefsAccount * account)
{
  Pop3Session *session;
  ...
  SESSION (session)->recv_data_as_file_finished = pop3_session_recv_data_as_file_finished;
...
#if USE_SSL
  SESSION (session)->port = account->set_popport ? account->popport : account->ssl_pop == SSL_TUNNEL ? 995 : 110;
...
static void
pop3_session_destroy (Session * session)
{
...
GHashTable *
pop3_get_uidl_table (PrefsAccount * ac_prefs)
{
  GHashTable *table;
  gchar *path;
  FILE *fp;
  ...
  path = g_strconcat (get_rc_dir (), G_DIR_SEPARATOR_S,
                      UIDL_DIR, G_DIR_SEPARATOR_S, ac_prefs->recv_server, "-", uid, NULL);
  g_free (uid);
...
gint
pop3_write_uidl_list (Pop3Session * session)
{
  gchar *path;
  PrefFile *pfile;
  Pop3MsgInfo *msg;
  ...
  path = g_strconcat (get_rc_dir (), G_DIR_SEPARATOR_S,
                      UIDL_DIR, G_DIR_SEPARATOR_S, session->ac_prefs->recv_server, "-", uid, NULL);
  g_free (uid);
...
gint
pop3_write_msg_to_file (const gchar * file, FILE * src_fp, guint len)
{
  FILE *fp;
  gchar buf[BUFFSIZE];
  gchar last_ch = '\0';
...
static Pop3State
pop3_lookup_next (Pop3Session * session)
{
  Pop3MsgInfo *msg;
  ...
  for (;;)
    {
      ...
      if (msg->recv_time == RECV_TIME_DELETE ||
          (ac->rmmail &&
           ...
           session->current_time - msg->recv_time >= ac->msg_leave_time * 24 * 60 * 60))
        {
...
Pop3ErrorValue
pop3_ok (Pop3Session * session, const gchar * msg)
{
  Pop3ErrorValue ok;
...
static gint
pop3_session_recv_msg (Session * session, const gchar * msg)
{
  Pop3Session *pop3_session = POP3_SESSION (session);
  gint val = PS_SUCCESS;
  const gchar *body;
  ...
  switch (pop3_session->state)
    ...
    case POP3_GETRANGE_LAST:
      ...
      else if ((val = pop3_getrange_last_recv (pop3_session, body)) != PS_SUCCESS)
        return -1;
...
static gint
pop3_session_recv_data_finished (Session * session, guchar * data, guint len)
{
  Pop3Session *pop3_session = POP3_SESSION (session);
...
static gint
pop3_session_recv_data_as_file_finished (Session * session, FILE * fp, guint len)
{
  Pop3Session *pop3_session = POP3_SESSION (session);
  ...
  if (pop3_session->msg[pop3_session->cur_msg].recv_time
      ...
      (pop3_session->ac_prefs->rmmail &&
       ...
       pop3_session->msg[pop3_session->cur_msg].recv_time != RECV_TIME_KEEP))
