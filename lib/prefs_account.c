...
PrefsAccount *
prefs_account_get_tmp_prefs (void)
{
  return &tmp_ac_prefs;
}

void
prefs_account_set_tmp_prefs (PrefsAccount * ac_prefs)
{
  tmp_ac_prefs = *ac_prefs;
}

void
prefs_account_apply_tmp_prefs (PrefsAccount * ac_prefs)
{
  *ac_prefs = tmp_ac_prefs;
}

PrefParam *
prefs_account_get_params (void)
{
  return param;
}
...
void
prefs_account_read_config (PrefsAccount * ac_prefs, const gchar * label)
{
  const gchar *p = label;
  gchar *rcpath;
  gint id;
...
void
prefs_account_write_config_all (GList * account_list)
{
  GList *cur;
  gchar *rcpath;
  PrefFile *pfile;
  ...
  for (cur = account_list; cur != NULL; cur = cur->next)
    {
      ...
      if (fprintf (pfile->fp, "[Account: %d]\n",
                   tmp_ac_prefs.account_id) <= 0 || prefs_file_write_param (pfile, param) < 0)
        {
...
void
prefs_account_free (PrefsAccount * ac_prefs)
{
  if (!ac_prefs)
    return;
