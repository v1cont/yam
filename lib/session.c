...
static gboolean session_read_data_as_file_cb (SockInfo * source, GIOCondition condition, gpointer data);
...
session_init (Session * session)
{
  ...
  clock_gettime (CLOCK_MONOTONIC, &session->tv_prev);
...
static SessionPrivData *
session_get_priv (Session * session)
{
  SessionPrivData *priv;
  GList *cur;
...
gint
session_connect (Session * session, const gchar * server, gushort port)
{
  return session_connect_full (session, server, port, NULL);
}

gint
session_connect_full (Session * session, const gchar * server, gushort port, SocksInfo * socks_info)
{
  SessionPrivData *priv;
#ifndef G_OS_UNIX
  SockInfo *sock = NULL;
...
static gint
session_connect_cb (SockInfo * sock, gpointer data)
{
  ...
  if (sock->state == CONN_LOOKUPFAILED)
    ...
    {
      g_warning ("can't connect to server (ConnectionState: %d).", sock->state);
      session->state = SESSION_ERROR;
      priv->error_val = SESSION_ERROR_CONNFAIL;
      return -1;
    }
  ...
  if (priv->socks_info)
    {
      ...
      if (socks_connect (sock, session->server, session->port, priv->socks_info) < 0)
        {
...
void
session_set_timeout (Session * session, guint interval)
{
  ...
  if (interval > 0)
    session->timeout_tag = g_timeout_add_full (G_PRIORITY_LOW, interval, session_timeout_cb, session, NULL);
  else
    session->timeout_tag = 0;
}
...
void
session_set_recv_message_notify (Session * session, RecvMsgNotify notify_func, gpointer data)
{
...
void
session_set_recv_data_notify (Session * session, RecvDataNotify notify_func, gpointer data)
{
...
void
session_set_send_data_notify (Session * session, SendDataNotify notify_func, gpointer data)
{
...
#if USE_SSL
gint
session_start_tls (Session * session)
{
  gboolean nb_mode;
...
gint
session_send_msg (Session * session, SessionMsgType type, const gchar * msg)
{
  gboolean ret;
...
gint
session_recv_msg (Session * session)
{
  g_return_val_if_fail (session->sock != NULL, -1);
  ...
  if (session->read_buf_len > 0)
    session->idle_tag = g_idle_add (session_recv_msg_idle_cb, session);
  else
    session->io_tag = sock_add_watch (session->sock, G_IO_IN, session_read_msg_cb, session);
...
gint
session_send_data (Session * session, FILE * data_fp, guint size)
{
  gboolean ret;
...
gint
session_recv_data (Session * session, guint size, const gchar * terminator)
{
  g_return_val_if_fail (session->sock != NULL, -1);
  ...
  if (session->read_buf_len > 0)
    session->idle_tag = g_idle_add (session_recv_data_idle_cb, session);
  else
...
gint
session_recv_data_as_file (Session * session, guint size, const gchar * terminator)
{
  g_return_val_if_fail (session->sock != NULL, -1);
  ...
  if (session->read_buf_len > 0)
    session->idle_tag = g_idle_add (session_recv_data_as_file_idle_cb, session);
  else
    session->io_tag = sock_add_watch (session->sock, G_IO_IN, session_read_data_as_file_cb, session);

  return 0;
}

static gboolean
session_recv_data_as_file_idle_cb (gpointer data)
{
  Session *session = SESSION (data);
  gboolean ret;
...
static gboolean
session_read_msg_cb (SockInfo * source, GIOCondition condition, gpointer data)
{
  Session *session = SESSION (data);
  SessionPrivData *priv;
...
static gboolean
session_read_data_cb (SockInfo * source, GIOCondition condition, gpointer data)
{
  Session *session = SESSION (data);
  SessionPrivData *priv;
  GByteArray *data_buf;
  ...
  g_byte_array_append (data_buf, (guchar *) session->read_buf_p, session->read_buf_len);
      ...
      if (memcmp (data_buf->data, session->read_data_terminator, terminator_len) == 0)
        complete = TRUE;
               ...
               memcmp (data_buf->data + data_buf->len -
                       (terminator_len + 2), "\r\n", 2) == 0 &&
               memcmp (data_buf->data + data_buf->len -
                       terminator_len, session->read_data_terminator, terminator_len) == 0)
        complete = TRUE;
    }
  ...
  if (!complete)
    {
      ...
      if (tv_cur.tv_sec - session->tv_prev.tv_sec > 0 ||
          tv_cur.tv_nsec - session->tv_prev.tv_nsec > UI_REFRESH_INTERVAL)
        {
          if (session->recv_data_progressive_notify)
            session->recv_data_progressive_notify
              (session, data_buf->len, 0, session->recv_data_progressive_notify_data);
          clock_gettime (CLOCK_MONOTONIC, &session->tv_prev);
        }
      return TRUE;
    }
  ...
  /* callback */
  ret = session->recv_data_finished (session, (guchar *) data_buf->data, data_len);
...
static gboolean
session_read_data_as_file_cb (SockInfo * source, GIOCondition condition, gpointer data)
{
  Session *session = SESSION (data);
  SessionPrivData *priv;
  gint terminator_len;
  ...
  if (session->read_buf_len == 0)
    {
      read_len = sock_read (session->sock, session->read_buf_p, READ_BUF_LEFT ());
  ...
  if (buf_data_len >= terminator_len)
    {
      if (session->read_data_pos == 0 &&
          buf_data_len == terminator_len && memcmp (data_begin_p, session->read_data_terminator, terminator_len) == 0)
        complete = TRUE;
  ...
  if (!complete)
    {
      ...
      if (buf_data_len <= PREREAD_SIZE)
        {
          if (data_begin_p > session->read_buf)
            {
              memmove (session->read_buf, data_begin_p, buf_data_len);
              data_begin_p = session->read_buf;
              session->read_buf_p = session->read_buf + session->preread_len;
            }
      ...
      if (fwrite (data_begin_p, write_len, 1, session->read_data_fp) < 1)
        {
          g_warning ("session_read_data_as_file_cb: " "writing data to file failed\n");
          session->state = SESSION_ERROR;
          priv = session_get_priv (session);
      ...
      memmove (session->read_buf, data_begin_p + write_len, PREREAD_SIZE);
      ...
      if (tv_cur.tv_sec - session->tv_prev.tv_sec > 0 ||
          tv_cur.tv_nsec - session->tv_prev.tv_nsec > UI_REFRESH_INTERVAL)
        {
          if (session->recv_data_progressive_notify)
            session->recv_data_progressive_notify
              (session, session->read_data_pos, 0, session->recv_data_progressive_notify_data);
          clock_gettime (CLOCK_MONOTONIC, &session->tv_prev);
        }
  ...
  if (write_len > 0 && fwrite (data_begin_p, write_len, 1, session->read_data_fp) < 1)
    {
      g_warning ("session_read_data_as_file_cb: " "writing data to file failed\n");
      session->state = SESSION_ERROR;
      priv = session_get_priv (session);
  ...
  /* callback */
  ret = session->recv_data_as_file_finished (session, session->read_data_fp, session->read_data_pos);

  fclose (session->read_data_fp);
  session->read_data_fp = NULL;

  if (session->recv_data_notify)
    session->recv_data_notify (session, session->read_data_pos, session->recv_data_notify_data);
...
static gint
session_write_buf (Session * session)
{
  gint write_len;
  gint to_write_len;
  SessionPrivData *priv;
...
static gint
session_write_data (Session * session, gint * nwritten)
{
  ...
  if (session->write_data_pos + write_len < session->write_data_len)
    {
      ...
      if (write_len < to_write_len)
        {
          if (fseek (session->write_data_fp, session->write_data_pos, SEEK_SET) < 0)
            {
...
static gboolean
session_write_msg_cb (SockInfo * source, GIOCondition condition, gpointer data)
{
  Session *session = SESSION (data);
  SessionPrivData *priv;
  gint ret;
...
static gboolean
session_write_data_cb (SockInfo * source, GIOCondition condition, gpointer data)
{
  Session *session = SESSION (data);
  SessionPrivData *priv;
          ...
          tv_cur.tv_nsec - session->tv_prev.tv_nsec > UI_REFRESH_INTERVAL)
        {
          ...
          if (session->send_data_progressive_notify)
            session->send_data_progressive_notify
              (session, session->write_data_pos, write_data_len, session->send_data_progressive_notify_data);
          clock_gettime (CLOCK_MONOTONIC, &session->tv_prev);
        }
      return TRUE;
    }
