...
static gboolean filter_match_cond (FilterCond * cond, MsgInfo * msginfo, GSList * hlist, FilterInfo * fltinfo);
static gboolean filter_match_header_cond (FilterCond * cond, GSList * hlist);
static gboolean filter_match_in_addressbook (FilterCond * cond, GSList * hlist, FilterInfo * fltinfo);
...
gint
filter_apply (GSList * fltlist, const gchar * file, FilterInfo * fltinfo)
{
  MsgInfo *msginfo;
  gint ret = 0;
  ...
  fltinfo->flags.tmp_flags =
    (fltinfo->flags.tmp_flags & ~MSG_CACHED_FLAG_MASK) | (msginfo->flags.tmp_flags & MSG_CACHED_FLAG_MASK);
...
gint
filter_apply_msginfo (GSList * fltlist, MsgInfo * msginfo, FilterInfo * fltinfo)
{
  gchar *file;
  GSList *hlist, *cur;
  FilterRule *rule;
      ...
      if (matched)
        {
          ...
          if (fltinfo->drop_done == TRUE || fltinfo->actions[FLT_ACTION_STOP_EVAL] == TRUE)
            break;
        }
    }
...
gint
filter_action_exec (FilterRule * rule, MsgInfo * msginfo, const gchar * file, FilterInfo * fltinfo)
{
  ...
  for (cur = rule->action_list; cur != NULL; cur = cur->next)
    {
      ...
      switch (action->type)
        ...
        case FLT_ACTION_COLOR_LABEL:
          ...
          MSG_UNSET_PERM_FLAGS (fltinfo->flags, MSG_CLABEL_FLAG_MASK);
          MSG_SET_COLORLABEL_VALUE (fltinfo->flags, action->int_value);
          fltinfo->actions[action->type] = TRUE;
          break;
        ...
        case FLT_ACTION_EXEC:
          cmdline = g_strconcat (action->str_value, " \"", file, "\"", NULL);
  ...
  for (cur = rule->action_list; cur != NULL; cur = cur->next)
    {
      ...
      switch (action->type)
        ...
        case FLT_ACTION_COPY:
          dest_folder = folder_find_item_from_identifier (action->str_value);
          if (!dest_folder)
            {
          ...
          if (msginfo->folder)
                  ...
                  if (action->type == FLT_ACTION_COPY)
                    {
                      ...
                      val = folder_item_copy_msg (dest_folder, msginfo);
          ...
          else
            {
              ...
              save_flags = msginfo->flags;
              msginfo->flags = fltinfo->flags;
              if (folder_item_add_msg_msginfo (dest_folder, msginfo, FALSE) < 0)
                {
                  msginfo->flags = save_flags;
                  fltinfo->error = FLT_ERROR_ERROR;
                  return -1;
          ...
          fltinfo->dest_list = g_slist_append (fltinfo->dest_list, dest_folder);
...
static gboolean
strmatch_regex (const gchar * haystack, const gchar * needle)
{
  gint ret = 0;
  regex_t preg;
...
gboolean
filter_match_rule (FilterRule * rule, MsgInfo * msginfo, GSList * hlist, FilterInfo * fltinfo)
{
  FilterCond *cond;
  GSList *cur;
  gboolean matched;
  ...
  if (rule->bool_op == FLT_AND)
    {
      for (cur = rule->cond_list; cur != NULL; cur = cur->next)
        {
          ...
          if (cond->type >= FLT_COND_SIZE_GREATER)
            {
              matched = filter_match_cond (cond, msginfo, hlist, fltinfo);
              if (matched == FALSE)
                return FALSE;
            }
        }
      ...
      for (cur = rule->cond_list; cur != NULL; cur = cur->next)
        {
          cond = (FilterCond *) cur->data;
          if (cond->type == FLT_COND_BODY || cond->type == FLT_COND_CMD_TEST)
            {
              matched = filter_match_cond (cond, msginfo, hlist, fltinfo);
              if (matched == FALSE)
                return FALSE;
...
static gboolean
filter_match_cond (FilterCond * cond, MsgInfo * msginfo, GSList * hlist, FilterInfo * fltinfo)
{
  gint ret;
  ...
  switch (cond->type)
    ...
    case FLT_COND_TO_OR_CC:
      if (cond->match_type == FLT_IN_ADDRESSBOOK)
        ...
        return filter_match_header_cond (cond, hlist);
    case FLT_COND_BODY:
      matched = procmime_find_string (msginfo, cond->str_value, cond->match_func);
      break;
...
static gboolean
filter_match_header_cond (FilterCond * cond, GSList * hlist)
{
  gboolean matched = FALSE;
  gboolean not_match = FALSE;
  GSList *cur;
  ...
  for (cur = hlist; cur != NULL; cur = cur->next)
    {
      ...
      switch (cond->type)
        {
        case FLT_COND_HEADER:
          if (!g_ascii_strcasecmp (header->name, cond->header_name))
            {
              if (!cond->str_value || cond->match_func (header->body, cond->str_value))
                matched = TRUE;
            }
          break;
        case FLT_COND_ANY_HEADER:
          ...
          break;
        case FLT_COND_TO_OR_CC:
          if (!g_ascii_strcasecmp (header->name, "To") || !g_ascii_strcasecmp (header->name, "Cc"))
            {
              if (!cond->str_value || cond->match_func (header->body, cond->str_value))
                matched = TRUE;
            }
...
static gboolean
filter_match_in_addressbook (FilterCond * cond, GSList * hlist, FilterInfo * fltinfo)
{
  gboolean matched = FALSE;
  gboolean not_match = FALSE;
  GSList *cur;
...
gboolean
filter_rule_requires_full_headers (FilterRule * rule)
{
  GSList *cur;
      ...
      if (cond->type == FLT_COND_HEADER && name)
        {
          if (g_ascii_strcasecmp (name, "Date") != 0 &&
              ...
              g_ascii_strcasecmp (name, "Newsgroups") != 0 && g_ascii_strcasecmp (name, "Subject") != 0)
            return TRUE;
        }
      else if (cond->type == FLT_COND_ANY_HEADER || cond->type == FLT_COND_TO_OR_CC)
        return TRUE;
    }
...
static gboolean
filter_xml_node_func (GNode * node, gpointer data)
{
  XMLNode *xmlnode;
  GList *list;
  ...
  for (list = xmlnode->tag->attr; list != NULL; list = list->next)
    {
      ...
      if (attr && attr->name && attr->value && !strcmp (attr->name, "bool"))
        {
  ...
  for (cond_child = child->children; cond_child != NULL; cond_child = cond_child->next)
    {
      ...
      if (!xmlnode || !xmlnode->tag || !xmlnode->tag->tag)
        continue;
      ...
      for (list = xmlnode->tag->attr; list != NULL; list = list->next)
        {
          ...
          STR_SWITCH (attr->name) STR_CASE_BEGIN ("type") type = attr->value;
          STR_CASE ("name") name = attr->value;
          STR_CASE ("case") case_sens = TRUE;
          STR_CASE ("recursive") if (!strcmp (attr->value, "true"))
            recursive = TRUE;
          else
            recursive = FALSE;
        STR_CASE_END}
      ...
      if (type)
        {
          filter_rule_match_type_str_to_enum (type, &match_type, &match_flag);
        }
      if (case_sens)
        match_flag |= FLT_CASE_SENS;
      value = xmlnode->element;

      STR_SWITCH (xmlnode->tag->tag) STR_CASE_BEGIN ("match-header") cond_type = FLT_COND_HEADER;
      STR_CASE ("match-any-header") cond_type = FLT_COND_ANY_HEADER;
      STR_CASE ("match-to-or-cc") cond_type = FLT_COND_TO_OR_CC;
      STR_CASE ("match-body-text") cond_type = FLT_COND_BODY;
      STR_CASE ("command-test") cond_type = FLT_COND_CMD_TEST;
      STR_CASE ("size") cond_type = FLT_COND_SIZE_GREATER;
      STR_CASE ("age") cond_type = FLT_COND_AGE_GREATER;
      STR_CASE ("unread") cond_type = FLT_COND_UNREAD;
      STR_CASE ("mark") cond_type = FLT_COND_MARK;
      STR_CASE ("color-label") cond_type = FLT_COND_COLOR_LABEL;
      STR_CASE ("mime") cond_type = FLT_COND_MIME;
      STR_CASE ("account-id") cond_type = FLT_COND_ACCOUNT;
      STR_CASE ("target-folder") target_folder = value;
      continue;
      STR_CASE_END cond = filter_cond_new (cond_type, match_type, match_flag, name, value);
  ...
  for (action_child = child->children; action_child != NULL; action_child = action_child->next)
    {
      FilterAction *action;
      ...
      STR_SWITCH (xmlnode->tag->tag) STR_CASE_BEGIN ("move") action_type = FLT_ACTION_MOVE;
      STR_CASE ("copy") action_type = FLT_ACTION_COPY;
      STR_CASE ("not-receive") action_type = FLT_ACTION_NOT_RECEIVE;
      STR_CASE ("delete") action_type = FLT_ACTION_DELETE;
      STR_CASE ("exec") action_type = FLT_ACTION_EXEC;
      STR_CASE ("exec-async") action_type = FLT_ACTION_EXEC_ASYNC;
      STR_CASE ("mark") action_type = FLT_ACTION_MARK;
      STR_CASE ("color-label") action_type = FLT_ACTION_COLOR_LABEL;
      STR_CASE ("mark-as-read") action_type = FLT_ACTION_MARK_READ;
      STR_CASE ("forward") action_type = FLT_ACTION_FORWARD;
      STR_CASE ("forward-as-attachment") action_type = FLT_ACTION_FORWARD_AS_ATTACHMENT;
      STR_CASE ("redirect") action_type = FLT_ACTION_REDIRECT;
      STR_CASE ("stop-eval") action_type = FLT_ACTION_STOP_EVAL;
      STR_CASE_END action = filter_action_new (action_type, xmlnode->element);
...
GSList *
filter_xml_node_to_filter_list (GNode * node)
{
  GSList *fltlist = NULL;

  g_return_val_if_fail (node != NULL, NULL);

  g_node_traverse (node, G_PRE_ORDER, G_TRAVERSE_ALL, 2, filter_xml_node_func, &fltlist);
...
GSList *
filter_read_file (const gchar * file)
{
  GNode *node;
  GSList *list;
...
filter_read_config (void)
{
  ...
  rcpath = g_strconcat (get_rc_dir (), G_DIR_SEPARATOR_S, FILTER_LIST, NULL);
...
void
filter_write_file (GSList * list, const gchar * file)
{
  PrefFile *pfile;
  GSList *cur;
  ...
  for (cur = list; cur != NULL; cur = cur->next)
    {
      ...
      for (cur_cond = rule->cond_list; cur_cond != NULL; cur_cond = cur_cond->next)
        {
          ...
          switch (cond->match_type)
            ...
            case FLT_CONTAIN:
              strncpy2 (match_type,
                        FLT_IS_NOT_MATCH (cond->match_flag) ? "not-contain" : "contains", sizeof (match_type));
              break;
            case FLT_EQUAL:
              strncpy2 (match_type, FLT_IS_NOT_MATCH (cond->match_flag) ? "is-not" : "is", sizeof (match_type));
              break;
            case FLT_REGEX:
              strncpy2 (match_type, FLT_IS_NOT_MATCH (cond->match_flag) ? "not-regex" : "regex", sizeof (match_type));
              break;
            case FLT_IN_ADDRESSBOOK:
              strncpy2 (match_type,
                        FLT_IS_NOT_MATCH (cond->match_flag)
                        ? "not-in-addressbook" : "in-addressbook", sizeof (match_type));
              break;
            default:
              match_type[0] = '\0';
              break;
          ...
          switch (cond->type)
            ...
            case FLT_COND_SIZE_GREATER:
              ...
              ADD_ATTR ("type", FLT_IS_NOT_MATCH (cond->match_flag) ? "lt" : "gt");
              break;
            case FLT_COND_AGE_GREATER:
            ...
            case FLT_COND_UNREAD:
              ...
              ADD_ATTR ("type", FLT_IS_NOT_MATCH (cond->match_flag) ? "is-not" : "is");
              break;
            case FLT_COND_MARK:
              NODE_NEW ("mark", NULL);
              ADD_ATTR ("type", FLT_IS_NOT_MATCH (cond->match_flag) ? "is-not" : "is");
      ...
      if (rule->target_folder)
        {
          ...
          ADD_ATTR ("recursive", rule->recursive ? "true" : "false");
          fputs ("            ", pfile->fp);
          xml_file_put_node (pfile->fp, node);
          xml_free_node (node);
        }
      ...
      for (cur_action = rule->action_list; cur_action != NULL; cur_action = cur_action->next)
        {
          ...
          switch (action->type)
            ...
            case FLT_ACTION_FORWARD_AS_ATTACHMENT:
              NODE_NEW ("forward-as-attachment", action->str_value);
              break;
...
gchar *
filter_get_str (FilterRule * rule)
{
  gchar *str;
  ...
  for (cur = rule->action_list; cur != NULL; cur = cur->next)
    {
      action = (FilterAction *) cur->data;
      if (action->type == FLT_ACTION_MOVE ||
          action->type == FLT_ACTION_NOT_RECEIVE || action->type == FLT_ACTION_DELETE)
        break;
    }
...
FilterRule *
filter_read_str (const gchar * str)
{
  FilterRule *rule;
  ...
  if (*name1)
    {
      ...
      if (flag & FLT_O_CASE_SENS)
        match_flag |= FLT_CASE_SENS;
      cond = filter_cond_new (FLT_COND_HEADER, match_type, match_flag, name1, body1);
      cond_list = g_slist_append (cond_list, cond);
    }
  if (*name2)
    {
      ...
      if (flag & FLT_O_CASE_SENS)
        match_flag |= FLT_CASE_SENS;
      cond = filter_cond_new (FLT_COND_HEADER, match_type, match_flag, name2, body2);
      cond_list = g_slist_append (cond_list, cond);
    }

  action = filter_action_new (FLT_ACTION_MOVE, *dest ? g_strdup (dest) : NULL);
  if (action1)
    {
      switch (*action1)
        {
...
void
filter_set_addressbook_func (FilterInAddressBookFunc func)
{
  default_addrbook_func = func;
}

FilterInAddressBookFunc
filter_get_addressbook_func (void)
{
  return default_addrbook_func;
}

FilterRule *
filter_rule_new (const gchar * name, FilterBoolOp bool_op, GSList * cond_list, GSList * action_list)
{
  FilterRule *rule;
...
FilterCond *
filter_cond_new (FilterCondType type,
                 FilterMatchType match_type, FilterMatchFlag match_flag, const gchar * header, const gchar * value)
{
  FilterCond *cond;
  ...
  if (type == FLT_COND_HEADER)
    cond->header_name = (header && *header) ? g_strdup (header) : NULL;
  else
    cond->header_name = NULL;

  cond->str_value = (value && *value) ? g_strdup (value) : NULL;
  if (type == FLT_COND_SIZE_GREATER || type == FLT_COND_AGE_GREATER || type == FLT_COND_ACCOUNT)
...
FilterAction *
filter_action_new (FilterActionType type, const gchar * str)
{
  FilterAction *action;
...
FilterRule *
filter_junk_rule_create (PrefsAccount * account, FolderItem * default_junk, gboolean is_manual)
{
  FilterRule *rule;
  FilterCond *cond;
  FilterAction *action;
  ...
  if (prefs_common.nofilter_junk_sender_in_book)
    {
      cond = filter_cond_new (FLT_COND_HEADER, FLT_IN_ADDRESSBOOK, FLT_NOT_MATCH, "From", NULL);
      cond_list = g_slist_append (cond_list, cond);
    }

  cond = filter_cond_new (FLT_COND_CMD_TEST, 0, 0, NULL, prefs_common.junk_classify_cmd);
  cond_list = g_slist_append (cond_list, cond);
  ...
  if (is_manual)
    rule = filter_rule_new (_("Junk mail filter (manual)"), FLT_AND, cond_list, action_list);
  else
    rule = filter_rule_new (_("Junk mail filter"), FLT_AND, cond_list, action_list);
...
void
filter_rule_rename_dest_path (FilterRule * rule, const gchar * old_path, const gchar * new_path)
{
  FilterAction *action;
  GSList *cur;
  gchar *base;
  ...
  for (cur = rule->action_list; cur != NULL; cur = cur->next)
    {
      ...
      if (action->type != FLT_ACTION_MOVE && action->type != FLT_ACTION_COPY)
        continue;

      if (action->str_value && !strncmp (old_path, action->str_value, oldpathlen))
        {
          ...
          debug_print ("filter_rule_rename_dest_path(): " "renaming %s -> %s\n", action->str_value, dest_path);
...
void
filter_rule_delete_action_by_dest_path (FilterRule * rule, const gchar * path)
{
  FilterAction *action;
  GSList *cur;
  GSList *next;
  ...
  for (cur = rule->action_list; cur != NULL; cur = next)
    {
      ...
      if (action->str_value &&
          ...
          (action->str_value[pathlen] == '/' || action->str_value[pathlen] == '\0'))
        {
          debug_print ("filter_rule_delete_action_by_dest_path(): " "deleting %s\n", action->str_value);
          rule->action_list = g_slist_remove (rule->action_list, action);
...
void
filter_list_rename_path (const gchar * old_path, const gchar * new_path)
{
  GSList *cur;
...
void
filter_list_delete_path (const gchar * path)
{
  GSList *cur;
  GSList *next;
...
void
filter_rule_match_type_str_to_enum (const gchar * match_type, FilterMatchType * type, FilterMatchFlag * flag)
{
...
void
filter_get_keyword_from_msg (MsgInfo * msginfo, gchar ** header, gchar ** key, FilterCreateType type)
{
...
void
filter_rule_list_free (GSList * fltlist)
{
  GSList *cur;
...
void
filter_cond_list_free (GSList * cond_list)
{
  GSList *cur;
...
void
filter_action_list_free (GSList * action_list)
{
  GSList *cur;
...
static void
filter_cond_free (FilterCond * cond)
{
...
static void
filter_action_free (FilterAction * action)
{
  g_free (action->str_value);
  g_free (action);
}
...
void
filter_info_free (FilterInfo * fltinfo)
{
